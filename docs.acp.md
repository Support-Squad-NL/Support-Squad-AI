# SupportSquadAI ACP Bridge

This document describes how the SupportSquadAI ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`supportsquadai acp` exposes an ACP agent over stdio and forwards prompts to a running
SupportSquadAI Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `supportsquadai acp` and speaks ACP messages over stdio.
...
supportsquadai acp --reset-session
```
...
`supportsquadai acp` resolves the Gateway URL and auth from CLI flags or config:
