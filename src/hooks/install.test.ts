...
const fixtureRoot = path.join(os.tmpdir(), `supportsquadai-hook-install-${randomUUID()}`);
let tempDirIndex = 0;
...
describe("installHooksFromPath", () => {
  it("uses --ignore-scripts for dependency install", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@supportsquadai/test-hooks",
        version: "0.0.1",
        supportsquadai: { hooks: ["./hooks/one-hook"] },
    ...
    fs.writeFileSync(
      ...
      [
        ...
        'metadata: {"supportsquadai":{"events":["command:new"]}}',
        "---",
        "",
        "# One Hook",
      ].join("\n"),
  ...
  it("rejects hook pack entries that traverse outside package directory", async () => {
    ...
    expect(result.error).toContain("supportsquadai.hooks entry escapes package directory");
  });
  ...
  it("rejects hook pack entries that escape via symlink", async () => {
    ...
    expect(result.error).toContain("supportsquadai.hooks entry resolves outside package directory");
  });
});
...
describe("installHooksFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
      ...
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        return {
          ...
          stdout: JSON.stringify([
            {
              id: "@supportsquadai/test-hooks@0.0.1",
              name: "@supportsquadai/test-hooks",
              version: "0.0.1",
              filename: packedName,
    ...
    const result = await installHooksFromNpmSpec({
      spec: "@supportsquadai/test-hooks@0.0.1",
      hooksDir,
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(true);
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@supportsquadai/test-hooks@0.0.1");
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@supportsquadai/test-hooks@0.0.1",
    });
