...
homepage: https://docs.supportsquadai.ai/automation/hooks#command-logger
metadata:
  {
    "supportsquadai":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.supportsquadai/logs/commands.log`
...
`~/.supportsquadai/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.supportsquadai/logs/commands.log ~/.supportsquadai/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/supportsquadai`:
   ```
   /home/username/.supportsquadai/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
