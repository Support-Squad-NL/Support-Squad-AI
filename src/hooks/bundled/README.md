...
This directory contains hooks that ship with SupportSquadAI. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.supportsquadai/supportsquadai.json`:
...
2. Restart gateway: `pkill -9 -f 'supportsquadai.*gateway' && pnpm supportsquadai gateway`
3. Enable the hook: `supportsquadai hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.supportsquadai.ai/automation/hooks
