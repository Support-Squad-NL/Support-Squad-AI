...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => SupportSquadAIConfig;
  action?: "new" | "reset";
}): Promise<{ tempDir: string; files: string[]; memoryContent: string }> {
  const tempDir = await makeTempWorkspace("supportsquadai-session-memory-");
...
function makeSessionMemoryConfig(tempDir: string, messages?: number): SupportSquadAIConfig {
  return {
    agents: { defaults: { workspace: tempDir } },
