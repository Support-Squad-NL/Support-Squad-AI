...
async function ensureSupportSquadAIHooks(manifest: HookPackageManifest) {
  const hooks = manifest[MANIFEST_KEY]?.hooks;
  if (!Array.isArray(hooks)) {
    throw new Error("package.json missing supportsquadai.hooks");
  }
  const list = hooks.map((e) => (typeof e === "string" ? e.trim() : "")).filter(Boolean);
  if (list.length === 0) {
    throw new Error("package.json supportsquadai.hooks is empty");
  }
  return list;
}
...
async function installHookPackageFromDir(params: {
  ...
  try {
    hookEntries = await ensureSupportSquadAIHooks(manifest);
  } catch (err) {
  ...
  for (const entry of hookEntries) {
    ...
    if (!isPathInside(params.packageDir, hookDir)) {
      return {
        ok: false,
        error: `supportsquadai.hooks entry escapes package directory: ${entry}`,
      };
    }
    ...
    if (
      ...
      return {
        ok: false,
        error: `supportsquadai.hooks entry resolves outside package directory: ${entry}`,
      };
    }
...
export async function installHooksFromArchive(params: {
  ...
  return await withExtractedArchiveRoot({
    archivePath,
    tempDirPrefix: "supportsquadai-hook-",
    timeoutMs,
    logger,
...
export async function installHooksFromNpmSpec(params: {
  ...
  const flowResult = await installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "supportsquadai-hook-pack-",
    spec,
    timeoutMs,
