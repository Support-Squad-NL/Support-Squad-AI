...
describe("buildSlackSlashCommandMatcher", () => {
  it("matches with or without a leading slash", () => {
    const matcher = buildSlackSlashCommandMatcher("supportsquadai");

    expect(matcher.test("supportsquadai")).toBe(true);
    expect(matcher.test("/supportsquadai")).toBe(true);
  });
  ...
  it("does not match similar names", () => {
    ...
    expect(matcher.test("/supportsquadai-bot")).toBe(false);
    expect(matcher.test("supportsquadai-bot")).toBe(false);
  });
});
