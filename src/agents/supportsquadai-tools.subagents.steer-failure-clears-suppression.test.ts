...
} from "./supportsquadai-tools.subagents.test-harness.js";
import {
...
describe("supportsquadai-tools: subagents steer failure", () => {
  beforeEach(() => {
    resetSubagentRegistryForTests();
    callGatewayMock.mockClear();
    const storePath = path.join(
      os.tmpdir(),
      `supportsquadai-subagents-steer-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
    );
    setSubagentsConfigOverride({
      session: {
        mainKey: "main",
