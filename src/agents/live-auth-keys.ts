...
const GOOGLE_LIVE_SINGLE_KEY = "SUPPORTSQUADAI_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "SUPPORTSQUADAI_LIVE_ANTHROPIC_KEY",
    listVar: "SUPPORTSQUADAI_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "SUPPORTSQUADAI_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `SUPPORTSQUADAI_LIVE_${base}_KEY`;
