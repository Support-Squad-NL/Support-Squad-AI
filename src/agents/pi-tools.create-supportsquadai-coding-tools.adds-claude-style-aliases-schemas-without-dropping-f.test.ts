...
describe("createSupportSquadAICodingTools", () => {
  it("accepts Claude Code parameter aliases for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-alias-"));
    try {
      const tools = createSupportSquadAICodingTools({ workspaceDir: tmpDir });
  ...
  it("coerces structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-structured-write-"));
    try {
      const tools = createSupportSquadAICodingTools({ workspaceDir: tmpDir });
  ...
  it("coerces structured old/new text blocks for edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-structured-edit-"));
    try {
