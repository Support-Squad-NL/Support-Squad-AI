...
function resolveSkillsLimits(config?: SupportSquadAIConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const extraSkills = mergedExtraDirs.flatMap((dir) => {
    ...
    return loadSkills({
      dir: resolved,
      source: "supportsquadai-extra",
    });
  });
  const managedSkills = loadSkills({
    dir: managedSkillsDir,
    source: "supportsquadai-managed",
  });
  ...
  const workspaceSkills = loadSkills({
    ...
    source: "supportsquadai-workspace",
  });
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveSupportSquadAIMetadata(frontmatter),
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: SupportSquadAIConfig }): {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`supportsquadai skills check\` to audit.`
    : "";
  const prompt = [
    remoteNote,
    truncationNote,
