...
function asSupportSquadAIConfig(config: Partial<SupportSquadAIConfig>): SupportSquadAIConfig {
  return config as SupportSquadAIConfig;
}

function createToolConfig() {
  return asSupportSquadAIConfig({ agents: { list: [{ id: "main", default: true }] } });
}

function createMemoryGetToolOrThrow(config: SupportSquadAIConfig = createToolConfig()) {
...
describe("memory search citations", () => {
  it("appends source information when citations are enabled", async () => {
    ...
    const cfg = asSupportSquadAIConfig({
      memory: { citations: "on" },
