...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): SupportSquadAIConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as SupportSquadAIConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): SupportSquadAIConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as SupportSquadAIConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  async function expectAccountStickerSend(cfg: SupportSquadAIConfig, accountId = "media") {
    await handleTelegramAction(
