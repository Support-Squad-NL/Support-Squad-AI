...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.SUPPORTSQUADAI_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".supportsquadai", `workspace-${profile}`);
  }
  return path.join(home, ".supportsquadai", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".supportsquadai";
