...
} from "./supportsquadai-tools.subagents.sessions-spawn.test-harness.js";
import { resetSubagentRegistryForTests } from "./subagent-registry.js";

const fastModeEnv = vi.hoisted(() => {
  const previous = process.env.SUPPORTSQUADAI_TEST_FAST;
...
describe("supportsquadai-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnConfigOverride();
    setSessionsSpawnConfigOverride({
      session: {
  ...
  afterAll(() => {
    if (fastModeEnv.previous === undefined) {
      delete process.env.SUPPORTSQUADAI_TEST_FAST;
      return;
    }
    process.env.SUPPORTSQUADAI_TEST_FAST = fastModeEnv.previous;
  });
