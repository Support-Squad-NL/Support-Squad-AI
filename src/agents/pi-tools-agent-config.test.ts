...
import { createSupportSquadAICodingTools } from "./pi-tools.js";
...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: SupportSquadAIConfig) {
    return createSupportSquadAICodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<SupportSquadAIConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<SupportSquadAIConfig["agents"]>["list"]>[number]["tools"];
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createSupportSquadAICodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createSupportSquadAICodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createSupportSquadAICodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createSupportSquadAICodingTools({
      config: cfg,
    ...
    const bobTools = createSupportSquadAICodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createSupportSquadAICodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createSupportSquadAICodingTools({
      config: cfg,
