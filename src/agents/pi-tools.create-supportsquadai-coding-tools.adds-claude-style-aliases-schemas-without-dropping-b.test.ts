...
const defaultTools = createSupportSquadAICodingTools({ senderIsOwner: true });
...
describe("createSupportSquadAICodingTools", () => {
  ...
  it("gates apply_patch behind tools.exec.applyPatch for OpenAI models", () => {
    ...
    const openAiTools = createSupportSquadAICodingTools({
      config,
    ...
    const anthropicTools = createSupportSquadAICodingTools({
      config,
  ...
  it("respects apply_patch allowModels", () => {
    ...
    const allowed = createSupportSquadAICodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5.2",
    });
    ...
    const denied = createSupportSquadAICodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
