...
async function resolveContext(config: SupportSquadAIConfig, sessionKey: string, workspaceDir: string) {
...
function createWorkSetupCommandConfig(scope: "agent" | "shared"): SupportSquadAIConfig {
  return {
    agents: {
      defaults: {
        sandbox: {
    ...
    const cfg: SupportSquadAIConfig = {
      agents: {
        ...
        list: [
          {
            id: "isolated",
            workspace: "~/supportsquadai-isolated",
            sandbox: {
              mode: "all",
              scope: "agent",
