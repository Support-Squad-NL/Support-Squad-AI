...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses SUPPORTSQUADAI_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      SUPPORTSQUADAI_HOME: "/srv/supportsquadai-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/supportsquadai-home"), ".supportsquadai", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".supportsquadai", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("supportsquadai-workspace-");
