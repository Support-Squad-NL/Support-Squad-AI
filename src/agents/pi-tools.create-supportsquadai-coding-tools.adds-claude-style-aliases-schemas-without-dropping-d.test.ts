...
describe("createSupportSquadAICodingTools", () => {
  it("returns image metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-read-"));
    try {
      ...
      const contents = "Hello from supportsquadai read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    ...
    const tools = createSupportSquadAICodingTools({ sandbox });
