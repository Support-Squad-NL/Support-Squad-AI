...
import {
  ...
  DEFAULT_SUPPORTSQUADAI_BROWSER_COLOR,
  DEFAULT_SUPPORTSQUADAI_BROWSER_PROFILE_NAME,
...
const CDP_SOURCE_RANGE_ENV_KEY = "SUPPORTSQUADAI_BROWSER_CDP_SOURCE_RANGE";
...
function buildSandboxBrowserResolvedConfig(params: {
  ...
  return {
    ...
    color: DEFAULT_SUPPORTSQUADAI_BROWSER_COLOR,
    executablePath: undefined,
    headless: params.headless,
    noSandbox: false,
    attachOnly: true,
    defaultProfile: DEFAULT_SUPPORTSQUADAI_BROWSER_PROFILE_NAME,
    extraArgs: [],
    profiles: {
      [DEFAULT_SUPPORTSQUADAI_BROWSER_PROFILE_NAME]: {
...
export async function ensureSandboxBrowser(params: {
  ...
  if (hasContainer) {
    ...
    currentHash = await readDockerContainerLabel(containerName, "supportsquadai.configHash");
      ...
      if (isHot) {
        const hint = (() => {
          if (params.cfg.scope === "session") {
            return `supportsquadai sandbox recreate --browser --session ${params.scopeKey}`;
          }
          if (params.cfg.scope === "agent") {
            const agentId = resolveSandboxAgentId(params.scopeKey) ?? "main";
            return `supportsquadai sandbox recreate --browser --agent ${agentId}`;
          }
          return "supportsquadai sandbox recreate --browser --all";
        })();
        defaultRuntime.log(
  ...
  if (!hasContainer) {
    ...
    const args = buildSandboxCreateArgs({
      ...
      labels: {
        "supportsquadai.sandboxBrowser": "1",
        "supportsquadai.browserConfigEpoch": SANDBOX_BROWSER_SECURITY_HASH_EPOCH,
      },
    ...
    args.push("-e", `SUPPORTSQUADAI_BROWSER_HEADLESS=${params.cfg.browser.headless ? "1" : "0"}`);
    args.push("-e", `SUPPORTSQUADAI_BROWSER_ENABLE_NOVNC=${params.cfg.browser.enableNoVnc ? "1" : "0"}`);
    args.push("-e", `SUPPORTSQUADAI_BROWSER_CDP_PORT=${params.cfg.browser.cdpPort}`);
    if (cdpSourceRange) {
      args.push("-e", `${CDP_SOURCE_RANGE_ENV_KEY}=${cdpSourceRange}`);
    }
    args.push("-e", `SUPPORTSQUADAI_BROWSER_VNC_PORT=${params.cfg.browser.vncPort}`);
    args.push("-e", `SUPPORTSQUADAI_BROWSER_NOVNC_PORT=${params.cfg.browser.noVncPort}`);
  ...
  const existingProfile = existing
    ? resolveProfile(existing.bridge.state.resolved, DEFAULT_SUPPORTSQUADAI_BROWSER_PROFILE_NAME)
    : null;
