...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "supportsquadai.sandbox=1");
  args.push("--label", `supportsquadai.sessionKey=${params.scopeKey}`);
  args.push("--label", `supportsquadai.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `supportsquadai.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "supportsquadai.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`supportsquadai sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`supportsquadai sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("supportsquadai sandbox recreate --all");
}
