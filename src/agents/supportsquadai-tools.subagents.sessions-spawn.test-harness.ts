...
type CreateSupportSquadAITools = (typeof import("./supportsquadai-tools.js"))["createSupportSquadAITools"];
export type CreateSupportSquadAIToolsOpts = Parameters<CreateSupportSquadAITools>[0];
...
export async function getSessionsSpawnTool(opts: CreateSupportSquadAIToolsOpts) {
  // Dynamic import: ensure harness mocks are installed before tool modules load.
  const { createSupportSquadAITools } = await import("./supportsquadai-tools.js");
  const tool = createSupportSquadAITools(opts).find((candidate) => candidate.name === "sessions_spawn");
  if (!tool) {
