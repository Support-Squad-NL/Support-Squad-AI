...
import { createSupportSquadAITools } from "./supportsquadai-tools.js";
import { __testing, createSupportSquadAICodingTools } from "./pi-tools.js";
import { createSupportSquadAIReadTool, createSandboxedReadTool } from "./pi-tools.read.js";
import { createHostSandboxFsBridge } from "./test-helpers/host-sandbox-fs-bridge.js";
import { createBrowserTool } from "./tools/browser-tool.js";

const defaultTools = createSupportSquadAICodingTools();
...
describe("createSupportSquadAICodingTools", () => {
  ...
  it("exposes raw for gateway config.apply tool calls", () => {
    const gateway = createSupportSquadAICodingTools({ senderIsOwner: true }).find(
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createSupportSquadAITools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createSupportSquadAICodingTools({ messageProvider: "discord" });
  ...
  it("filters session tools for sub-agent sessions by default", () => {
    const tools = createSupportSquadAICodingTools({
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-depth-policy-"));
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    ...
    const googleTools = createSupportSquadAICodingTools({
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "supportsquadai-outside.txt");
  ...
  it("auto-pages read output across chunks when context window budget allows", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-read-autopage-"));
  ...
  it("adds capped continuation guidance when aggregated read output reaches budget", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-read-cap-"));
  ...
  it("strips truncation.content details from read results while preserving other fields", async () => {
    ...
    const wrapped = createSupportSquadAIReadTool(
      baseRead as unknown as Parameters<typeof createSupportSquadAIReadTool>[0],
    );
