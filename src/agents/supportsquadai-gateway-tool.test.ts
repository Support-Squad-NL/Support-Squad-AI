...
function requireGatewayTool(agentSessionKey?: string) {
  const tool = createSupportSquadAITools({
...
describe("gateway tool", () => {
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-test-"));

    try {
      await withEnvAsync(
        { SUPPORTSQUADAI_STATE_DIR: stateDir, SUPPORTSQUADAI_PROFILE: "isolated" },
        async () => {
          const tool = requireGatewayTool();
          ...
          expect(parsed.payload?.doctorHint).toBe(
            "Run: supportsquadai --profile isolated doctor --non-interactive",
          );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/supportsquadai" } }\n}\n';
