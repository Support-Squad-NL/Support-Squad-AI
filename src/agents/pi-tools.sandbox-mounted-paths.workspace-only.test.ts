...
async function withUnsafeMountedSandboxHarness(
  ...
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-sbx-mounts-"));
...
describe("tools.fs.workspaceOnly", () => {
  it("defaults to allowing sandbox mounts outside the workspace root", async () => {
    await withUnsafeMountedSandboxHarness(async ({ sandboxRoot, agentRoot, sandbox }) => {
      ...
      const tools = createSupportSquadAICodingTools({ sandbox, workspaceDir: sandboxRoot });
  ...
  it("rejects sandbox mounts outside the workspace root when enabled", async () => {
    await withUnsafeMountedSandboxHarness(async ({ sandboxRoot, agentRoot, sandbox }) => {
      ...
      const cfg = { tools: { fs: { workspaceOnly: true } } } as unknown as SupportSquadAIConfig;
      const tools = createSupportSquadAICodingTools({ sandbox, workspaceDir: sandboxRoot, config: cfg });
