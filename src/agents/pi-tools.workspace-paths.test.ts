...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("supportsquadai-ws-", async (workspaceDir) => {
      await withTempDir("supportsquadai-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createSupportSquadAICodingTools({ workspaceDir });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("supportsquadai-ws-", async (workspaceDir) => {
      await withTempDir("supportsquadai-override-", async (overrideDir) => {
        const tools = createSupportSquadAICodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("supportsquadai-sandbox-", async (sandboxDir) => {
      await withTempDir("supportsquadai-workspace-", async (workspaceDir) => {
        ...
        const tools = createSupportSquadAICodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
