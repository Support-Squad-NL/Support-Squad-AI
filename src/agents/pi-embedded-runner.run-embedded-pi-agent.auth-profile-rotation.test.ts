...
vi.mock("./models-config.js", async (importOriginal) => {
  ...
  return {
    ...mod,
    ensureSupportSquadAIModelsJson: vi.fn(async () => ({ wrote: false })),
  };
});
...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): SupportSquadAIConfig =>
  ({
    agents: {
      defaults: {
        model: {
  ...
  }) satisfies SupportSquadAIConfig;
...
async function withTimedAgentWorkspace<T>(
  ...
  vi.useFakeTimers();
  try {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-workspace-"));
    const now = Date.now();
