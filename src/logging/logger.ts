...
import type { SupportSquadAIConfig } from "../config/types.js";
...
export const DEFAULT_LOG_DIR = resolvePreferredSupportSquadAITmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "supportsquadai.log"); // legacy single-file path

const LOG_PREFIX = "supportsquadai";
...
function resolveSettings(): ResolvedSettings {
  let cfg: SupportSquadAIConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig?.("../config/config.js") as
        | {
            loadConfig?: () => SupportSquadAIConfig;
          }
        | undefined;
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.SUPPORTSQUADAI_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (nextBytes > settings.maxFileBytes) {
        if (!warnedAboutSizeCap) {
          ...
          process.stderr.write(
            `[supportsquadai] log file size cap reached; suppressing writes file=${settings.file} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
        return;
      }
