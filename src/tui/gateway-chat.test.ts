...
describe("resolveGatewayConnection", () => {
  ...
  it("uses SUPPORTSQUADAI_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ SUPPORTSQUADAI_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers SUPPORTSQUADAI_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ SUPPORTSQUADAI_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
