...
<title>SupportSquadAI Canvas</title>
<style>
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.supportsquadaiCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.supportsquadaiCanvasA2UIAction &&
        typeof window.supportsquadaiCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.supportsquadaiSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("supportsquadai:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android SupportSquadAI node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.supportsquadaiSendUserAction === "function"
        ? window.supportsquadaiSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.SUPPORTSQUADAI_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.SUPPORTSQUADAI_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>SupportSquadAI Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
