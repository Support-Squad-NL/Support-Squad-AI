...
describe("canvas host state dir defaults", () => {
  it("uses SUPPORTSQUADAI_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("supportsquadai-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("SupportSquadAI Canvas");
      } finally {
        await handler.close();
      }
    });
