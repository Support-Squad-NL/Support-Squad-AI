...
function makeCfg(overrides?: Partial<SupportSquadAIConfig>): SupportSquadAIConfig {
  return {
    bindings: [],
    channels: {},
    ...overrides,
...
describe("resolveWhatsAppHeartbeatRecipients", () => {
  ...
  function resolveWith(
    cfgOverrides: Partial<SupportSquadAIConfig> = {},
  ...
  it("ignores session store when session scope is global", () => {
    ...
    const result = resolveWith({
      session: { scope: "global" } as SupportSquadAIConfig["session"],
