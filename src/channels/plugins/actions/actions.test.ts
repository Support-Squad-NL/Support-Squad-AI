...
function telegramCfg(): SupportSquadAIConfig {
  return { channels: { telegram: { botToken: "tok" } } } as SupportSquadAIConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: SupportSquadAIConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as SupportSquadAIConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as SupportSquadAIConfig;
...
function createSignalAccountOverrideCfg(): SupportSquadAIConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as SupportSquadAIConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as SupportSquadAIConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as SupportSquadAIConfig,
        accountId: undefined,
        params: {
