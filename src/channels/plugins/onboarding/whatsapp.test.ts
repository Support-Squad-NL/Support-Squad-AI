...
const resolveWhatsAppAuthDirMock = vi.hoisted(() =>
  vi.fn(() => ({
    authDir: "/tmp/supportsquadai-whatsapp-test",
  })),
);
...
describe("whatsappOnboardingAdapter.configure", () => {
  beforeEach(() => {
    ...
    resolveWhatsAppAuthDirMock.mockReturnValue({ authDir: "/tmp/supportsquadai-whatsapp-test" });
  });
  ...
  it("skips relink note when already linked and relink is declined", async () => {
    ...
    expect(harness.note).not.toHaveBeenCalledWith(
      expect.stringContaining("supportsquadai channels login"),
      "WhatsApp",
    );
  });
