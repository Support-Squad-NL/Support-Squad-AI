...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: SupportSquadAIConfig, policy: ChannelAccessPolicy) => SupportSquadAIConfig;
  resolveAllowlist: (params: { cfg: SupportSquadAIConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: SupportSquadAIConfig; resolved: TResolved }) => SupportSquadAIConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: SupportSquadAIConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: SupportSquadAIConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: SupportSquadAIConfig, policy: ChannelAccessPolicy): SupportSquadAIConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: SupportSquadAIConfig, policy: ChannelAccessPolicy): SupportSquadAIConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: SupportSquadAIConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: SupportSquadAIConfig; resolved: string[] }) => {
      calls.push("apply");
