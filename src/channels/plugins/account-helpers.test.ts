...
function cfg(accounts?: Record<string, unknown> | null): SupportSquadAIConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as SupportSquadAIConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as SupportSquadAIConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as SupportSquadAIConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as SupportSquadAIConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as SupportSquadAIConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as SupportSquadAIConfig)).toBe("default");
    });
  });
});
