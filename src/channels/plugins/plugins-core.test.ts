...
describe("channel plugin catalog", () => {
  it("includes Microsoft Teams", () => {
    ...
    expect(entry?.install.npmSpec).toBe("@supportsquadai/msteams");
  ...
  it("includes external catalog entries", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "supportsquadai-catalog-"));
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        entries: [
          {
            name: "@supportsquadai/demo-channel",
            supportsquadai: {
              channel: {
                id: "demo-channel",
                label: "Demo Channel",
              ...
              install: {
                npmSpec: "@supportsquadai/demo-channel",
              },
            },
          },
        ],
...
async function listDirectoryEntriesWithDefaults(listFn: DirectoryListFn, cfg: SupportSquadAIConfig) {
  return await listFn({
    cfg,
    accountId: "default",
    query: null,
