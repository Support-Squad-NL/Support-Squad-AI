import type { BrowserConfig, BrowserProfileConfig, SupportSquadAIConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_SUPPORTSQUADAI_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "supportsquadai" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_SUPPORTSQUADAI_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "supportsquadai" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_SUPPORTSQUADAI_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_SUPPORTSQUADAI_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an SupportSquadAI browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for supportsquadai/supportsquadai CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: SupportSquadAIConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_SUPPORTSQUADAI_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "supportsquadai";
