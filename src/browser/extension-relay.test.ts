...
describe("chrome extension relay server", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["SUPPORTSQUADAI_GATEWAY_TOKEN"]);
  ...
  it("uses relay-scoped token only for known relay ports", async () => {
    ...
    expect(Object.keys(headers)).toContain("x-supportsquadai-relay-token");
    expect(headers["x-supportsquadai-relay-token"]).not.toBe(TEST_GATEWAY_TOKEN);
  });
  ...
  it("accepts extension websocket access with relay token query param", async () => {
    ...
    const token = relayAuthHeaders(`ws://127.0.0.1:${port}/extension`)["x-supportsquadai-relay-token"];
  ...
  it("does not swallow EADDRINUSE when occupied port is not an supportsquadai relay", async () => {
    const port = await getFreePort();
