...
describe("cdp.helpers", () => {
  ...
  it("does not add relay header for unknown loopback ports", () => {
    ...
    expect(headers["x-supportsquadai-relay-token"]).toBeUndefined();
  });
  ...
  it("adds relay header for known relay ports", async () => {
    ...
    process.env.SUPPORTSQUADAI_GATEWAY_TOKEN = "test-gateway-token";
    try {
      await ensureChromeExtensionRelayServer({ cdpUrl });
      const headers = getHeadersWithAuth(`${cdpUrl}/json/version`);
      expect(headers["x-supportsquadai-relay-token"]).toBeTruthy();
      expect(headers["x-supportsquadai-relay-token"]).not.toBe("test-gateway-token");
    } finally {
      await stopChromeExtensionRelayServer({ cdpUrl }).catch(() => {});
      if (prev === undefined) {
        delete process.env.SUPPORTSQUADAI_GATEWAY_TOKEN;
      } else {
        process.env.SUPPORTSQUADAI_GATEWAY_TOKEN = prev;
      }
    }
  });
});
...
describe("browser server-context listKnownProfileNames", () => {
  it("includes configured and runtime-only profile names", () => {
    ...
    if (!supportsquadai) {
      throw new Error("expected supportsquadai profile");
    }
    ...
    const state: BrowserServerState = {
      ...
      profiles: new Map([
        [
          "stale-removed",
          {
            profile: { ...supportsquadai, name: "stale-removed" },
            running: null,
          },
        ],
      ]),
