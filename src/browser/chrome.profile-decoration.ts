...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".supportsquadai-profile-decorated");
}
 ...
 */
export function decorateSupportSquadAIProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_SUPPORTSQUADAI_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_SUPPORTSQUADAI_BROWSER_COLOR).toUpperCase();
