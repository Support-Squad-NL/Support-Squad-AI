...
const RELAY_TOKEN_CONTEXT = "supportsquadai-extension-relay-v1";
const DEFAULT_RELAY_PROBE_TIMEOUT_MS = 500;
const SUPPORTSQUADAI_RELAY_BROWSER = "SupportSquadAI/extension-relay";

function resolveGatewayAuthToken(): string | null {
  const envToken =
    process.env.SUPPORTSQUADAI_GATEWAY_TOKEN?.trim() || process.env.CLAWDBOT_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
...
export function resolveRelayAuthTokenForPort(port: number): string {
  ...
  throw new Error(
    "extension relay requires gateway auth token (set gateway.auth.token or SUPPORTSQUADAI_GATEWAY_TOKEN)",
  );
}

export async function probeAuthenticatedSupportSquadAIRelay(params: {
  baseUrl: string;
  ...
  try {
    ...
    return browserName === SUPPORTSQUADAI_RELAY_BROWSER;
  } catch {
    return false;
  } finally {
    clearTimeout(timer);
