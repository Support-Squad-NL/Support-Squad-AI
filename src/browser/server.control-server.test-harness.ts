...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.SUPPORTSQUADAI_GATEWAY_PORT = prevGatewayPort;
}
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/supportsquadai" }));
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        headless: true,
        defaultProfile: "supportsquadai",
        profiles: {
          supportsquadai: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchSupportSquadAIChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.prevGatewayPort = process.env.SUPPORTSQUADAI_GATEWAY_PORT;
  process.env.SUPPORTSQUADAI_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.SUPPORTSQUADAI_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.SUPPORTSQUADAI_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.SUPPORTSQUADAI_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.SUPPORTSQUADAI_GATEWAY_PASSWORD;
  } else {
    process.env.SUPPORTSQUADAI_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
