...
import {
  probeAuthenticatedSupportSquadAIRelay,
...
describe("extension-relay-auth", () => {
  ...
  beforeEach(() => {
    ...
    process.env.SUPPORTSQUADAI_GATEWAY_TOKEN = TEST_GATEWAY_TOKEN;
  });
  ...
  it("accepts authenticated supportsquadai relay probe responses", async () => {
    ...
    await withRelayServer(
      (req, res) => {
        ...
        const header = req.headers["x-supportsquadai-relay-token"];
        seenToken = Array.isArray(header) ? header[0] : header;
        res.writeHead(200, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ Browser: "SupportSquadAI/extension-relay" }));
      },
      async ({ port }) => {
        const token = resolveRelayAuthTokenForPort(port);
        const ok = await probeAuthenticatedSupportSquadAIRelay({
          baseUrl: `http://127.0.0.1:${port}`,
          relayAuthHeader: "x-supportsquadai-relay-token",
