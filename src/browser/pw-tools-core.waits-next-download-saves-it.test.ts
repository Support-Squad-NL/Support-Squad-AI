...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredSupportSquadAITmpDir: vi.fn(() => "/tmp/supportsquadai"),
}));
vi.mock("../infra/tmp-supportsquadai-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredSupportSquadAITmpDir.mockReturnValue("/tmp/supportsquadai");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredSupportSquadAITmpDir.mockReturnValue("/tmp/supportsquadai-preferred");
    ...
    const expectedRootedDownloadsDir = path.join(
      path.sep,
      "tmp",
      "supportsquadai-preferred",
      "downloads",
    );
    const expectedDownloadsTail = `${path.join("tmp", "supportsquadai-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredSupportSquadAITmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "supportsquadai-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "supportsquadai-preferred", "downloads")}${path.sep}`),
    );
  });
