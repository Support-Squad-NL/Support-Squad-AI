...
vi.mock("./chrome.js", () => ({
  resolveSupportSquadAIUserDataDir: vi.fn(() => "/tmp/supportsquadai-test/supportsquadai/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveSupportSquadAIUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "supportsquadai-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveSupportSquadAIUserDataDir).mockReturnValue(userDataDir);
