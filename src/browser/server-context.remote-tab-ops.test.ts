...
describe("browser server-context tab selection state", () => {
  it("updates lastTargetId when openTab is created via CDP", async () => {
    ...
    const state = makeState("supportsquadai");
    const ctx = createBrowserRouteContext({ getState: () => state });
    const supportsquadai = ctx.forProfile("supportsquadai");

    const opened = await supportsquadai.openTab("http://127.0.0.1:8080");
    expect(opened.targetId).toBe("CREATED");
    expect(state.profiles.get("supportsquadai")?.lastTargetId).toBe("CREATED");
  ...
  it("blocks unsupported non-network URLs before any HTTP tab-open fallback", async () => {
    ...
    await expect(supportsquadai.openTab("file:///etc/passwd")).rejects.toBeInstanceOf(
