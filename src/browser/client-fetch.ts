...
function withLoopbackBrowserAuthImpl(
  ...
  if (headers.has("authorization") || headers.has("x-supportsquadai-password")) {
    return { ...init, headers };
  }
  ...
  try {
    ...
    if (auth.password) {
      headers.set("x-supportsquadai-password", auth.password);
      return { ...init, headers };
    }
  } catch {
  ...
  try {
    ...
    if (bridgeAuth?.token) {
      ...
      headers.set("x-supportsquadai-password", bridgeAuth.password);
    }
  } catch {
    // ignore
  }
...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  ...
  const operatorHint = isLocal
    ? `Restart the SupportSquadAI gateway (SupportSquadAI.app menubar, or \`${formatCliCommand("supportsquadai gateway")}\`).`
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the SupportSquadAI browser control service (timed out after ${timeoutMs}ms). ${operatorHint} ${modelHint}`,
    );
  }
  return new Error(
    `Can't reach the SupportSquadAI browser control service. ${operatorHint} ${modelHint} (${msg})`,
  );
}
