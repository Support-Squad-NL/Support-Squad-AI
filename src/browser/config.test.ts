...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    const supportsquadai = resolveProfile(resolved, "supportsquadai");
    expect(supportsquadai?.driver).toBe("supportsquadai");
    expect(supportsquadai?.cdpPort).toBe(18800);
    expect(supportsquadai?.cdpUrl).toBe("http://127.0.0.1:18800");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });

  it("derives default ports from SUPPORTSQUADAI_GATEWAY_PORT when unset", () => {
    withEnv({ SUPPORTSQUADAI_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const supportsquadai = resolveProfile(resolved, "supportsquadai");
      expect(supportsquadai?.cdpPort).toBe(19012);
      expect(supportsquadai?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ SUPPORTSQUADAI_GATEWAY_PORT: undefined }, () => {
      ...
      const supportsquadai = resolveProfile(resolved, "supportsquadai");
      expect(supportsquadai?.cdpPort).toBe(19022);
      expect(supportsquadai?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "supportsquadai");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        supportsquadai: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("supportsquadai");
  });
