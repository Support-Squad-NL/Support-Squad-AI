  ...
  return {
    agents: {
      defaults: {
        ...
        workspace: "/tmp/supportsquadai",
      },
    },
    session: { store: "/tmp/supportsquadai-sessions.json" },
...
describe("discord tool result dispatch", () => {
  it(
    ...
    async () => {
      const cfg = createMentionRequiredGuildConfig({
        messages: {
          responsePrefix: "PFX",
          groupChat: { mentionPatterns: ["\\bsupportsquadai\\b"] },
        },
      });
      ...
      await handler(
        createGuildMessageEvent({ messageId: "m2", content: "supportsquadai: hello" }),
        client,
      );
      ...
      createGuildMessageEvent({
        ...
        messagePatch: {
          referencedMessage: {
            ...
            mentionedUsers: [],
            mentionedRoles: [],
            author: { id: "bot-id", bot: true, username: "SupportSquadAI" },
          },
        },
        eventPatch: {
  ...
  it("treats forum threads as distinct sessions without channel payloads", async () => {
    ...
    const cfg = {
      agent: { model: "anthropic/claude-opus-4-5", workspace: "/tmp/supportsquadai" },
      session: { store: "/tmp/supportsquadai-sessions.json" },
      channels: {
        discord: {
          dm: { enabled: true, policy: "open" },
