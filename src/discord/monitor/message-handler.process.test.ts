...
const resolveStorePath = vi.fn(() => "/tmp/supportsquadai-discord-process-test-sessions.json");
...
beforeEach(() => {
  ...
  resolveStorePath.mockReturnValue("/tmp/supportsquadai-discord-process-test-sessions.json");
  ...
  async function createBlockModeContext() {
    return await createBaseContext({
      cfg: {
        ...
        session: { store: "/tmp/supportsquadai-discord-process-test-sessions.json" },
        channels: {
          discord: {
