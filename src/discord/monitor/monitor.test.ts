...
describe("agent components", () => {
  const createCfg = (): SupportSquadAIConfig => ({}) as SupportSquadAIConfig;
...
describe("discord component interactions", () => {
  const createCfg = (): SupportSquadAIConfig =>
    ({
      channels: {
        discord: {
          replyToMode: "first",
  ...
  beforeEach(() => {
    ...
    resolveStorePathMock.mockClear().mockReturnValue("/tmp/supportsquadai-sessions-test.json");
  });
...
describe("resolveDiscordPresenceUpdate", () => {
  ...
  it("includes streaming url when activityType is streaming", () => {
    ...
    expect(presence?.activities[0]).toMatchObject({
      type: 1,
      name: "Live",
      url: "https://twitch.tv/supportsquadai",
    });
  });
});
