...
describe("discord allowlist helpers", () => {
  it("normalizes slugs", () => {
    expect(normalizeDiscordSlug("Friends of SupportSquadAI")).toBe("friends-of-supportsquadai");
  ...
  it("matches ids or names", () => {
    const allow = normalizeDiscordAllowList(
      ["123", "steipete", "Friends of SupportSquadAI"],
    ...
    expect(allowListMatches(allow, { name: "friends-of-supportsquadai" })).toBe(true);
...
describe("discord guild/channel resolution", () => {
  it("resolves guild entry by id", () => {
    const guildEntries = makeEntries({
      "123": { slug: "friends-of-supportsquadai" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of SupportSquadAI"),
      guildEntries,
    });
    expect(resolved?.id).toBe("123");
    expect(resolved?.slug).toBe("friends-of-supportsquadai");
  });

  it("resolves guild entry by slug key", () => {
    const guildEntries = makeEntries({
      "friends-of-supportsquadai": { slug: "friends-of-supportsquadai" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of SupportSquadAI"),
      guildEntries,
...
describe("discord group DM gating", () => {
  ...
  it("matches group DM allowlist", () => {
    expect(
      resolveGroupDmAllow({
        channels: ["supportsquadai-dm"],
        channelId: "1",
        channelName: "SupportSquadAI DM",
        channelSlug: "supportsquadai-dm",
      }),
    ).toBe(true);
    expect(
      resolveGroupDmAllow({
