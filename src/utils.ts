...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".supportsquadai");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.SUPPORTSQUADAI_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$SUPPORTSQUADAI_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via SUPPORTSQUADAI_STATE_DIR.
