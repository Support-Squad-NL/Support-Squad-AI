...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "supportsquadai.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "supportsquadai.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.SUPPORTSQUADAI_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.SUPPORTSQUADAI_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.SUPPORTSQUADAI_NODE_OPTIONS_READY = "1";
    ...
    child.once("error", (error) => {
      console.error(
        "[supportsquadai] Failed to respawn CLI:",
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[supportsquadai] ${parsed.error}`);
      process.exit(2);
    }
    ...
    import("./cli/run-main.js")
      ...
      .catch((error) => {
        console.error(
          "[supportsquadai] Failed to start CLI:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
