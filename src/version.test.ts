...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-version-"));
  try {
...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "supportsquadai", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "supportsquadai", version: "2.3.4" });
  ...
  it("ignores non-supportsquadai package and blank build-info versions", async () => {
    await withTempDir(async (root) => {
  ...
  it("prefers SUPPORTSQUADAI_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        SUPPORTSQUADAI_VERSION: "9.9.9",
        SUPPORTSQUADAI_SERVICE_VERSION: "2.2.2",
  ...
  it("uses service and package fallbacks and ignores blank env values", () => {
    expect(
      resolveRuntimeServiceVersion({
        ...
        SUPPORTSQUADAI_SERVICE_VERSION: "  2.0.0  ",
