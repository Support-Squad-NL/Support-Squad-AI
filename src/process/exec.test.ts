...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with process.env", async () => {
    await withEnvAsync({ SUPPORTSQUADAI_BASE_ENV: "base" }, async () => {
      const result = await runCommandWithTimeout(
        [
          process.execPath,
          "-e",
          'process.stdout.write((process.env.SUPPORTSQUADAI_BASE_ENV ?? "") + "|" + (process.env.SUPPORTSQUADAI_TEST_ENV ?? ""))',
        ],
        {
          timeoutMs: 5_000,
          env: { SUPPORTSQUADAI_TEST_ENV: "ok" },
        },
      );
