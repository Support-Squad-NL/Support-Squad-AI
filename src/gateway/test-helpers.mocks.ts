...
type GetReplyFromConfigFn = (
  ...
  configOverride?: SupportSquadAIConfig,
...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `supportsquadai-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.SUPPORTSQUADAI_CONFIG_PATH = path.join(root, "supportsquadai.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "supportsquadai.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "supportsquadai-gateway-test"),
        ...fileDefaults,
...
vi.mock("../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadSupportSquadAIPlugins: () => pluginRegistryState.registry,
  };
});
