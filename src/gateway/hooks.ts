...
export function resolveHooksConfig(cfg: SupportSquadAIConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: SupportSquadAIConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-supportsquadai-token"] === "string"
      ? req.headers["x-supportsquadai-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
