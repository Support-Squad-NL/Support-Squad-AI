...
describe("handleControlUiHttpRequest", () => {
  async function withControlUiRoot<T>(params: {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-ui-"));
    try {
  ...
  async function withBasePathRootFixture<T>(params: {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-ui-root-"));
    try {
  ...
  it("serves bootstrap config JSON under basePath", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        const { res, end } = makeMockHttpResponse();
        const handled = handleControlUiHttpRequest(
          { url: `/supportsquadai${CONTROL_UI_BOOTSTRAP_CONFIG_PATH}`, method: "GET" } as IncomingMessage,
          res,
          {
        ...
        expect(handled).toBe(true);
        const parsed = parseBootstrapPayload(end);
        expect(parsed.basePath).toBe("/supportsquadai");
        expect(parsed.assistantName).toBe("Ops");
        expect(parsed.assistantAvatar).toBe("/supportsquadai/avatar/main");
        expect(parsed.assistantAgentId).toBe("main");
      },
    });
  });
  ...
  it("serves local avatar bytes through hardened avatar handler", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-avatar-http-"));
    try {
  ...
  it("rejects avatar symlink paths from resolver", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-avatar-http-link-"));
    const outside = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-avatar-http-outside-"));
    try {
  ...
  it("rejects symlinked assets that resolve outside control-ui root", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        ...
        const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-ui-outside-"));
        try {
  ...
  it("rejects symlinked SPA fallback index.html outside control-ui root", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-ui-index-outside-"));
        try {
  ...
  it("rejects absolute-path escape attempts under basePath routes", async () => {
    await withBasePathRootFixture({
      ...
      fn: async ({ root, sibling }) => {
        ...
        const { res, end, handled } = runControlUiRequest({
          url: `/supportsquadai/${absolutePathUrl}`,
          method: "GET",
          rootPath: root,
          basePath: "/supportsquadai",
        });
  ...
  it("rejects symlink escape attempts under basePath routes", async () => {
    await withBasePathRootFixture({
      ...
      fn: async ({ root, sibling }) => {
        ...
        const { res, end, handled } = runControlUiRequest({
          url: "/supportsquadai/assets/leak.txt",
          method: "GET",
          rootPath: root,
          basePath: "/supportsquadai",
        });
