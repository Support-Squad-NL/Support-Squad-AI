...
describe("buildGatewayConnectionDetails", () => {
  ...
  it("throws for insecure ws:// remote URLs (CWE-319)", () => {
    ...
    expect((thrown as Error).message).toContain("supportsquadai doctor --fix");
  });
...
describe("callGateway password resolution", () => {
  ...
  const explicitAuthCases = [
    {
      label: "password",
      authKey: "password",
      envKey: "SUPPORTSQUADAI_GATEWAY_PASSWORD",
      envValue: "from-env",
    ...
    },
    {
      label: "token",
      authKey: "token",
      envKey: "SUPPORTSQUADAI_GATEWAY_TOKEN",
      envValue: "env-token",
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["SUPPORTSQUADAI_GATEWAY_PASSWORD", "SUPPORTSQUADAI_GATEWAY_TOKEN"]);
    resetGatewayCallMocks();
  ...
  it.each([
    ...
    if (envPassword !== undefined) {
      process.env.SUPPORTSQUADAI_GATEWAY_PASSWORD = envPassword;
    }
