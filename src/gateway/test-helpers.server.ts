...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "SUPPORTSQUADAI_SKIP_BROWSER_CONTROL_SERVER",
  "SUPPORTSQUADAI_SKIP_GMAIL_WATCHER",
  "SUPPORTSQUADAI_SKIP_CANVAS_HOST",
  "SUPPORTSQUADAI_BUNDLED_PLUGINS_DIR",
  "SUPPORTSQUADAI_SKIP_CHANNELS",
  "SUPPORTSQUADAI_SKIP_PROVIDERS",
  "SUPPORTSQUADAI_SKIP_CRON",
  "SUPPORTSQUADAI_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.SUPPORTSQUADAI_STATE_DIR = path.join(tempHome, ".supportsquadai");
  delete process.env.SUPPORTSQUADAI_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.SUPPORTSQUADAI_SKIP_BROWSER_CONTROL_SERVER = "1";
  ...
  process.env.SUPPORTSQUADAI_SKIP_PROVIDERS = "1";
  process.env.SUPPORTSQUADAI_SKIP_CRON = "1";
  process.env.SUPPORTSQUADAI_TEST_MINIMAL_GATEWAY = "1";
  process.env.SUPPORTSQUADAI_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "supportsquadai-test-no-bundled-extensions")
    : "supportsquadai-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "supportsquadai-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".supportsquadai-test");
...
const CONNECT_CHALLENGE_NONCE_KEY = "__supportsquadaiTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__supportsquadaiTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["SUPPORTSQUADAI_GATEWAY_TOKEN"]);
  const prev = process.env.SUPPORTSQUADAI_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.SUPPORTSQUADAI_GATEWAY_TOKEN = fallbackToken;
  }
