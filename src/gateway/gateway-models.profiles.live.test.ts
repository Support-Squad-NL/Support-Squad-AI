...
import { resolveSupportSquadAIAgentDir } from "../agents/agent-paths.js";
...
import { ensureSupportSquadAIModelsJson } from "../agents/models-config.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { loadConfig } from "../config/config.js";
import type { ModelsConfig, SupportSquadAIConfig, ModelProviderConfig } from "../config/types.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.SUPPORTSQUADAI_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.SUPPORTSQUADAI_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.SUPPORTSQUADAI_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  cfg: SupportSquadAIConfig;
  agentDir: string;
}): SupportSquadAIConfig["auth"] | undefined {
  ...
  let profiles: NonNullable<SupportSquadAIConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.SUPPORTSQUADAI_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.SUPPORTSQUADAI_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveSupportSquadAIAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-live-state-"));
  process.env.SUPPORTSQUADAI_STATE_DIR = tempStateDir;
  ...
  process.env.SUPPORTSQUADAI_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.supportsquadai-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveSupportSquadAIAgentDir();
  const sanitizedCfg: SupportSquadAIConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-live-"));
  ...
  await ensureSupportSquadAIModelsJson(nextCfg);
  ...
  try {
          ...
          for (
            ...
            const toolProbe = await client.request<AgentFinalPayload>(
              "agent",
              {
                ...
                message: strictReply
                  ? "SupportSquadAI live tool probe (local, safe): " +
    ...
    process.env.SUPPORTSQUADAI_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.SUPPORTSQUADAI_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureSupportSquadAIModelsJson(cfg);
      ...
      const rawModels = process.env.SUPPORTSQUADAI_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.supportsquadai-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
