...
import type { SupportSquadAIConfig, ConfigFileSnapshot, GatewayReloadMode } from "../config/config.js";
...
export function resolveGatewayReloadSettings(cfg: SupportSquadAIConfig): GatewayReloadSettings {
...
export function startGatewayConfigReloader(opts: {
  initialConfig: SupportSquadAIConfig;
  readSnapshot: () => Promise<ConfigFileSnapshot>;
  onHotReload: (plan: GatewayReloadPlan, nextConfig: SupportSquadAIConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: SupportSquadAIConfig) => void;
  log: {
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: SupportSquadAIConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (nextConfig: SupportSquadAIConfig) => {
