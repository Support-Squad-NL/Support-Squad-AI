...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (/supportsquadai/i.test(trimmed)) {
    return trimmed;
  }
  return `${trimmed} (SupportSquadAI)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.SUPPORTSQUADAI_CLI_PATH?.trim();
  if (envPath) {
    return envPath;
  }
  ...
  const binCli = path.join(cwd, "bin", "supportsquadai");
  if (isFile(binCli)) {
    return binCli;
  }
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.SUPPORTSQUADAI_TAILNET_DNS?.trim();
