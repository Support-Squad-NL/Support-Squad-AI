...
declare const __SUPPORTSQUADAI_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "supportsquadai";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["SUPPORTSQUADAI_VERSION"],
      env["SUPPORTSQUADAI_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current SupportSquadAI version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION =
  (typeof __SUPPORTSQUADAI_VERSION__ === "string" && __SUPPORTSQUADAI_VERSION__) ||
  process.env.SUPPORTSQUADAI_BUNDLED_VERSION ||
