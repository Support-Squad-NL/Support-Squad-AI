...
import { resolvePreferredSupportSquadAITmpDir } from "../infra/tmp-supportsquadai-dir.js";
...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredSupportSquadAITmpDir(), "supportsquadai-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub SUPPORTSQUADAI_STATE_DIR.
    // Also keep it outside the SupportSquadAI temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["SUPPORTSQUADAI_STATE_DIR"]);
    process.env.SUPPORTSQUADAI_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "supportsquadai-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredSupportSquadAITmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("allows default SupportSquadAI state workspace and sandbox roots", async () => {
  ...
  it("rejects default SupportSquadAI state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
