...
describe("createTempHomeEnv", () => {
  it("sets home env vars and restores them on cleanup", async () => {
    ...
    const tempHome = await createTempHomeEnv("supportsquadai-temp-home-");
    expect(process.env.HOME).toBe(tempHome.home);
    expect(process.env.USERPROFILE).toBe(tempHome.home);
    expect(process.env.SUPPORTSQUADAI_STATE_DIR).toBe(path.join(tempHome.home, ".supportsquadai"));
    ...
    expect(process.env.SUPPORTSQUADAI_STATE_DIR).toBe(previousStateDir);
