...
export function isCommandEnabled(cfg: SupportSquadAIConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: SupportSquadAIConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as SupportSquadAIConfig),
...
export function getCommandDetection(_cfg?: SupportSquadAIConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: SupportSquadAIConfig) {
