...
describe("trigger handling", () => {
  ...
  it("rejects /restart when explicitly disabled", async () => {
    await withTempHome(async (home) => {
      const runEmbeddedPiAgentMock = getRunEmbeddedPiAgentMock();
      const cfg = { ...makeCfg(home), commands: { restart: false } } as SupportSquadAIConfig;
      const res = await getReplyFromConfig(
        {
          Body: "/restart",
          From: "+1001",
  ...
  it("reports status without invoking the agent", async () => {
    await withTempHome(async (home) => {
      ...
      const text = Array.isArray(res) ? res[0]?.text : res?.text;
      expect(text).toContain("SupportSquadAI");
      expect(runEmbeddedPiAgentMock).not.toHaveBeenCalled();
    });
  });
  ...
  it("reports active auth profile and key snippet in status", async () => {
    await withTempHome(async (home) => {
      const runEmbeddedPiAgentMock = getRunEmbeddedPiAgentMock();
      const cfg = makeCfg(home);
      const agentDir = join(home, ".supportsquadai", "agents", "main", "agent");
