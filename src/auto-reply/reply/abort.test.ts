...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop")).toBe(true);
    expect(isAbortRequestText("/stop@supportsquadai_bot", { botUsername: "supportsquadai_bot" })).toBe(true);
