...
beforeAll(async () => {
  suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-session-suite-"));
});
...
describe("initSessionState thread forking", () => {
  it("forks a new session from the parent session file", async () => {
    ...
    const root = await makeCaseDir("supportsquadai-thread-session-");
  ...
  it("forks from parent when thread session key already exists but was not forked yet", async () => {
    const warn = vi.spyOn(console, "warn").mockImplementation(() => {});
    const root = await makeCaseDir("supportsquadai-thread-session-existing-");
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir);
  ...
  it("records topic-specific session files when MessageThreadId is present", async () => {
    const root = await makeCaseDir("supportsquadai-topic-session-");
    const storePath = path.join(root, "sessions.json");
...
describe("initSessionState RawBody", () => {
  it("triggerBodyNormalized correctly extracts commands when Body contains context but RawBody is clean", async () => {
    const root = await makeCaseDir("supportsquadai-rawbody-");
    const storePath = path.join(root, "sessions.json");
    const cfg = { session: { store: storePath } } as SupportSquadAIConfig;
  ...
  it("Reset triggers (/new, /reset) work with RawBody", async () => {
    const root = await makeCaseDir("supportsquadai-rawbody-reset-");
    const storePath = path.join(root, "sessions.json");
    const cfg = { session: { store: storePath } } as SupportSquadAIConfig;
  ...
  it("preserves argument casing while still matching reset triggers case-insensitively", async () => {
    const root = await makeCaseDir("supportsquadai-rawbody-reset-case-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("falls back to Body when RawBody is undefined", async () => {
    const root = await makeCaseDir("supportsquadai-rawbody-fallback-");
    const storePath = path.join(root, "sessions.json");
    const cfg = { session: { store: storePath } } as SupportSquadAIConfig;
  ...
  it("uses the default per-agent sessions store when config store is unset", async () => {
    const root = await makeCaseDir("supportsquadai-session-store-default-");
    const stateDir = path.join(root, ".supportsquadai");
    ...
    vi.stubEnv("SUPPORTSQUADAI_STATE_DIR", stateDir);
    try {
      ...
      const cfg = {} as SupportSquadAIConfig;
      const result = await initSessionState({
        ctx: {
          Body: "hello",
          ChatType: "direct",
...
describe("initSessionState reset policy", () => {
  ...
  it("defaults to daily reset at 4am local time", async () => {
    ...
    const root = await makeCaseDir("supportsquadai-reset-daily-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("treats sessions as stale before the daily reset when updated before yesterday's boundary", async () => {
    ...
    const root = await makeCaseDir("supportsquadai-reset-daily-edge-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("expires sessions when idle timeout wins over daily reset", async () => {
    ...
    const root = await makeCaseDir("supportsquadai-reset-idle-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("uses per-type overrides for thread sessions", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 0, 0));
    const root = await makeCaseDir("supportsquadai-reset-thread-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("detects thread sessions without thread key suffix", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 0, 0));
    const root = await makeCaseDir("supportsquadai-reset-thread-nosuffix-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("defaults to daily resets when only resetByType is configured", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 0, 0));
    const root = await makeCaseDir("supportsquadai-reset-type-default-");
    const storePath = path.join(root, "sessions.json");
  ...
  it("keeps legacy idleMinutes behavior without reset config", async () => {
    vi.setSystemTime(new Date(2026, 0, 18, 5, 0, 0));
    const root = await makeCaseDir("supportsquadai-reset-legacy-");
    const storePath = path.join(root, "sessions.json");
...
describe("initSessionState channel reset overrides", () => {
  it("uses channel-specific reset policy when configured", async () => {
    const root = await makeCaseDir("supportsquadai-channel-idle-");
    const storePath = path.join(root, "sessions.json");
...
describe("initSessionState reset triggers in WhatsApp groups", () => {
  ...
  function makeCfg(params: { storePath: string; allowFrom: string[] }): SupportSquadAIConfig {
    return {
    ...
    const cases = [
      {
        ...
        storePrefix: "supportsquadai-group-reset-",
      ...
      },
      {
        ...
        storePrefix: "supportsquadai-group-reset-unauth-",
        allowFrom: ["+41796666864"],
      ...
      },
      {
        ...
        storePrefix: "supportsquadai-group-rawbody-",
        allowFrom: ["*"],
      ...
      },
      {
        ...
        storePrefix: "supportsquadai-group-reset-lid-",
        allowFrom: ["+41796666864"],
      ...
      },
      {
        ...
        storePrefix: "supportsquadai-group-reset-lid-unauth-",
        allowFrom: ["+41796666864"],
...
describe("initSessionState reset triggers in Slack channels", () => {
  ...
  it("supports mention-prefixed Slack reset commands and preserves args", async () => {
    ...
    const cases = [
      {
        name: "reset command",
        storePrefix: "supportsquadai-slack-channel-reset-",
      ...
      },
      {
        ...
        storePrefix: "supportsquadai-slack-channel-new-",
...
describe("initSessionState preserves behavior overrides across /new and /reset", () => {
  ...
  it("/new preserves verboseLevel from previous session", async () => {
    const storePath = await createStorePath("supportsquadai-reset-verbose-");
  ...
  it("/reset preserves thinkingLevel and reasoningLevel from previous session", async () => {
    const storePath = await createStorePath("supportsquadai-reset-thinking-");
  ...
  it("/new preserves session label from previous session", async () => {
    const storePath = await createStorePath("supportsquadai-reset-label-");
  ...
  it("/new in a new session does not preserve overrides", async () => {
    const storePath = await createStorePath("supportsquadai-new-no-preserve-");
  ...
  it("archives the old session store entry on /new", async () => {
    const storePath = await createStorePath("supportsquadai-archive-old-");
  ...
  it("idle-based new session does NOT preserve overrides (no entry to read)", async () => {
    const storePath = await createStorePath("supportsquadai-idle-no-preserve-");
...
describe("persistSessionUsageUpdate", () => {
  ...
  it("uses lastCallUsage for totalTokens when provided", async () => {
    const storePath = await createStorePath("supportsquadai-usage-");
