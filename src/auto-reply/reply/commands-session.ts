...
import { scheduleGatewaySigusr1Restart, triggerSupportSquadAIRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting SupportSquadAI in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerSupportSquadAIRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting SupportSquadAI via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
