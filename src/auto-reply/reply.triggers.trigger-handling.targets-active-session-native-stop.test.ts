...
beforeAll(async () => {
  previousFastTestEnv = process.env.SUPPORTSQUADAI_TEST_FAST;
...
afterAll(() => {
  ...
  process.env.SUPPORTSQUADAI_TEST_FAST = previousFastTestEnv;
});
...
describe("trigger handling", () => {
  ...
  it("uses the target agent model for native /status", async () => {
    await withTempHome(async (home) => {
      const cfg = makeCfg(home) as unknown as SupportSquadAIConfig;
      cfg.agents = {
        ...cfg.agents,
