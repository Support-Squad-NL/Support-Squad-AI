...
function createReplyConfig(home: string, streamMode?: "block"): SupportSquadAIConfig {
  return {
    agents: {
      defaults: {
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHarness("supportsquadai-stream-", async (home) => {
...
describe("block streaming", () => {
  beforeEach(() => {
    vi.stubEnv("SUPPORTSQUADAI_TEST_FAST", "1");
