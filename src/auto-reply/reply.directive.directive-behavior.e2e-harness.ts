...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(
    ...
    {
      env: {
        SUPPORTSQUADAI_AGENT_DIR: (home) => path.join(home, ".supportsquadai", "agent"),
        PI_CODING_AGENT_DIR: (home) => path.join(home, ".supportsquadai", "agent"),
      },
      prefix: "supportsquadai-reply-",
    },
  );
}
