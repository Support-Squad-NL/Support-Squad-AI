...
function summarizeGroupPolicy(cfg: SupportSquadAIConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: SupportSquadAIConfig): boolean {
...
function collectModels(cfg: SupportSquadAIConfig): ModelRef[] {
  const out: ModelRef[] = [];
  addModel(
    out,
...
function listKnownNodeCommands(cfg: SupportSquadAIConfig): Set<string> {
  const baseCfg: SupportSquadAIConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: SupportSquadAIConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: SupportSquadAIConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: SupportSquadAIConfig): boolean {
...
function isBrowserEnabled(cfg: SupportSquadAIConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: SupportSquadAIConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: SupportSquadAIConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep SUPPORTSQUADAI_STATE_DIR on a local-only volume and re-run "${formatCliCommand("supportsquadai security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: SupportSquadAIConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer SUPPORTSQUADAI_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const supportsquadaiGatewayToken =
    typeof env.SUPPORTSQUADAI_GATEWAY_TOKEN === "string" && env.SUPPORTSQUADAI_GATEWAY_TOKEN.trim()
      ? env.SUPPORTSQUADAI_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-supportsquadai-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: SupportSquadAIConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: SupportSquadAIConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
  ...
  if (browserExposurePaths.length > 0) {
    findings.push({
      ...
      remediation:
        "Set sandbox.browser.network to a dedicated bridge network (recommended default: supportsquadai-sandbox-browser), " +
...
export function collectNodeDenyCommandPatternFindings(cfg: SupportSquadAIConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: SupportSquadAIConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: SupportSquadAIConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: SupportSquadAIConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
