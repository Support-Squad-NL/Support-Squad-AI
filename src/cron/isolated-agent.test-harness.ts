...
export async function withTempCronHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "supportsquadai-cron-" });
}
...
export async function writeSessionStoreEntries(
  ...
  const dir = path.join(home, ".supportsquadai", "sessions");
...
export function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<SupportSquadAIConfig> = {},
): SupportSquadAIConfig {
  const base: SupportSquadAIConfig = {
    agents: {
      defaults: {
