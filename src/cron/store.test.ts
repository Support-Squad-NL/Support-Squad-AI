...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses SUPPORTSQUADAI_HOME for tilde expansion", () => {
    vi.stubEnv("SUPPORTSQUADAI_HOME", "/srv/supportsquadai-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/supportsquadai-home", "cron", "jobs.json"));
  });
});
