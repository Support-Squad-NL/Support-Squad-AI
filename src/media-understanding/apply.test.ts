...
const TEMP_MEDIA_PREFIX = "supportsquadai-media-";
...
function createGroqAudioConfig(): SupportSquadAIConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): SupportSquadAIConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): SupportSquadAIConfig {
  return {
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredSupportSquadAITmpDir();
  ...
  it("auto-detects sherpa for audio when binary and model files are available", async () => {
    ...
    const cfg: SupportSquadAIConfig = { tools: { media: { audio: {} } } };
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        SUPPORTSQUADAI_AGENT_DIR: isolatedAgentDir,
