...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.SUPPORTSQUADAI_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service SUPPORTSQUADAI_GATEWAY_TOKEN does not match gateway.auth.token in supportsquadai.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `supportsquadai gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
