...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.SUPPORTSQUADAI_GATEWAY_PORT).toBe("18789");
    expect(env.SUPPORTSQUADAI_GATEWAY_TOKEN).toBe("secret");
    expect(env.SUPPORTSQUADAI_SERVICE_MARKER).toBe("supportsquadai");
    expect(env.SUPPORTSQUADAI_SERVICE_KIND).toBe("gateway");
    expect(typeof env.SUPPORTSQUADAI_SERVICE_VERSION).toBe("string");
    expect(env.SUPPORTSQUADAI_SYSTEMD_UNIT).toBe("supportsquadai-gateway.service");
    if (process.platform === "darwin") {
      expect(env.SUPPORTSQUADAI_LAUNCHD_LABEL).toBe("ai.supportsquadai.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", SUPPORTSQUADAI_PROFILE: "work" },
      port: 18789,
    });
    expect(env.SUPPORTSQUADAI_SYSTEMD_UNIT).toBe("supportsquadai-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.SUPPORTSQUADAI_LAUNCHD_LABEL).toBe("ai.supportsquadai.work");
    }
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".supportsquadai"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", SUPPORTSQUADAI_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".supportsquadai-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", SUPPORTSQUADAI_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".supportsquadai"));
  });

  it("uses SUPPORTSQUADAI_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", SUPPORTSQUADAI_STATE_DIR: "/var/lib/supportsquadai" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/supportsquadai"));
  });

  it("expands ~ in SUPPORTSQUADAI_STATE_DIR", () => {
    const env = { HOME: "/Users/test", SUPPORTSQUADAI_STATE_DIR: "~/supportsquadai-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/supportsquadai-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { SUPPORTSQUADAI_STATE_DIR: "C:\\State\\supportsquadai" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\supportsquadai");
  });
});
