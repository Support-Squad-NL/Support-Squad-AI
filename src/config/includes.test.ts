...
const ETC_SUPPORTSQUADAI_DIR = path.join(ROOT_DIR, "etc", "supportsquadai");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "supportsquadai.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcSupportSquadAIPath(...parts: string[]) {
  return path.join(ETC_SUPPORTSQUADAI_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("rejects absolute path outside config directory (CWE-22)", () => {
    const absolute = etcSupportSquadAIPath("agents.json");
  ...
  it("rejects parent directory traversal escaping config directory (CWE-22)", () => {
    ...
    expectResolveIncludeError(
      () => resolve(obj, files, configPath("sub", "supportsquadai.json")),
...
describe("security: path traversal protection (CWE-22)", () => {
  ...
  describe("edge cases", () => {
    ...
    it("allows include files when the config root path is a symlink", async () => {
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-includes-symlink-"));
      try {
        ...
        const result = resolveConfigIncludes(
          ...
          path.join(linkRoot, "supportsquadai.json"),
        );
