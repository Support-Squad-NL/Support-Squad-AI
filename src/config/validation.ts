...
import type { SupportSquadAIConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: SupportSquadAIConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: SupportSquadAIConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = SupportSquadAISchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as SupportSquadAIConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as SupportSquadAIConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as SupportSquadAIConfig,
  };
}
