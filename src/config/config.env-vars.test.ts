...
describe("config env vars", () => {
  it("applies env vars from env block when missing", async () => {
    await withEnvOverride({ OPENROUTER_API_KEY: undefined }, async () => {
      applyConfigEnvVars({ env: { vars: { OPENROUTER_API_KEY: "config-key" } } } as SupportSquadAIConfig);
  ...
  it("applies env vars from env.vars when missing", async () => {
    await withEnvOverride({ GROQ_API_KEY: undefined }, async () => {
      applyConfigEnvVars({ env: { vars: { GROQ_API_KEY: "gsk-config" } } } as SupportSquadAIConfig);
  ...
  it("blocks dangerous startup env vars from config env", async () => {
    await withEnvOverride(
      ...
      async () => {
        ...
        const entries = collectConfigRuntimeEnvVars(config as SupportSquadAIConfig);
        ...
        applyConfigEnvVars(config as SupportSquadAIConfig);
  ...
  it("loads ${VAR} substitutions from ~/.supportsquadai/.env on repeated runtime loads", async () => {
    await withTempHome(async (_home) => {
      await withEnvOverride({ BRAVE_API_KEY: undefined }, async () => {
        const stateDir = process.env.SUPPORTSQUADAI_STATE_DIR?.trim();
        if (!stateDir) {
          throw new Error("Expected SUPPORTSQUADAI_STATE_DIR to be set by withTempHome");
        }
        await fs.mkdir(stateDir, { recursive: true });
        await fs.writeFile(path.join(stateDir, ".env"), "BRAVE_API_KEY=from-dotenv\n", "utf-8");

        const config: SupportSquadAIConfig = {
          tools: {
            web: {
              search: {
        ...
        const first = resolveConfigEnvVars(config, process.env) as SupportSquadAIConfig;
        expect(first.tools?.web?.search?.apiKey).toBe("from-dotenv");

        delete process.env.BRAVE_API_KEY;
        loadDotEnv({ quiet: true });
        const second = resolveConfigEnvVars(config, process.env) as SupportSquadAIConfig;
