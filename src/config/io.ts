...
import type { SupportSquadAIConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareSupportSquadAIVersions } from "./version.js";
...
const SHELL_ENV_EXPECTED_KEYS = [
  ...
  "SUPPORTSQUADAI_GATEWAY_PASSWORD",
];
...
function unsetPathForWrite(
  root: SupportSquadAIConfig,
  pathSegments: string[],
): { changed: boolean; next: SupportSquadAIConfig } {
...
function coerceConfig(value: unknown): SupportSquadAIConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as SupportSquadAIConfig;
}
...
function stampConfigVersion(cfg: SupportSquadAIConfig): SupportSquadAIConfig {
...
function warnIfConfigFromFuture(cfg: SupportSquadAIConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareSupportSquadAIVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer SupportSquadAI (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as SupportSquadAIConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): SupportSquadAIConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as SupportSquadAIConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: SupportSquadAIConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as SupportSquadAIConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.SUPPORTSQUADAI_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.SUPPORTSQUADAI_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.SUPPORTSQUADAI_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.SUPPORTSQUADAI_WATCH_SESSION === "string" &&
        deps.env.SUPPORTSQUADAI_WATCH_SESSION.trim().length > 0
          ? deps.env.SUPPORTSQUADAI_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.SUPPORTSQUADAI_WATCH_COMMAND === "string" &&
        deps.env.SUPPORTSQUADAI_WATCH_COMMAND.trim().length > 0
          ? deps.env.SUPPORTSQUADAI_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `SUPPORTSQUADAI_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.SUPPORTSQUADAI_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.SUPPORTSQUADAI_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): SupportSquadAIConfig {
