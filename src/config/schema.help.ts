...
export const FIELD_HELP: Record<string, string> = {
  meta: "Metadata fields automatically maintained by SupportSquadAI to record write/version history for this config file. Keep these values system-managed and avoid manual edits unless debugging migration history.",
  "meta.lastTouchedVersion": "Auto-set when SupportSquadAI writes the config.",
  ...
  "env.vars":
    "Explicit key/value environment variable overrides merged into runtime process environment for SupportSquadAI. Use this for deterministic env configuration instead of relying only on shell profile side effects.",
  wizard:
  ...
  "wizard.lastRunVersion":
    "SupportSquadAI version recorded at the time of the most recent wizard run on this config. Use this when diagnosing behavior differences across version-to-version onboarding changes.",
  ...
  update:
    "Update-channel and startup-check behavior for keeping SupportSquadAI runtime versions current. Use conservative channels in production and more experimental channels only in controlled environments.",
  ...
  "agents.defaults.sandbox.browser.network":
    "Docker network for sandbox browser containers (default: supportsquadai-sandbox-browser). Avoid bridge if you need stricter isolation.",
  ...
  "gateway.controlUi.basePath":
    "Optional URL prefix where the Control UI is served (e.g. /supportsquadai).",
  ...
  "gateway.nodes.allowCommands":
    "Extra node.invoke commands to allow beyond the gateway defaults (array of command strings). Enabling dangerous commands here is a security-sensitive override and is flagged by `supportsquadai security audit`.",
  ...
  "diagnostics.cacheTrace.filePath":
    "JSONL output path for cache trace logs (default: $SUPPORTSQUADAI_STATE_DIR/logs/cache-trace.jsonl).",
  ...
  "agents.defaults.memorySearch.store.path":
    "Sets where the SQLite memory index is stored on disk for each agent. Keep the default `~/.supportsquadai/memory/{agentId}.sqlite` unless you need custom storage placement or backup policy alignment.",
  ...
  "memory.backend":
    'Selects the global memory engine: "builtin" uses SupportSquadAI memory internals, while "qmd" uses the QMD sidecar pipeline. Keep "builtin" unless you intentionally operate QMD.',
  "memory.citations":
  ...
  "ui.brand":
    "Product-brand settings for visible Control UI chrome such as the title, subtitle, and external docs links. Use this for first-pass whitelabeling without changing lower-level runtime identifiers.",
  "ui.brand.name":
    "Primary product name shown in the Control UI header and browser title. Keep this customer-facing and separate from internal compatibility identifiers.",
  "ui.brand.subtitle":
    "Secondary header label shown beneath the product name in the Control UI. Use this for terms like dashboard, console, or admin center.",
  "ui.brand.docsUrl":
    "Absolute docs root opened by Control UI resource links. Point this at your branded documentation site when shipping a whitelabeled build.",
  "ui.brand.siteUrl":
    "Absolute website root used by visible onboarding and website/help links. Point this at your branded marketing or product home page.",
  "ui.assistant":
  ...
  "plugins.installs":
    "CLI-managed install metadata (used by `supportsquadai plugins update` to locate install sources).",
  "plugins.installs.*.source": 'Install source ("npm", "archive", or "path").',
  "plugins.installs.*.spec": "Original npm spec used for install (if source is npm).",
  "plugins.installs.*.sourcePath": "Original archive/path used for install (if any).",
  "plugins.installs.*.installPath":
    "Resolved install directory (usually ~/.supportsquadai/extensions/<id>).",
  ...
  hooks:
    "Inbound webhook automation surface for mapping external events into wake or agent actions in SupportSquadAI. Keep this locked down with explicit token/session/agent controls before exposing it beyond trusted networks.",
  "hooks.enabled":
  ...
  "channels.irc":
    "IRC channel provider configuration and compatibility settings for classic IRC transport workflows. Use this section when bridging legacy chat infrastructure into SupportSquadAI.",
  "channels.defaults":
