...
describe("multi-agent agentDir validation", () => {
  it("rejects shared agents.list agentDir", async () => {
    const shared = path.join(tmpdir(), "supportsquadai-shared-agentdir");
    ...
    await withTempHomeConfig(
      {
        agents: {
          list: [
            { id: "a", agentDir: "~/.supportsquadai/agents/shared/agent" },
            { id: "b", agentDir: "~/.supportsquadai/agents/shared/agent" },
          ],
        },
