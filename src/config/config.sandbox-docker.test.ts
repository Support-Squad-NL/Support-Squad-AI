...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("supportsquadai-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "supportsquadai-sandbox-browser-global" },
      agentBrowser: { network: "supportsquadai-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("supportsquadai-sandbox-browser-agent");
  });
