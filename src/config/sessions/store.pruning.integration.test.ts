...
// Keep integration tests deterministic: never read a real supportsquadai.json.
vi.mock("../config.js", () => ({
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-pruning-integ-"));
  });
  ...
  beforeEach(async () => {
    ...
    savedCacheTtl = process.env.SUPPORTSQUADAI_SESSION_CACHE_TTL_MS;
    process.env.SUPPORTSQUADAI_SESSION_CACHE_TTL_MS = "0";
  ...
  afterEach(() => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.SUPPORTSQUADAI_SESSION_CACHE_TTL_MS;
    } else {
      process.env.SUPPORTSQUADAI_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
