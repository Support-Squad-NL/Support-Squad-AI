...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "supportsquadai-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    SUPPORTSQUADAI_GATEWAY_TOKEN: "test-token",
    SUPPORTSQUADAI_CONFIG_DIR: join(sandbox.rootDir, "config"),
    SUPPORTSQUADAI_WORKSPACE_DIR: join(sandbox.rootDir, "supportsquadai"),
  };
...
describe("docker-setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      SUPPORTSQUADAI_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      SUPPORTSQUADAI_EXTRA_MOUNTS: undefined,
      SUPPORTSQUADAI_HOME_VOLUME: "supportsquadai-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("SUPPORTSQUADAI_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("SUPPORTSQUADAI_EXTRA_MOUNTS=");
    expect(envFile).toContain("SUPPORTSQUADAI_HOME_VOLUME=supportsquadai-home");
    const extraCompose = await readFile(
      join(activeSandbox.rootDir, "docker-compose.extra.yml"),
      "utf8",
    );
    expect(extraCompose).toContain("supportsquadai-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("supportsquadai-home:");
    const log = await readFile(activeSandbox.logPath, "utf8");
    expect(log).toContain("--build-arg SUPPORTSQUADAI_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });
  ...
  it("precreates config identity dir for CLI device auth writes", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      SUPPORTSQUADAI_CONFIG_DIR: configDir,
      SUPPORTSQUADAI_WORKSPACE_DIR: workspaceDir,
    });
  ...
  it("rejects injected multiline SUPPORTSQUADAI_EXTRA_MOUNTS values", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      SUPPORTSQUADAI_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("SUPPORTSQUADAI_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid SUPPORTSQUADAI_EXTRA_MOUNTS mount format", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      SUPPORTSQUADAI_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid SUPPORTSQUADAI_HOME_VOLUME names", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      SUPPORTSQUADAI_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("SUPPORTSQUADAI_HOME_VOLUME must match");
  });
