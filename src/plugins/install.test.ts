...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `supportsquadai-plugin-install-${randomUUID()}`);
    ...
    JSON.stringify(
      {
        ...
        supportsquadai: { extensions: params.extensions },
      },
      null,
      2,
    ),
...
async function createZipperArchiveBuffer(): Promise<Buffer> {
  ...
  zip.file(
    ...
    JSON.stringify({
      name: "@supportsquadai/zipper",
      version: "0.0.1",
      supportsquadai: { extensions: ["./dist/index.js"] },
    }),
  );
...
function setupInstallPluginFromDirFixture(params?: { devDependencies?: Record<string, string> }) {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: "@supportsquadai/test-plugin",
      version: "0.0.1",
      supportsquadai: { extensions: ["./dist/index.js"] },
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.supportsquadai/extensions and uses unscoped id", async () => {
  ...
  it("rejects packages without supportsquadai.extensions", async () => {
    const result = await installArchivePackageAndReturnResult({
      packageJson: { name: "@supportsquadai/nope", version: "0.0.1" },
      outName: "bad.tgz",
    });
    expect(result.ok).toBe(false);
    if (result.ok) {
    ...
    expect(result.error).toContain("supportsquadai.extensions");
  });
...
describe("installPluginFromDir", () => {
  ...
  it("strips workspace devDependencies before npm install", async () => {
    ...
    expect(manifest.devDependencies?.supportsquadai).toBeUndefined();
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
      ...
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        return {
          ...
          stdout: JSON.stringify([
            {
              id: "@supportsquadai/voice-call@0.0.1",
              name: "@supportsquadai/voice-call",
              version: "0.0.1",
              filename: packedName,
    ...
    const result = await installPluginFromNpmSpec({
      spec: "@supportsquadai/voice-call@0.0.1",
      extensionsDir,
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(true);
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@supportsquadai/voice-call@0.0.1");
    expect(result.npmResolution?.integrity).toBe("sha512-plugin-test");

    expectSingleNpmPackIgnoreScriptsCall({
      calls: run.mock.calls,
      expectedSpec: "@supportsquadai/voice-call@0.0.1",
    });
