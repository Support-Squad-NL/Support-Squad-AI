...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: SupportSquadAIPluginDefinition;
  register?: SupportSquadAIPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as SupportSquadAIPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as SupportSquadAIPluginDefinition;
...
export function loadSupportSquadAIPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverSupportSquadAIPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "supportsquadai/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "supportsquadai/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: SupportSquadAIPluginModule | null = null;
    try {
      mod = getJiti()(candidate.source) as SupportSquadAIPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
