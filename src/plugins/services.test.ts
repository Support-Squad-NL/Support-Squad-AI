...
import type { SupportSquadAIPluginService, SupportSquadAIPluginServiceContext } from "./types.js";
...
function createRegistry(services: SupportSquadAIPluginService[]) {
...
describe("startPluginServices", () => {
  ...
  it("starts services and stops them in reverse order", async () => {
    const starts: string[] = [];
    const stops: string[] = [];
    const contexts: SupportSquadAIPluginServiceContext[] = [];

    const serviceA: SupportSquadAIPluginService = {
      id: "service-a",
      start: (ctx) => {
        starts.push("a");
        contexts.push(ctx);
    ...
    const serviceB: SupportSquadAIPluginService = {
      id: "service-b",
      start: (ctx) => {
        starts.push("b");
        contexts.push(ctx);
    ...
    const serviceC: SupportSquadAIPluginService = {
      id: "service-c",
      start: (ctx) => {
        starts.push("c");
        contexts.push(ctx);
