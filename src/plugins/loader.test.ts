...
import { loadSupportSquadAIPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `supportsquadai-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.SUPPORTSQUADAI_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "supportsquadai.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.SUPPORTSQUADAI_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadSupportSquadAIPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.SUPPORTSQUADAI_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.SUPPORTSQUADAI_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadSupportSquadAIPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadSupportSquadAIPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadSupportSquadAIPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("preserves package.json metadata for bundled memory plugins", () => {
    const registry = loadBundledMemoryPluginRegistry({
      packageMeta: {
        name: "@supportsquadai/memory-core",
        version: "1.2.3",
  ...
  it("loads plugins from config paths", () => {
    process.env.SUPPORTSQUADAI_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
  ...
  it("warns when loaded non-bundled plugin has no install/load-path provenance", () => {
    ...
    withEnv({ SUPPORTSQUADAI_STATE_DIR: stateDir, CLAWDBOT_STATE_DIR: undefined }, () => {
