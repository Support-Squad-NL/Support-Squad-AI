...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `supportsquadai-manifest-registry-${randomUUID()}`);
...
function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "supportsquadai.plugin.json"), JSON.stringify(manifest), "utf-8");
}
