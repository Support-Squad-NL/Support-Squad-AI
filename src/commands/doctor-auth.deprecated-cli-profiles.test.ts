...
beforeEach(() => {
  envSnapshot = captureEnv(["SUPPORTSQUADAI_AGENT_DIR", "PI_CODING_AGENT_DIR"]);
  tempAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportsquadai-auth-"));
...
describe("maybeRemoveDeprecatedCliAuthProfiles", () => {
  it("removes deprecated CLI auth profiles from store + config", async () => {
    ...
    const next = await maybeRemoveDeprecatedCliAuthProfiles(
      cfg as unknown as SupportSquadAIConfig,
      makePrompter(true),
    );
