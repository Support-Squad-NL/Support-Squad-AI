...
export async function setupSkills(
  ...
  let next: SupportSquadAIConfig = cfg;
  if (installable.length > 0) {
    ...
    for (const name of selected) {
      ...
      runtime.log(
        `Tip: run \`${formatCliCommand("supportsquadai doctor")}\` to review skills + requirements.`,
      );
      runtime.log(`Docs: ${resolveBrandDocsLink("/skills")}`);
    }
  }
