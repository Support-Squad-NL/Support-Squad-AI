 ...
 * Runs as part of `supportsquadai doctor` — config-only, no network calls.
 */
export async function noteMemorySearchHealth(cfg: SupportSquadAIConfig): Promise<void> {
  ...
  if (resolved.provider !== "auto") {
    if (resolved.provider === "local") {
      ...
      note(
        [
          ...
          `- Switch to a remote provider: ${formatCliCommand("supportsquadai config set agents.defaults.memorySearch.provider openai")}`,
          "",
          `Verify: ${formatCliCommand("supportsquadai memory status --deep")}`,
        ].join("\n"),
        "Memory search",
      );
      return;
    ...
    note(
      [
        ...
        `- Add credentials: ${formatCliCommand(`supportsquadai auth add --provider ${resolved.provider}`)}`,
        `- To disable: ${formatCliCommand("supportsquadai config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("supportsquadai memory status --deep")}`,
      ].join("\n"),
      "Memory search",
  ...
  note(
    [
      ...
      `- Add credentials: ${formatCliCommand("supportsquadai auth add --provider openai")}`,
