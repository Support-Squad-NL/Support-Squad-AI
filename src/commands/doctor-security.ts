...
import type { SupportSquadAIConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: SupportSquadAIConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("supportsquadai security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.supportsquadai/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("supportsquadai approvals get --gateway")}`,
    );
  }
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("supportsquadai configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("supportsquadai config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("supportsquadai doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("supportsquadai config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('supportsquadai config set session.dmScope "per-channel-peer"') +
