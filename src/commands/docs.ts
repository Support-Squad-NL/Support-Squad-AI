...
const SEARCH_TOOL = "https://docs.supportsquadai.ai/mcp.SearchSupportSquadAI";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.supportsquadai.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('supportsquadai docs "your query"')}`);
    } else {
      runtime.log(`Docs: ${resolveBrandDocsUrl()}/`);
      runtime.log(`Search: ${formatCliCommand('supportsquadai docs "your query"')}`);
    }
    return;
  }
