...
function findOtherStateDirs(stateDir: string): string[] {
  ...
  for (const root of roots) {
    ...
    for (const entry of entries) {
      ...
      const candidates = [".supportsquadai"].map((dir) => path.resolve(root, entry.name, dir));
...
function shouldRequireOAuthDir(cfg: SupportSquadAIConfig, env: NodeJS.ProcessEnv): boolean {
  if (env.SUPPORTSQUADAI_OAUTH_DIR?.trim()) {
    return true;
  }
...
export async function noteStateIntegrity(
  ...
  const defaultStateDir = path.join(homedir(), ".supportsquadai");
...
export function noteWorkspaceBackupTip(workspaceDir: string) {
  ...
  note(
    [
      ...
      "- Keep ~/.supportsquadai out of git; it contains credentials and session history.",
