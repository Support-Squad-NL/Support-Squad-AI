...
vi.mock("../../plugins/loader.js", () => ({
  loadSupportSquadAIPlugins: vi.fn(),
}));
...
async function runInitialValueForChannel(channel: "dev" | "beta") {
  ...
  const cfg: SupportSquadAIConfig = { update: { channel } };
...
describe("ensureOnboardingPluginInstalled", () => {
  it("installs from npm and enables the plugin", async () => {
    ...
    const cfg: SupportSquadAIConfig = { plugins: { allow: ["other"] } };
    ...
    expect(result.cfg.plugins?.installs?.zalo?.spec).toBe("@supportsquadai/zalo");
    expect(result.cfg.plugins?.installs?.zalo?.installPath).toBe("/tmp/zalo");
    expect(installPluginFromNpmSpec).toHaveBeenCalledWith(
      expect.objectContaining({ spec: "@supportsquadai/zalo" }),
    );
  });
