...
describe("writeOAuthCredentials", () => {
  const lifecycle = createAuthTestLifecycle([
    ...
    "SUPPORTSQUADAI_AGENT_DIR",
    "PI_CODING_AGENT_DIR",
    "SUPPORTSQUADAI_OAUTH_DIR",
  ]);
  ...
  it("writes auth-profiles.json under SUPPORTSQUADAI_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("supportsquadai-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-oauth-sync-"));
    ...
    process.env.SUPPORTSQUADAI_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-oauth-nosync-"));
    process.env.SUPPORTSQUADAI_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside SUPPORTSQUADAI_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-oauth-external-"));
    process.env.SUPPORTSQUADAI_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* SUPPORTSQUADAI_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to SUPPORTSQUADAI_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("supportsquadai-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
