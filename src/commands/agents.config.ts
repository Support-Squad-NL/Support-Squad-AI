...
function resolveAgentName(cfg: SupportSquadAIConfig, agentId: string) {
...
function resolveAgentModel(cfg: SupportSquadAIConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: SupportSquadAIConfig): AgentSummary[] {
