...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "SUPPORTSQUADAI_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("SUPPORTSQUADAI_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv SUPPORTSQUADAI_GATEWAY_TOKEN");
    expect(message).not.toContain("SUPPORTSQUADAI_GATEWAY_PASSWORD");
  });
