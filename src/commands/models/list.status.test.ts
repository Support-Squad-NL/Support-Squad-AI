...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveSupportSquadAIAgentDir: vi.fn().mockReturnValue("/tmp/supportsquadai-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/supportsquadai-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/supportsquadai-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveSupportSquadAIAgentDir: mocks.resolveSupportSquadAIAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/supportsquadai-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveSupportSquadAIAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/supportsquadai-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/supportsquadai-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/supportsquadai-agent-custom");
