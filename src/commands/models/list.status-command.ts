...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveSupportSquadAIAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("supportsquadai models auth setup-token")}\` or \`${formatCliCommand("supportsquadai configure")}\`.`
          : `Run \`${formatCliCommand("supportsquadai configure")}\` or set an API key env var.`;
