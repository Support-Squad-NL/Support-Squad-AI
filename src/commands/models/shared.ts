...
export async function loadValidConfigOrThrow(): Promise<SupportSquadAIConfig> {
...
export async function updateConfig(
  mutator: (cfg: SupportSquadAIConfig) => SupportSquadAIConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: SupportSquadAIConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: SupportSquadAIConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("supportsquadai agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
