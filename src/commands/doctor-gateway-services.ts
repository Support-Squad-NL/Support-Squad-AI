...
function resolveGatewayAuthToken(cfg: SupportSquadAIConfig, env: NodeJS.ProcessEnv): string | undefined {
  const configToken = cfg.gateway?.auth?.token?.trim();
  if (configToken) {
    return configToken;
  }
  const envToken = env.SUPPORTSQUADAI_GATEWAY_TOKEN ?? env.CLAWDBOT_GATEWAY_TOKEN;
...
export async function maybeScanExtraGatewayServices(
  ...
  if (legacyServices.length > 0) {
    ...
    if (shouldRemove) {
      ...
      if (removed.length > 0) {
        runtime.log("Legacy gateway services removed. Installing SupportSquadAI gateway next.");
      }
    }
  }
