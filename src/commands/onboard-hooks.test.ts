...
describe("onboard-hooks", () => {
  ...
  const createMockHook = (
    ...
    ...params,
    source: "supportsquadai-bundled" as const,
    pluginId: undefined,
    homepage: undefined,
    always: false,
    disabled: false,
  ...
  const createMockHookReport = (eligible = true): HookStatusReport => ({
    ...
    managedHooksDir: "/mock/.supportsquadai/hooks",
    hooks: [
      createMockHook(
        {
          name: "session-memory",
  ...
  describe("setupInternalHooks", () => {
    ...
    it("should show informative notes to user", async () => {
      ...
      expect(noteCalls[1][0]).toMatch(/(?:supportsquadai|supportsquadai)( --profile isolated)? hooks list/);
    });
  });
});
