...
export const resolveSupportSquadAIPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadSupportSquadAIPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/supportsquadai.json",
    createConfigIO,
...
vi.mock("../infra/supportsquadai-root.js", () => ({
  resolveSupportSquadAIPackageRoot,
}));
...
beforeEach(() => {
  ...
  resolveSupportSquadAIPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.SUPPORTSQUADAI_STATE_DIR;
  originalUpdateInProgress = process.env.SUPPORTSQUADAI_UPDATE_IN_PROGRESS;
  process.env.SUPPORTSQUADAI_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportsquadai-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.SUPPORTSQUADAI_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.SUPPORTSQUADAI_UPDATE_IN_PROGRESS;
  } else {
    process.env.SUPPORTSQUADAI_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
