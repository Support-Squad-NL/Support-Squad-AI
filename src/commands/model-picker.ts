...
type PromptDefaultModelResult = { model?: string; config?: SupportSquadAIConfig };
...
function resolveConfiguredModelRaw(cfg: SupportSquadAIConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: SupportSquadAIConfig): string[] {
...
export function applyPrimaryModel(cfg: SupportSquadAIConfig, model: string): SupportSquadAIConfig {
...
export function applyModelAllowlist(cfg: SupportSquadAIConfig, models: string[]): SupportSquadAIConfig {
  const defaults = cfg.agents?.defaults;
