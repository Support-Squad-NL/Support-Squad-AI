...
function stripUnknownConfigKeys(config: SupportSquadAIConfig): {
  config: SupportSquadAIConfig;
  removed: string[];
} {
  const parsed = SupportSquadAISchema.safeParse(config);
  if (parsed.success) {
...
function noteOpencodeProviderOverrides(cfg: SupportSquadAIConfig) {
...
function scanTelegramAllowFromUsernameEntries(cfg: SupportSquadAIConfig): TelegramAllowFromUsernameHit[] {
...
async function maybeRepairTelegramAllowFromUsernames(cfg: SupportSquadAIConfig): Promise<{
  config: SupportSquadAIConfig;
  changes: string[];
}> {
...
function scanDiscordNumericIdEntries(cfg: SupportSquadAIConfig): DiscordNumericIdHit[] {
...
function maybeRepairDiscordNumericIds(cfg: SupportSquadAIConfig): {
  config: SupportSquadAIConfig;
  changes: string[];
} {
 ...
 */
function maybeRepairOpenPolicyAllowFrom(cfg: SupportSquadAIConfig): {
  config: SupportSquadAIConfig;
  changes: string[];
} {
...
function collectExecSafeBinScopes(cfg: SupportSquadAIConfig): ExecSafeBinScopeRef[] {
...
function scanExecSafeBinCoverage(cfg: SupportSquadAIConfig): ExecSafeBinCoverageHit[] {
...
function maybeRepairExecSafeBinProfiles(cfg: SupportSquadAIConfig): {
  config: SupportSquadAIConfig;
  changes: string[];
  warnings: string[];
} {
...
function scanLegacyToolsBySenderKeys(cfg: SupportSquadAIConfig): LegacyToolsBySenderKeyHit[] {
  const hits: LegacyToolsBySenderKeyHit[] = [];
  collectLegacyToolsBySenderKeyHits(cfg, [], hits);
  return hits;
}

function maybeRepairLegacyToolsBySenderKeys(cfg: SupportSquadAIConfig): {
  config: SupportSquadAIConfig;
  changes: string[];
} {
  const next = structuredClone(cfg);
...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetDir = path.join(home, ".supportsquadai");
  const targetPath = path.join(targetDir, "supportsquadai.json");
  try {
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: SupportSquadAIConfig = baseCfg;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("supportsquadai doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("supportsquadai doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (shouldRepair) {
    ...
    if (hits.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("supportsquadai doctor --fix")}" to auto-resolve @username entries to numeric IDs (requires a Telegram bot token).`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (discordHits.length > 0) {
      note(
        [
          ...
          `- Discord IDs must be strings; run "${formatCliCommand("supportsquadai doctor --fix")}" to convert numeric IDs to quoted strings.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (allowFromScan.changes.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("supportsquadai doctor --fix")}" to add missing allowFrom wildcards.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (toolsBySenderHits.length > 0) {
      ...
      note(
        [
          ...
          `- Run "${formatCliCommand("supportsquadai doctor --fix")}" to migrate legacy keys to typed id: entries.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (safeBinCoverage.length > 0) {
      ...
      lines.push(
        `- Run "${formatCliCommand("supportsquadai doctor --fix")}" to scaffold missing custom safeBinProfiles entries.`,
      );
      note(lines.join("\n"), "Doctor warnings");
    }
  }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "supportsquadai doctor --fix" to remove these keys.');
    }
  }
