...
const ensureSupportSquadAIModelsJson = vi.fn().mockResolvedValue(undefined);
const ensurePiAuthJsonFromAuthProfiles = vi
  .fn()
  .mockResolvedValue({ wrote: false, authPath: "/tmp/supportsquadai-agent/auth.json" });
const resolveSupportSquadAIAgentDir = vi.fn().mockReturnValue("/tmp/supportsquadai-agent");
...
vi.mock("../config/config.js", () => ({
  ...
  STATE_DIR: "/tmp/supportsquadai-state",
  loadConfig,
}));

vi.mock("../agents/models-config.js", () => ({
  ensureSupportSquadAIModelsJson,
}));
...
describe("models list/status", () => {
  ...
  it("models list syncs auth-profiles into auth.json before availability checks", async () => {
    ...
    expect(ensurePiAuthJsonFromAuthProfiles).toHaveBeenCalledWith("/tmp/supportsquadai-agent");
  });
