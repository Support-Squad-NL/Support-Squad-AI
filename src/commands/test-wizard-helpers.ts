...
export function requireSupportSquadAIAgentDir(): string {
  const agentDir = process.env.SUPPORTSQUADAI_AGENT_DIR;
  if (!agentDir) {
    throw new Error("SUPPORTSQUADAI_AGENT_DIR not set");
  }
  return agentDir;
}
