...
function resolveMode(cfg: SupportSquadAIConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("SupportSquadAI doctor");

  const root = await resolveSupportSquadAIPackageRoot({
  ...
  let cfg: SupportSquadAIConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("supportsquadai configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("supportsquadai config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("supportsquadai setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("supportsquadai doctor --fix")}" to apply changes.`);
  }
