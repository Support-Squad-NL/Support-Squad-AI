...
describe("applyAuthChoice", () => {
  ...
  async function setupTempState() {
    const env = await setupAuthTestEnv("supportsquadai-auth-");
  ...
  it("ignores legacy LiteLLM oauth profiles when selecting litellm-api-key", async () => {
    ...
    const authProfilePath = authProfilePathForAgent(requireSupportSquadAIAgentDir());
    await fs.writeFile(
      authProfilePath,
      JSON.stringify(
        {
