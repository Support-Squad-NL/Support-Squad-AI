...
async function runGatewayHealthCheck(params: {
  ...
  const token = params.cfg.gateway?.auth?.token ?? process.env.SUPPORTSQUADAI_GATEWAY_TOKEN;
  const password = params.cfg.gateway?.auth?.password ?? process.env.SUPPORTSQUADAI_GATEWAY_PASSWORD;
  ...
  try {
    ...
    note(
      [
        "Docs:",
        resolveBrandDocsLink("/gateway/health"),
        resolveBrandDocsLink("/gateway/troubleshooting"),
      ].join("\n"),
      "Health check help",
    );
  }
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from supportsquadai.json",
        },
      ],
...
async function promptWebToolsConfig(
  ...
  note(
    [
      ...
      `Docs: ${resolveBrandDocsLink("/tools/web")}`,
    ].join("\n"),
    "Web search",
  );
...
export async function runConfigureWizard(
  ...
  try {
    const productName = resolveBrandName();
    printWizardHeader(runtime);
    intro(opts.command === "update" ? `${productName} update wizard` : `${productName} configure`);
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: SupportSquadAIConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid && snapshot.issues.length > 0) {
        note(
          [
            ...
            "",
            `Docs: ${resolveBrandDocsLink("/gateway/configuration")}`,
          ].join("\n"),
          "Config issues",
        );
      }
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("supportsquadai doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: baseConfig.gateway?.auth?.token ?? process.env.SUPPORTSQUADAI_GATEWAY_TOKEN,
      password: baseConfig.gateway?.auth?.password ?? process.env.SUPPORTSQUADAI_GATEWAY_PASSWORD,
    });
    ...
    const newPassword = nextConfig.gateway?.auth?.password ?? process.env.SUPPORTSQUADAI_GATEWAY_PASSWORD;
    const oldPassword = baseConfig.gateway?.auth?.password ?? process.env.SUPPORTSQUADAI_GATEWAY_PASSWORD;
    const token = nextConfig.gateway?.auth?.token ?? process.env.SUPPORTSQUADAI_GATEWAY_TOKEN;
    ...
    note(
      [
        ...
        gatewayStatusLine,
        `Docs: ${resolveBrandDocsLink("/web/control-ui")}`,
      ].join("\n"),
      "Control UI",
    );
