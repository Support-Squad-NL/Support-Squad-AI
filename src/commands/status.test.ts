...
beforeAll(() => {
  envSnapshot = captureEnv(["SUPPORTSQUADAI_PROFILE"]);
  process.env.SUPPORTSQUADAI_PROFILE = "isolated";
});
  ...
  MemoryIndexManager: {
    get: vi.fn(async ({ agentId }: { agentId: string }) => ({
      ...
      status: () => ({
        ...
        chunks: 3,
        dirty: false,
        workspaceDir: "/tmp/supportsquadai",
...
vi.mock("../infra/supportsquadai-root.js", () => ({
  resolveSupportSquadAIPackageRoot: vi.fn().mockResolvedValue("/tmp/supportsquadai"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/supportsquadai/pnpm-lock.yaml",
      markerPath: "/tmp/supportsquadai/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("SupportSquadAI status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("supportsquadai status --all") ||
          l.includes("supportsquadai --profile isolated status --all") ||
          l.includes("supportsquadai status --all") ||
          l.includes("supportsquadai --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.SUPPORTSQUADAI_GATEWAY_TOKEN = "abcd1234";
    try {
