...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: SupportSquadAIConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: SupportSquadAIConfig) => { changed: boolean; next: SupportSquadAIConfig };
  defaultModel: string;
  overrideConfig: SupportSquadAIConfig;
  alreadyDefaultConfig: SupportSquadAIConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: SupportSquadAIConfig) => config,
      applyDefaultConfig: (config: SupportSquadAIConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: SupportSquadAIConfig = { agents: { defaults: {} } };
