    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["supportsquadai nodes status", "List known nodes with live status."],
          ["supportsquadai nodes pairing pending", "Show pending node pairing requests."],
          ['supportsquadai nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["supportsquadai nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.supportsquadai.ai/cli/nodes")}\n`,
    );
