...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("supportsquadai node install"), formatCliCommand("supportsquadai node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("supportsquadai node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    SUPPORTSQUADAI_LOG_PREFIX: baseEnv.SUPPORTSQUADAI_LOG_PREFIX ?? "node",
