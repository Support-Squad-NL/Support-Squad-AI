    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "supportsquadai node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["supportsquadai node status", "Check node host service status."],
          ["supportsquadai node install", "Install the node host service."],
          ["supportsquadai node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.supportsquadai.ai/cli/node")}\n`,
    );
