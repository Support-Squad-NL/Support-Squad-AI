...
${formatHelpExamples([
  ['supportsquadai agent --to +15555550123 --message "status update"', "Start a new session."],
  ['supportsquadai agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'supportsquadai agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'supportsquadai agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['supportsquadai agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'supportsquadai agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.supportsquadai.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.supportsquadai.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['supportsquadai agents set-identity --agent main --name "SupportSquadAI" --emoji "🦞"', "Set name + emoji."],
  ["supportsquadai agents set-identity --agent main --avatar avatars/supportsquadai.png", "Set avatar path."],
  [
    "supportsquadai agents set-identity --workspace ~/.supportsquadai/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "supportsquadai agents set-identity --identity-file ~/.supportsquadai/workspace/IDENTITY.md --agent main",
