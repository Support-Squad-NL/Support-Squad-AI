...
beforeEach(() => {
  ...
  originalHideBanner = process.env.SUPPORTSQUADAI_HIDE_BANNER;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.SUPPORTSQUADAI_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.SUPPORTSQUADAI_HIDE_BANNER = originalHideBanner;
  }
});

describe("registerPreActionHooks", () => {
  function buildProgram() {
    const program = new Command().name("supportsquadai");
  ...
  it("emits banner, resolves config, and enables verbose from --debug", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "supportsquadai", "status", "--debug"],
    });
    ...
    expect(process.title).toBe("supportsquadai-status");
  });

  it("loads plugin registry for plugin-required commands", async () => {
    await runCommand({
      parseArgv: ["message", "send"],
      processArgv: ["node", "supportsquadai", "message", "send"],
    });
  ...
  it("skips config guard for doctor and completion commands", async () => {
    await runCommand({
      parseArgv: ["doctor"],
      processArgv: ["node", "supportsquadai", "doctor"],
    });
    await runCommand({
      parseArgv: ["completion"],
      processArgv: ["node", "supportsquadai", "completion"],
    });
  ...
  it("skips preaction work when argv indicates help/version", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "supportsquadai", "--version"],
    });
  ...
  it("hides banner when SUPPORTSQUADAI_HIDE_BANNER is truthy", async () => {
    process.env.SUPPORTSQUADAI_HIDE_BANNER = "1";
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "supportsquadai", "status"],
    });
