...
describe("reparseProgramFromActionArgs", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    buildParseArgvMock.mockReturnValue(["node", "supportsquadai", "status"]);
  ...
  it("uses action command name + args as fallback argv", async () => {
    ...
    const actionCommand = {
      name: () => "status",
      parent: {
        rawArgs: ["node", "supportsquadai", "status", "--json"],
      },
    ...
    expect(parseAsync).toHaveBeenCalledWith(["node", "supportsquadai", "status"]);
  });
