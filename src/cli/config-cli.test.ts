...
import type { ConfigFileSnapshot, SupportSquadAIConfig } from "../config/types.js";

/**
 * Test for issue #6070:
 * `supportsquadai config set/unset` must update snapshot.resolved (user config after $include/${ENV},
 * but before runtime defaults), so runtime defaults don't leak into the written config.
 */

const mockReadConfigFileSnapshot = vi.fn<() => Promise<ConfigFileSnapshot>>();
const mockWriteConfigFile = vi.fn<
  (cfg: SupportSquadAIConfig, options?: { unsetPaths?: string[][] }) => Promise<void>
>(async () => {});

vi.mock("../config/config.js", () => ({
  readConfigFileSnapshot: () => mockReadConfigFileSnapshot(),
  writeConfigFile: (cfg: SupportSquadAIConfig, options?: { unsetPaths?: string[][] }) =>
...
function setSnapshot(resolved: SupportSquadAIConfig, config: SupportSquadAIConfig) {
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      const resolved: SupportSquadAIConfig = {
        agents: {
      ...
      const runtimeMerged: SupportSquadAIConfig = {
        ...resolved,
        agents: {
          ...resolved.agents,
          defaults: {
  ...
  describe("config get", () => {
    it("redacts sensitive values", async () => {
      ...
      expect(mockLog).toHaveBeenCalledWith("__SUPPORTSQUADAI_REDACTED__");
    });
  });

  describe("config set parsing flags", () => {
    it("falls back to raw string when parsing fails and strict mode is off", async () => {
      const resolved: SupportSquadAIConfig = { gateway: { port: 18789 } };
      setSnapshot(resolved, resolved);
