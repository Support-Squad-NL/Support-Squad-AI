...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@supportsquadai/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@supportsquadai/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@supportsquadai/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@supportsquadai/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @supportsquadai/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      mapNpmResolutionMetadata({
        name: "@supportsquadai/plugin-alpha",
        version: "1.2.3",
        resolvedSpec: "@supportsquadai/plugin-alpha@1.2.3",
      ...
      resolvedName: "@supportsquadai/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@supportsquadai/plugin-alpha@1.2.3",
        installPath: "/tmp/supportsquadai/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@supportsquadai/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @supportsquadai/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@supportsquadai/plugin-alpha@latest",
      true,
      "/tmp/supportsquadai/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@supportsquadai/plugin-alpha@latest",
      installPath: "/tmp/supportsquadai/extensions/alpha",
      version: "1.2.3",
