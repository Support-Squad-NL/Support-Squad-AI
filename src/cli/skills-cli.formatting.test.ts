...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportsquadai-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "supportsquadai-bundled-skills-test-"));
    process.env.SUPPORTSQUADAI_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
