...
function applySlotSelectionForPlugin(
  ...
  pluginId: string,
): { config: SupportSquadAIConfig; warnings: string[] } {
...
export function registerPluginsCli(program: Command) {
  const plugins = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.supportsquadai.ai/cli/plugins")}\n`,
    );
  ...
  plugins
    ...
    .action(async (id: string) => {
      ...
      let next: SupportSquadAIConfig = enableResult.config;
    ...
    .action(async (raw: string, opts: { link?: boolean; pin?: boolean }) => {
      ...
      if (fs.existsSync(resolved)) {
        if (opts.link) {
          ...
          let next: SupportSquadAIConfig = enablePluginInConfig(
            {
              ...cfg,
              plugins: {
                ...cfg.plugins,
  ...
  plugins
    ...
    .action(() => {
      ...
      const docs = formatDocsLink("/plugin", "docs.supportsquadai.ai/plugin");
      lines.push("");
