...
import { resolveSupportSquadAIPackageRoot } from "../../infra/supportsquadai-root.js";
...
const SUPPORTSQUADAI_REPO_URL = "https://github.com/supportsquadai/supportsquadai.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "supportsquadai";
...
export function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("supportsquadai@")) {
    return trimmed.slice("supportsquadai@".length);
  }
...
export function resolveGitInstallDir(): string {
  const override = process.env.SUPPORTSQUADAI_GIT_DIR?.trim();
  if (override) {
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveSupportSquadAIPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", SUPPORTSQUADAI_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `SUPPORTSQUADAI_GIT_DIR points at a non-git directory: ${params.dir}. Set SUPPORTSQUADAI_GIT_DIR to an empty folder or an supportsquadai checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`SUPPORTSQUADAI_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "supportsquadai.mjs");
