...
async function loadValidConfig(runtime: RuntimeEnv = defaultRuntime) {
  ...
  runtime.error(`Run \`${formatCliCommand("supportsquadai doctor")}\` to repair, then retry.`);
  runtime.exit(1);
  return snapshot;
}
...
export function registerConfigCli(program: Command) {
  const cmd = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.supportsquadai.ai/cli/config")}\n`,
    )
    .option(
      "--section <section>",
