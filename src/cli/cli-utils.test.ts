...
describe("shouldSkipRespawnForArgv", () => {
  it("skips respawn for help/version calls", () => {
    const cases = [
      ["node", "supportsquadai", "--help"],
      ["node", "supportsquadai", "-V"],
    ] as const;
  ...
  it("keeps respawn path for normal commands", () => {
    expect(shouldSkipRespawnForArgv(["node", "supportsquadai", "status"])).toBe(false);
  });
});
...
describe("dns cli", () => {
  it("prints setup info (no apply)", async () => {
    ...
    try {
      ...
      await program.parseAsync(["dns", "setup", "--domain", "supportsquadai.internal"], { from: "user" });
      const output = log.mock.calls.map((call) => call.join(" ")).join("\\n");
      expect(output).toContain("DNS setup");
      expect(output).toContain("supportsquadai.internal");
    } finally {
      log.mockRestore();
    }
  });
