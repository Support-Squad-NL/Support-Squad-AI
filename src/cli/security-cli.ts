...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["supportsquadai security audit", "Run a local security audit."],
          ["supportsquadai security audit --deep", "Include best-effort live Gateway probe checks."],
          ["supportsquadai security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["supportsquadai security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.supportsquadai.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("SupportSquadAI security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("supportsquadai security audit --deep")}`));

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("supportsquadai security audit --fix")}`));
        if (!fixResult) {
