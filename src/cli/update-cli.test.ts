...
import type { SupportSquadAIConfig, ConfigFileSnapshot } from "../config/types.supportsquadai.js";
...
vi.mock("../infra/supportsquadai-root.js", () => ({
  resolveSupportSquadAIPackageRoot: vi.fn(),
}));
...
const { resolveSupportSquadAIPackageRoot } = await import("../infra/supportsquadai-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/supportsquadai-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as SupportSquadAIConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/supportsquadai-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveSupportSquadAIPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("supportsquadai-update");
    setTty(false);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveSupportSquadAIPackageRoot).mockClear();
    ...
    vi.mocked(resolveSupportSquadAIPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("supportsquadai");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/supportsquadai-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "supportsquadai-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("SupportSquadAI update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as SupportSquadAIConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveSupportSquadAIPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("supportsquadai-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ SUPPORTSQUADAI_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.SUPPORTSQUADAI_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("supportsquadai-update-wizard");
    await withEnvAsync({ SUPPORTSQUADAI_GIT_DIR: tempDir }, async () => {
      setTty(true);
