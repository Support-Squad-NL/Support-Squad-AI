...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.supportsquadai.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + SupportSquadAI hooks")
    ...
    .option("--hook-url <url>", "SupportSquadAI hook URL")
    .option("--hook-token <token>", "SupportSquadAI hook token")
