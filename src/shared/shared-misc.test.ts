...
describe("shared/frontmatter", () => {
  ...
  test("resolveSupportSquadAIManifestBlock parses JSON5 metadata and picks supportsquadai block", () => {
    const frontmatter = {
      metadata: "{ supportsquadai: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveSupportSquadAIManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveSupportSquadAIManifestBlock returns undefined for invalid input", () => {
    expect(resolveSupportSquadAIManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveSupportSquadAIManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveSupportSquadAIManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
