...
function listAgents(cfg: SupportSquadAIConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: SupportSquadAIConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: SupportSquadAIConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<SupportSquadAIConfig, EvaluatedBindingsCache>();
