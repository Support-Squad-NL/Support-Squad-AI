...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.SUPPORTSQUADAI_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*supportsquadai-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "supportsquadai";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "supportsquadai");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "supportsquadai-gw");
  ...
  records.push(`_supportsquadai-gw._tcp IN PTR ${instanceLabel}._supportsquadai-gw._tcp`);
  records.push(`${instanceLabel}._supportsquadai-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._supportsquadai-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; supportsquadai-content-hash: ${contentHash}\n${contentBody}`;
}
