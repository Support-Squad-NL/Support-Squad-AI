...
function enableAdvertiserUnitMode(hostname = "test-host") {
  ...
  process.env.SUPPORTSQUADAI_MDNS_HOSTNAME = hostname;
}
...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    expect(gatewayCall?.[0]?.type).toBe("supportsquadai-gw");
    ...
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.cliPath).toBe(
      "/opt/homebrew/bin/supportsquadai",
    );
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("supportsquadai (SupportSquadAI)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
    expect(gatewayCall?.[0]?.hostname).toBe("supportsquadai");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("supportsquadai.local");
