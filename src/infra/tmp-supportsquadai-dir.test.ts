...
import { POSIX_SUPPORTSQUADAI_TMP_DIR, resolvePreferredSupportSquadAITmpDir } from "./tmp-supportsquadai-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredSupportSquadAITmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `supportsquadai-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredSupportSquadAITmpDir({
    accessSync,
...
describe("resolvePreferredSupportSquadAITmpDir", () => {
  it("prefers /tmp/supportsquadai when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_SUPPORTSQUADAI_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/supportsquadai when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_SUPPORTSQUADAI_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/supportsquadai when /tmp/supportsquadai is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/supportsquadai when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/supportsquadai is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/supportsquadai is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/supportsquadai is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
