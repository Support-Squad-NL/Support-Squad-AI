...
const TEMP_DIR_PREFIX = "supportsquadai-install-source-utils-";
...
describe("withTempDir", () => {
  it("creates a temp dir and always removes it after callback", async () => {
    ...
    const value = await withTempDir("supportsquadai-install-source-utils-", async (tmpDir) => {
      observedDir = tmpDir;
...
describe("resolveArchiveSourcePath", () => {
  it("returns not found error for missing archive paths", async () => {
    const result = await resolveArchiveSourcePath("/tmp/does-not-exist-supportsquadai-archive.tgz");
...
describe("packNpmSpecToArchive", () => {
  it("packs spec and returns archive path using JSON output metadata", async () => {
    ...
    mockPackCommandResult({
      stdout: JSON.stringify([
        {
          id: "supportsquadai-plugin@1.2.3",
          name: "supportsquadai-plugin",
          version: "1.2.3",
          filename: "supportsquadai-plugin-1.2.3.tgz",
    ...
    const result = await runPack("supportsquadai-plugin@1.2.3", cwd);

    expect(result).toEqual({
      ok: true,
      archivePath: path.join(cwd, "supportsquadai-plugin-1.2.3.tgz"),
      metadata: {
        name: "supportsquadai-plugin",
        version: "1.2.3",
        resolvedSpec: "supportsquadai-plugin@1.2.3",
        integrity: "sha512-test-integrity",
        shasum: "abc123",
      },
    });
    expect(runCommandWithTimeoutMock).toHaveBeenCalledWith(
      ["npm", "pack", "supportsquadai-plugin@1.2.3", "--ignore-scripts", "--json"],
  ...
  it("returns explicit error when npm pack produces no archive name", async () => {
    ...
    const result = await runPack("supportsquadai-plugin@1.2.3", cwd, 5000);
  ...
  it("parses scoped metadata from id-only json output even with npm notice prefix", async () => {
    ...
    mockPackCommandResult({
      stdout:
        ...
        JSON.stringify([
          {
            id: "@supportsquadai/plugin-demo@2.0.0",
            filename: "supportsquadai-plugin-demo-2.0.0.tgz",
          },
        ]),
    });

    const result = await runPack("@supportsquadai/plugin-demo@2.0.0", cwd);
    expect(result).toEqual({
      ok: true,
      archivePath: path.join(cwd, "supportsquadai-plugin-demo-2.0.0.tgz"),
      metadata: {
        resolvedSpec: "@supportsquadai/plugin-demo@2.0.0",
      },
    });
  });
