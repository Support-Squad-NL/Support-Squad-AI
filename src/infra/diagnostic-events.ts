...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __supportsquadaiDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__supportsquadaiDiagnosticEventsState) {
    globalStore.__supportsquadaiDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__supportsquadaiDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: SupportSquadAIConfig): boolean {
