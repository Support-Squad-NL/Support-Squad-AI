...
type EnsureSupportSquadAIPathOpts = {
  execPath?: string;
  cwd?: string;
  homeDir?: string;
...
function candidateBinDirs(opts: EnsureSupportSquadAIPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `supportsquadai` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "supportsquadai");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.SUPPORTSQUADAI_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "supportsquadai"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `supportsquadai` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureSupportSquadAICliOnPath(opts: EnsureSupportSquadAIPathOpts = {}) {
  if (isTruthyEnvValue(process.env.SUPPORTSQUADAI_PATH_BOOTSTRAPPED)) {
    return;
  }
