...
const DEFAULT_PACKAGE_NAME = "supportsquadai";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && !pkgRoot) {
    return {
      ...
      mode: "unknown",
      root: gitRoot,
      reason: "not-supportsquadai-root",
      steps: [],
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "supportsquadai.mjs");
    ...
    if (!doctorEntryExists) {
      steps.push({
        name: "supportsquadai doctor entry",
    ...
    const doctorStep = await runStep(
      step("supportsquadai doctor", doctorArgv, gitRoot, { SUPPORTSQUADAI_UPDATE_IN_PROGRESS: "1" }),
    );
