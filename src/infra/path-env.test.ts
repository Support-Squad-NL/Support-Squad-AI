...
let ensureSupportSquadAICliOnPath: typeof import("./path-env.js").ensureSupportSquadAICliOnPath;

describe("ensureSupportSquadAICliOnPath", () => {
  const envKeys = [
    "PATH",
    "SUPPORTSQUADAI_PATH_BOOTSTRAPPED",
    "SUPPORTSQUADAI_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureSupportSquadAICliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling supportsquadai exists", () => {
    const tmp = abs("/tmp/supportsquadai-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "supportsquadai");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.SUPPORTSQUADAI_PATH_BOOTSTRAPPED;

    ensureSupportSquadAICliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.SUPPORTSQUADAI_PATH_BOOTSTRAPPED = "1";
    ensureSupportSquadAICliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/supportsquadai-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "supportsquadai");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/supportsquadai-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "supportsquadai");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "supportsquadai");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/supportsquadai-path/case-linuxbrew");
