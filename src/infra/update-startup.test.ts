...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["SUPPORTSQUADAI_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.SUPPORTSQUADAI_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveSupportSquadAIPackageRoot } = await import("./supportsquadai-root.js"));
  ...
  function mockPackageUpdateStatus(tag = "latest", version = "2.0.0") {
    vi.mocked(resolveSupportSquadAIPackageRoot).mockResolvedValue("/opt/supportsquadai");
    vi.mocked(checkUpdateStatus).mockResolvedValue({
      root: "/opt/supportsquadai",
      installKind: "package",
      packageManager: "npm",
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/supportsquadai/dist/entry.js"];
    try {
      await runGatewayUpdateCheck({
        cfg: {
          update: {
    ...
    expect(runCommandWithTimeout).toHaveBeenCalledWith(
      [
        process.execPath,
        "/opt/supportsquadai/dist/entry.js",
        "update",
        "--yes",
        "--channel",
        "beta",
