...
describe("resolveEffectiveHomeDir", () => {
  it("prefers SUPPORTSQUADAI_HOME over HOME and USERPROFILE", () => {
    const env = {
    ...
    expect(resolveEffectiveHomeDir(env, () => "/fallback")).toBe(
      path.resolve("/srv/supportsquadai-home"),
    );
  });
  ...
  it("expands SUPPORTSQUADAI_HOME when set to ~", () => {
    const env = {
      SUPPORTSQUADAI_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for SUPPORTSQUADAI_HOME", () => {
    const result = resolveRequiredHomeDir(
      { SUPPORTSQUADAI_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when SUPPORTSQUADAI_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ SUPPORTSQUADAI_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { SUPPORTSQUADAI_HOME: "/srv/supportsquadai-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/supportsquadai-home")}/x`);
  });
