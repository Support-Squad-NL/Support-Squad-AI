...
import { resolveSupportSquadAIPackageRoot, resolveSupportSquadAIPackageRootSync } from "./supportsquadai-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveSupportSquadAIPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "supportsquadai" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveSupportSquadAIPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
  ...
  if (argv1Dir) {
    // supportsquadai.mjs or dist/<bundle>.js
