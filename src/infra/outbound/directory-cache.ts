...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: SupportSquadAIConfig | null = null;
  ...
  get(key: string, cfg: SupportSquadAIConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: SupportSquadAIConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  clear(cfg?: SupportSquadAIConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: SupportSquadAIConfig): void {
