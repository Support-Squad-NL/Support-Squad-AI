...
describe("fs-safe", () => {
  it("reads a local file safely", async () => {
    const dir = await tempDirs.make("supportsquadai-fs-safe-");
  ...
  it("blocks traversal outside root", async () => {
    const root = await tempDirs.make("supportsquadai-fs-safe-root-");
    const outside = await tempDirs.make("supportsquadai-fs-safe-outside-");
