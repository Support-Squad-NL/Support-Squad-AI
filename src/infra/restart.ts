...
export function triggerSupportSquadAIRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.SUPPORTSQUADAI_SYSTEMD_UNIT,
        process.env.SUPPORTSQUADAI_PROFILE,
      );
  ...
  const label =
    process.env.SUPPORTSQUADAI_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.SUPPORTSQUADAI_PROFILE);
