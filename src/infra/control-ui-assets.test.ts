...
vi.mock("./supportsquadai-root.js", () => ({
  resolveSupportSquadAIPackageRoot: vi.fn(async () => null),
  resolveSupportSquadAIPackageRootSync: vi.fn(() => null),
}));
...
let supportsquadaiRoot: typeof import("./supportsquadai-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    supportsquadaiRoot = await import("./supportsquadai-root.js");
  });
  ...
  it("uses resolveSupportSquadAIPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/supportsquadai");
    (
      supportsquadaiRoot.resolveSupportSquadAIPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/supportsquadai"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "supportsquadai" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "supportsquadai.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-supportsquadai");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/supportsquadai-bundle");
    (
      supportsquadaiRoot.resolveSupportSquadAIPackageRootSync as unknown as ReturnType<typeof vi.fn>
