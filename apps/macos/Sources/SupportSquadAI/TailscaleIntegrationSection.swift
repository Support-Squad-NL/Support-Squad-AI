...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.supportsquadai/supportsquadai.json. Prefer SUPPORTSQUADAI_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.supportsquadai/supportsquadai.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.supportsquadai/supportsquadai.json. Restart the gateway to apply."
        }
