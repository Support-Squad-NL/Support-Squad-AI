...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (SupportSquadAI token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: SupportSquadAIOAuthStore.AnthropicOAuthStatus = SupportSquadAIOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.supportsquadai", category: "anthropic-oauth")
...
enum SupportSquadAIOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let supportsquadaiOAuthDirEnv = "SUPPORTSQUADAI_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "SupportSquadAI OAuth token file not found"
            case .unreadableFile: "SupportSquadAI OAuth token file not readable"
            case .invalidJSON: "SupportSquadAI OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in SupportSquadAI OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "SupportSquadAI OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.supportsquadaiOAuthDirEnv]?
        ...
        return home.appendingPathComponent(".supportsquadai", isDirectory: true)
