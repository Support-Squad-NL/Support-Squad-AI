...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["SUPPORTSQUADAI_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/supportsquadai")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("supportsquadai-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("supportsquadai-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/supportsquadai/ (rolling or stdout), or nil if none exist.
