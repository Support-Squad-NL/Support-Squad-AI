...
private let deepLinkLogger = Logger(subsystem: "ai.supportsquadai", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "SupportSquadAI is paused", message: "Unpause SupportSquadAI to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run SupportSquadAI agent?", message: body) else { return }
        }
