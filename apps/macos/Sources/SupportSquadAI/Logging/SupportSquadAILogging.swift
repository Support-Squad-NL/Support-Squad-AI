...
enum SupportSquadAILogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = SupportSquadAIOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = SupportSquadAIFileLogHandler(label: label)
    ...
    static func parseLabel(_ label: String) -> (String, String) {
        guard let range = label.range(of: labelSeparator) else {
            return ("ai.supportsquadai", label)
        }
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        SupportSquadAILogging.bootstrapIfNeeded()
        let label = SupportSquadAILogging.makeLabel(subsystem: subsystem, category: category)
...
struct SupportSquadAIOSLogHandler: LogHandler {
...
struct SupportSquadAIFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = SupportSquadAILogging.parseLabel(self.label)
