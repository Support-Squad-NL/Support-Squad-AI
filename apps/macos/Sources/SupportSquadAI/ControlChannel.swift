...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: SupportSquadAIProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.supportsquadai", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: SupportSquadAIKit.AnyCodable]()) {
                $0[$1.key] = SupportSquadAIKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: SupportSquadAIProtocol.AnyCodable?) -> [String: SupportSquadAIProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: SupportSquadAIProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: SupportSquadAIKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: SupportSquadAIProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("supportsquadai.control.heartbeat")
    static let controlAgentEvent = Notification.Name("supportsquadai.control.agent")
}
