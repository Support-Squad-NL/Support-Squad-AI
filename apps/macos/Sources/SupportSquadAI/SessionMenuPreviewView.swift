...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.supportsquadai", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> SupportSquadAISessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: SupportSquadAISessionPreviewEntry,
    ...
    private static func cache(payload: SupportSquadAISessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: SupportSquadAIChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [SupportSquadAIKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [SupportSquadAIKit.AnyCodable]) -> [SupportSquadAIChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(SupportSquadAIChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: SupportSquadAIChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: SupportSquadAIChatMessage) -> Bool {
    ...
    private static func toolNames(for message: SupportSquadAIChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: SupportSquadAIChatMessage) -> String? {
