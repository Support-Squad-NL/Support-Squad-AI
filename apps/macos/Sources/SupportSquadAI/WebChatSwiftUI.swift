...
private let webChatSwiftLogger = Logger(subsystem: "ai.supportsquadai", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: SupportSquadAIChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> SupportSquadAIChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(SupportSquadAIChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> SupportSquadAIChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                SupportSquadAIGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    SupportSquadAIChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    SupportSquadAIAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<SupportSquadAIChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any SupportSquadAIChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = SupportSquadAIChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: SupportSquadAIChatView(
            viewModel: vm,
        ...
        contentViewController: NSViewController) -> NSWindow
    {
        ...
        case .window:
            let window = NSWindow(
                ...
                backing: .buffered,
                defer: false)
            window.title = "SupportSquadAI Chat"
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<SupportSquadAIChatView>) -> NSViewController
    {
