...
enum DebugActions {
    private static let verboseDefaultsKey = "supportsquadai.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "supportsquadai.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = SupportSquadAIPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "SupportSquadAI", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = SupportSquadAIPaths.configURL
        guard
