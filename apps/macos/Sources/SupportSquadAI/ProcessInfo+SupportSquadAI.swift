...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.supportsquadai.mac`) even if the shipped
    ...
    {
        if environment["SUPPORTSQUADAI_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "supportsquadai.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "supportsquadai.nixMode") { return true }
