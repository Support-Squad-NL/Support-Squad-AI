...
enum CommandResolver {
    private static let projectRootDefaultsKey = "supportsquadai.gatewayProjectRootPath"
    private static let helperName = "supportsquadai"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let supportsquadaiEntry = root.appendingPathComponent("supportsquadai.mjs").path
        if FileManager().isReadableFile(atPath: supportsquadaiEntry) { return supportsquadaiEntry }
        let binEntry = root.appendingPathComponent("bin/supportsquadai.js").path
    ...
    static func errorCommand(with message: String) -> [String] {
        let script = """
        cat <<'__SUPPORTSQUADAI_ERR__' >&2
        \(message)
        __SUPPORTSQUADAI_ERR__
        exit 1
        """
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/supportsquadai")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let supportsquadaiPaths = self.supportsquadaiManagedPaths(home: home)
        if !supportsquadaiPaths.isEmpty {
            extras.insert(contentsOf: supportsquadaiPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + supportsquadaiPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func supportsquadaiManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".supportsquadai"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func supportsquadaiExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectSupportSquadAIExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("supportsquadai.mjs").path,
            root.appendingPathComponent("bin/supportsquadai.js").path,
        ]
    ...
    static func hasAnySupportSquadAIInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.supportsquadaiExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func supportsquadaiNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let supportsquadaiPath = self.projectSupportSquadAIExecutable(projectRoot: root) {
                return [supportsquadaiPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "supportsquadai", subcommand] + extraArgs
            }
            if let supportsquadaiPath = self.supportsquadaiExecutable(searchPaths: searchPaths) {
                return [supportsquadaiPath, subcommand] + extraArgs
            }

            let missingEntry = """
            supportsquadai entrypoint missing (looked for dist/index.js or supportsquadai.mjs); run pnpm build.
            """
    ...
    static func supportsquadaiCommand(
        subcommand: String,
        extraArgs: [String] = [],
        defaults: UserDefaults = .standard,
        configRoot: [String: Any]? = nil,
        searchPaths: [String]? = nil) -> [String]
    {
        self.supportsquadaiNodeCommand(
            subcommand: subcommand,
            extraArgs: extraArgs,
            defaults: defaults,
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real supportsquadai CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/supportsquadai"
        ...
        \(cliSection)
        \(projectSection)
        if command -v supportsquadai >/dev/null 2>&1; then
          CLI="$(command -v supportsquadai)"
          supportsquadai \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/supportsquadai.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/supportsquadai.mjs"
            node "$PRJ/supportsquadai.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/supportsquadai.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/supportsquadai.js"
            node "$PRJ/bin/supportsquadai.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent supportsquadai"
          pnpm --silent supportsquadai \(quotedArgs);
        else
          echo "supportsquadai CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? SupportSquadAIConfigFile.loadDict()
