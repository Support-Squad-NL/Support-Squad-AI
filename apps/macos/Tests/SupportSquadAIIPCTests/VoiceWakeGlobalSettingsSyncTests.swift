...
@Suite(.serialized) struct VoiceWakeGlobalSettingsSyncTests {
    @Test func appliesVoiceWakeChangedEventToAppState() async {
        ...
        let payload = SupportSquadAIProtocol.AnyCodable(["triggers": ["supportsquadai", "computer"]])
        let evt = EventFrame(
            type: "event",
        ...
        #expect(updated == ["supportsquadai", "computer"])
    ...
    @Test func ignoresVoiceWakeChangedEventWithInvalidPayload() async {
        ...
        let payload = SupportSquadAIProtocol.AnyCodable(["unexpected": 123])
        let evt = EventFrame(
            type: "event",
            event: "voicewake.changed",
            payload: payload,
