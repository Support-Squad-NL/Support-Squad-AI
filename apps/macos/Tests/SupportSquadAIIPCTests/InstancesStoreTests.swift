...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: SupportSquadAIProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: SupportSquadAIProtocol.AnyCodable] = [
            "presence": .init([SupportSquadAIProtocol.AnyCodable(entry)]),
        ]
        let payload = SupportSquadAIProtocol.AnyCodable(payloadMap)
