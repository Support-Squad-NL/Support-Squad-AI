...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: SupportSquadAIProtocol.AnyCodable(["ok": SupportSquadAIProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: SupportSquadAIProtocol.AnyCodable(["ok": SupportSquadAIProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = SupportSquadAIProtocol.AnyCodable([
            "runId": SupportSquadAIProtocol.AnyCodable("run-1"),
            "sessionKey": SupportSquadAIProtocol.AnyCodable("main"),
            "state": SupportSquadAIProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: SupportSquadAIProtocol.AnyCodable(["a": SupportSquadAIProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
