...
@Suite struct LogLocatorTests {
    @Test func launchdGatewayLogPathEnsuresTmpDirExists() throws {
        ...
        let logDir = baseDir.appendingPathComponent("supportsquadai-tests-\(UUID().uuidString)")

        setenv("SUPPORTSQUADAI_LOG_DIR", logDir.path, 1)
        defer {
            unsetenv("SUPPORTSQUADAI_LOG_DIR")
