# SupportSquadAI iOS (Super Alpha)

NO TEST FLIGHT AVAILABLE AT THIS POINT

This iPhone app is super-alpha and internal-use only. It connects to an SupportSquadAI Gateway as a `role: node`.
...
cd apps/ios
xcodegen generate
open SupportSquadAI.xcodeproj
```

3. In Xcode:
   - Scheme: `SupportSquadAI`
...
- `apps/ios/Sources/SupportSquadAI.entitlements` sets `aps-environment` to `development`.
