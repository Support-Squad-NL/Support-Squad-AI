...
@Suite(.serialized) struct VoiceWakeManagerStateTests {
    ...
    @Test @MainActor func handleRecognitionCallbackDispatchesCommand() async {
        let manager = VoiceWakeManager()
        manager.triggerWords = ["supportsquadai"]
        manager.isEnabled = true
        ...
        let transcript = "supportsquadai hello"
        let triggerRange = transcript.range(of: "supportsquadai")!
        let helloRange = transcript.range(of: "hello")!
        let segments = [
            WakeWordSegment(text: "supportsquadai", start: 0.0, duration: 0.2, range: triggerRange),
