...
@Suite struct VoiceWakePreferencesTests {
    @Test func sanitizeTriggerWordsTrimsAndDropsEmpty() {
        #expect(VoiceWakePreferences.sanitizeTriggerWords([" supportsquadai ", "", " \nclaude\t"]) == ["supportsquadai", "claude"])
    }
    ...
    @Test func displayStringUsesSanitizedWords() {
        #expect(VoiceWakePreferences.displayString(for: ["", " "]) == "supportsquadai, claude")
    }
