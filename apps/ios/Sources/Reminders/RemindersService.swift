...
final class RemindersService: RemindersServicing {
    func list(params: SupportSquadAIRemindersListParams) async throws -> SupportSquadAIRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[SupportSquadAIReminderPayload], Error>) in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return SupportSquadAIReminderPayload(
        ...
        return SupportSquadAIRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: SupportSquadAIRemindersAddParams) async throws -> SupportSquadAIRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = SupportSquadAIReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return SupportSquadAIRemindersAddPayload(reminder: payload)
    }
