...
final class MotionService: MotionServicing {
    func activities(params: SupportSquadAIMotionActivityParams) async throws -> SupportSquadAIMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[SupportSquadAIMotionActivityEntry], Error>) in
            manager.queryActivityStarting(from: start, to: end, to: OperationQueue()) { activity, error in
                if let error {
                    ...
                    let entries = sliced.map { entry in
                        SupportSquadAIMotionActivityEntry(
        ...
        return SupportSquadAIMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: SupportSquadAIPedometerParams) async throws -> SupportSquadAIPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<SupportSquadAIPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = SupportSquadAIPedometerPayload(
