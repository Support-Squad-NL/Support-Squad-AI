...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> SupportSquadAINetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> SupportSquadAINetworkStatusPayload {
        let status: SupportSquadAINetworkPathStatus = switch path.status {
        ...
        var interfaces: [SupportSquadAINetworkInterfaceType] = []
        if path.usesInterfaceType(.wifi) { interfaces.append(.wifi) }
        if path.usesInterfaceType(.cellular) { interfaces.append(.cellular) }
        if path.usesInterfaceType(.wiredEthernet) { interfaces.append(.wired) }
        if interfaces.isEmpty { interfaces.append(.other) }

        return SupportSquadAINetworkStatusPayload(
            status: status,
    ...
    private static func fallbackPayload() -> SupportSquadAINetworkStatusPayload {
        SupportSquadAINetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
