...
final class NodeAppModel {
    private let deepLinkLogger = Logger(subsystem: "ai.supportsquadai.ios", category: "DeepLink")
    private let pushWakeLogger = Logger(subsystem: "ai.supportsquadai.ios", category: "PushWake")
    private let locationWakeLogger = Logger(subsystem: "ai.supportsquadai.ios", category: "LocationWake")
    private let watchReplyLogger = Logger(subsystem: "ai.supportsquadai.ios", category: "WatchReply")
    enum CameraHUDKind {
        case photo
        case recording
        case success
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = SupportSquadAICanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = SupportSquadAICanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = SupportSquadAICanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = SupportSquadAICanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = SupportSquadAICanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    func requestLocationPermissions(mode: SupportSquadAILocationMode) async -> Bool {
        ...
        self.gatewayHealthMonitor.start(
            check: { [weak self] in
                ...
                do {
                    ...
                    guard let decoded = try? JSONDecoder().decode(SupportSquadAIGatewayHealthOK.self, from: data) else {
                        return false
                    }
    ...
    private func sendAgentRequest(link: AgentDeepLink) async throws {
        ...
        // (Key-based unattended auth is handled on macOS for supportsquadai:// links.)
        ...
        do {
            ...
            case .unknownCommand:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: SupportSquadAINodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            case .handlerUnavailable:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: SupportSquadAINodeError(code: .unavailable, message: "node handler unavailable"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: SupportSquadAINodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(SupportSquadAILocationGetParams.self, from: req.paramsJSON)) ??
            SupportSquadAILocationGetParams()
        ...
        let payload = SupportSquadAILocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case SupportSquadAICanvasCommand.present.rawValue:
            // iOS ignores placement hints; canvas always fills the screen.
            let params = (try? Self.decodeParams(SupportSquadAICanvasPresentParams.self, from: req.paramsJSON)) ??
                SupportSquadAICanvasPresentParams()
        ...
        case SupportSquadAICanvasCommand.hide.rawValue:
            self.screen.showDefaultCanvas()
            return BridgeInvokeResponse(id: req.id, ok: true)
        case SupportSquadAICanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(SupportSquadAICanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case SupportSquadAICanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(SupportSquadAICanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case SupportSquadAICanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(SupportSquadAICanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let command = req.command
        switch command {
        case SupportSquadAICanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              const host = globalThis.supportsquadaiA2UI;
              if (!host) return JSON.stringify({ ok: false, error: "missing supportsquadaiA2UI" });
              return JSON.stringify(host.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case SupportSquadAICanvasA2UICommand.push.rawValue, SupportSquadAICanvasA2UICommand.pushJSONL.rawValue:
            let messages: [SupportSquadAIKit.AnyCodable]
            if command == SupportSquadAICanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(SupportSquadAICanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try SupportSquadAICanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(SupportSquadAICanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try SupportSquadAICanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.supportsquadaiA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case SupportSquadAICameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case SupportSquadAICameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(SupportSquadAICameraSnapParams.self, from: req.paramsJSON)) ??
                SupportSquadAICameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case SupportSquadAICameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(SupportSquadAICameraClipParams.self, from: req.paramsJSON)) ??
                SupportSquadAICameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(SupportSquadAIScreenRecordParams.self, from: req.paramsJSON)) ??
            SupportSquadAIScreenRecordParams()
    ...
    private func handleSystemNotify(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(SupportSquadAISystemNotifyParams.self, from: req.paramsJSON)
        ...
        if title.isEmpty, body.isEmpty {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: SupportSquadAINodeError(code: .invalidRequest, message: "INVALID_REQUEST: empty notification"))
        }
        ...
        guard finalStatus == .authorized || finalStatus == .provisional || finalStatus == .ephemeral else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: SupportSquadAINodeError(code: .unavailable, message: "NOT_AUTHORIZED: notifications"))
        }
        ...
        if case let .failure(error) = addResult {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: SupportSquadAINodeError(code: .unavailable, message: "NOTIFICATION_FAILED: \(error.message)"))
        }
        return BridgeInvokeResponse(id: req.id, ok: true)
    }

    private func handleChatPushInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(SupportSquadAIChatPushParams.self, from: req.paramsJSON)
        let text = params.text.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !text.isEmpty else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: SupportSquadAINodeError(code: .invalidRequest, message: "INVALID_REQUEST: empty chat.push text"))
        }
        ...
        let payload = SupportSquadAIChatPushPayload(messageId: messageId)
        let json = try Self.encodePayload(payload)
        return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
    }
    ...
    private func handleDeviceInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case SupportSquadAIDeviceCommand.status.rawValue:
            let payload = try await self.deviceStatusService.status()
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case SupportSquadAIDeviceCommand.info.rawValue:
    ...
    private func handlePhotosInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(SupportSquadAIPhotosLatestParams.self, from: req.paramsJSON)) ??
            SupportSquadAIPhotosLatestParams()
    ...
    private func handleContactsInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case SupportSquadAIContactsCommand.search.rawValue:
            let params = (try? Self.decodeParams(SupportSquadAIContactsSearchParams.self, from: req.paramsJSON)) ??
                SupportSquadAIContactsSearchParams()
            let payload = try await self.contactsService.search(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case SupportSquadAIContactsCommand.add.rawValue:
            let params = try Self.decodeParams(SupportSquadAIContactsAddParams.self, from: req.paramsJSON)
    ...
    private func handleCalendarInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case SupportSquadAICalendarCommand.events.rawValue:
            let params = (try? Self.decodeParams(SupportSquadAICalendarEventsParams.self, from: req.paramsJSON)) ??
                SupportSquadAICalendarEventsParams()
            let payload = try await self.calendarService.events(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case SupportSquadAICalendarCommand.add.rawValue:
            let params = try Self.decodeParams(SupportSquadAICalendarAddParams.self, from: req.paramsJSON)
    ...
    private func handleRemindersInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case SupportSquadAIRemindersCommand.list.rawValue:
            let params = (try? Self.decodeParams(SupportSquadAIRemindersListParams.self, from: req.paramsJSON)) ??
                SupportSquadAIRemindersListParams()
            let payload = try await self.remindersService.list(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case SupportSquadAIRemindersCommand.add.rawValue:
            let params = try Self.decodeParams(SupportSquadAIRemindersAddParams.self, from: req.paramsJSON)
    ...
    private func handleMotionInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case SupportSquadAIMotionCommand.activity.rawValue:
            let params = (try? Self.decodeParams(SupportSquadAIMotionActivityParams.self, from: req.paramsJSON)) ??
                SupportSquadAIMotionActivityParams()
            let payload = try await self.motionService.activities(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case SupportSquadAIMotionCommand.pedometer.rawValue:
            let params = (try? Self.decodeParams(SupportSquadAIPedometerParams.self, from: req.paramsJSON)) ??
                SupportSquadAIPedometerParams()
    ...
    private func handleTalkInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case SupportSquadAITalkCommand.pttStart.rawValue:
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case SupportSquadAITalkCommand.pttStop.rawValue:
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case SupportSquadAITalkCommand.pttCancel.rawValue:
            let payload = await self.talkMode.cancelPushToTalk()
            self.voiceWake.resumeAfterExternalAudioCapture(wasSuspended: self.pttVoiceWakeSuspended)
            self.pttVoiceWakeSuspended = false
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case SupportSquadAITalkCommand.pttOnce.rawValue:
            self.pttVoiceWakeSuspended = self.voiceWake.suspendForExternalAudioCapture()
            defer {
                self.voiceWake.resumeAfterExternalAudioCapture(wasSuspended: self.pttVoiceWakeSuspended)
                self.pttVoiceWakeSuspended = false
...
private extension NodeAppModel {
    ...
    func buildCapabilityRouter() -> NodeCapabilityRouter {
        ...
        register([SupportSquadAILocationCommand.get.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleLocationInvoke(req)
        }

        register([
            SupportSquadAICanvasCommand.present.rawValue,
            SupportSquadAICanvasCommand.hide.rawValue,
            SupportSquadAICanvasCommand.navigate.rawValue,
            SupportSquadAICanvasCommand.evalJS.rawValue,
            SupportSquadAICanvasCommand.snapshot.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            SupportSquadAICanvasA2UICommand.reset.rawValue,
            SupportSquadAICanvasA2UICommand.push.rawValue,
            SupportSquadAICanvasA2UICommand.pushJSONL.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            SupportSquadAICameraCommand.list.rawValue,
            SupportSquadAICameraCommand.snap.rawValue,
            SupportSquadAICameraCommand.clip.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleCameraInvoke(req)
        }

        register([SupportSquadAIScreenCommand.record.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleScreenRecordInvoke(req)
        }

        register([SupportSquadAISystemCommand.notify.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleSystemNotify(req)
        }

        register([SupportSquadAIChatCommand.push.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleChatPushInvoke(req)
        }

        register([
            SupportSquadAIDeviceCommand.status.rawValue,
            SupportSquadAIDeviceCommand.info.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            SupportSquadAIWatchCommand.status.rawValue,
            SupportSquadAIWatchCommand.notify.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleWatchInvoke(req)
        }

        register([SupportSquadAIPhotosCommand.latest.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handlePhotosInvoke(req)
        }

        register([
            SupportSquadAIContactsCommand.search.rawValue,
            SupportSquadAIContactsCommand.add.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            SupportSquadAICalendarCommand.events.rawValue,
            SupportSquadAICalendarCommand.add.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            SupportSquadAIRemindersCommand.list.rawValue,
            SupportSquadAIRemindersCommand.add.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            SupportSquadAIMotionCommand.activity.rawValue,
            SupportSquadAIMotionCommand.pedometer.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            SupportSquadAITalkCommand.pttStart.rawValue,
            SupportSquadAITalkCommand.pttStop.rawValue,
            SupportSquadAITalkCommand.pttCancel.rawValue,
            SupportSquadAITalkCommand.pttOnce.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
    ...
    func handleWatchInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case SupportSquadAIWatchCommand.status.rawValue:
            let status = await self.watchMessagingService.status()
            let payload = SupportSquadAIWatchStatusPayload(
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case SupportSquadAIWatchCommand.notify.rawValue:
            let params = try Self.decodeParams(SupportSquadAIWatchNotifyParams.self, from: req.paramsJSON)
            let title = params.title.trimmingCharacters(in: .whitespacesAndNewlines)
            let body = params.body.trimmingCharacters(in: .whitespacesAndNewlines)
            if title.isEmpty && body.isEmpty {
                return BridgeInvokeResponse(
            ...
            do {
                ...
                let payload = SupportSquadAIWatchNotifyPayload(
    ...
    func locationMode() -> SupportSquadAILocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return SupportSquadAILocationMode(rawValue: raw) ?? .off
    }
...
private extension NodeAppModel {
                        ...
                        await MainActor.run {
                            ...
                            if let requestId, !requestId.isEmpty {
                                self.gatewayStatusText =
                                    "Pairing required (requestId: \(requestId)). Approve on gateway and return to SupportSquadAI."
                            } else {
                                self.gatewayStatusText = "Pairing required. Approve on gateway and return to SupportSquadAI."
                            }
                        }
    ...
    func legacyClientIdFallback(currentClientId: String, error: Error) -> String? {
        ...
        guard normalizedClientId == "supportsquadai-ios" else { return nil }
...
extension NodeAppModel {
    ...
    func runSharePipelineSelfTest() async {
        ...
        let payload = SharedContentPayload(
            title: "SupportSquadAI Share Self-Test",
            url: URL(string: "https://supportsquadai.ai/share-self-test"),
    ...
    func handleSilentPushWake(_ userInfo: [AnyHashable: Any]) async -> Bool {
        ...
        let pushKind = Self.supportsquadaiPushKind(userInfo)
        self.pushWakeLogger.info(
    ...
    private static func supportsquadaiPushKind(_ userInfo: [AnyHashable: Any]) -> String {
        if let payload = userInfo["supportsquadai"] as? [String: Any],
           let kind = payload["kind"] as? String
        {
            let trimmed = kind.trimmingCharacters(in: .whitespacesAndNewlines)
            if !trimmed.isEmpty { return trimmed }
        }
        if let payload = userInfo["supportsquadai"] as? [AnyHashable: Any],
           let kind = payload["kind"] as? String
        {
            let trimmed = kind.trimmingCharacters(in: .whitespacesAndNewlines)
            if !trimmed.isEmpty { return trimmed }
