...
final class ContactsService: ContactsServicing {
    ...
    func search(params: SupportSquadAIContactsSearchParams) async throws -> SupportSquadAIContactsSearchPayload {
        ...
        return SupportSquadAIContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: SupportSquadAIContactsAddParams) async throws -> SupportSquadAIContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return SupportSquadAIContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return SupportSquadAIContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> SupportSquadAIContactPayload {
        SupportSquadAIContactPayload(
