...
struct IOSGatewayChatTransport: SupportSquadAIChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.supportsquadai", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> SupportSquadAIChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(SupportSquadAIChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> SupportSquadAIChatHistoryPayload {
        ...
        return try JSONDecoder().decode(SupportSquadAIChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [SupportSquadAIChatAttachmentPayload]) async throws -> SupportSquadAIChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [SupportSquadAIChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(SupportSquadAIChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(SupportSquadAIGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: SupportSquadAIGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: SupportSquadAIChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: SupportSquadAIAgentEventPayload.self)
                        {
