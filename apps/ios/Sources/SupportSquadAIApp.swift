...
@MainActor
final class SupportSquadAIAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.supportsquadai.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.supportsquadai.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.supportsquadai.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "supportsquadai.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "supportsquadai.watch.promptId"
    static let sessionKeyKey = "supportsquadai.watch.sessionKey"
    static let actionPrimaryIDKey = "supportsquadai.watch.action.primary.id"
    static let actionPrimaryLabelKey = "supportsquadai.watch.action.primary.label"
    static let actionSecondaryIDKey = "supportsquadai.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "supportsquadai.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "supportsquadai.watch.action.primary"
    static let actionSecondaryIdentifier = "supportsquadai.watch.action.secondary"
    static let categoryPrefix = "supportsquadai.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: SupportSquadAIWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> SupportSquadAIWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return SupportSquadAIWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "SupportSquadAI" : title
        content.body = body
    ...
    private static func categoryActions(
        primaryAction: SupportSquadAIWatchAction,
        secondaryAction: SupportSquadAIWatchAction?) -> [UNNotificationAction]
    {
...
@main
struct SupportSquadAIApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(SupportSquadAIAppDelegate.self) private var appDelegate
...
extension SupportSquadAIApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("SupportSquadAI: installing uncaught exception handler")
