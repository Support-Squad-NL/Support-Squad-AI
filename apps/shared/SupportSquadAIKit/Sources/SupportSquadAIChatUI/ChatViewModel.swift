...
private let chatUILogger = Logger(subsystem: "ai.supportsquadai", category: "SupportSquadAIChatUI")

@MainActor
@Observable
public final class SupportSquadAIChatViewModel {
    public private(set) var messages: [SupportSquadAIChatMessage] = []
    ...
    public var attachments: [SupportSquadAIPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [SupportSquadAIChatPendingToolCall] = []
    public private(set) var sessions: [SupportSquadAIChatSessionEntry] = []
    private let transport: any SupportSquadAIChatTransport
    ...
    private var pendingToolCallsById: [String: SupportSquadAIChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any SupportSquadAIChatTransport) {
    ...
    public var sessionChoices: [SupportSquadAIChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [SupportSquadAIChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: SupportSquadAIPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [SupportSquadAIChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: SupportSquadAIChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: SupportSquadAIChatMessage) -> SupportSquadAIChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> SupportSquadAIChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return SupportSquadAIChatMessageContent(
                type: content.type,
                text: cleaned,
        ...
        return SupportSquadAIChatMessage(
            id: message.id,
            role: message.role,
    ...
    private static func messageIdentityKey(for message: SupportSquadAIChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [SupportSquadAIChatMessage],
        incoming: [SupportSquadAIChatMessage]) -> [SupportSquadAIChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [SupportSquadAIChatMessage]) -> [SupportSquadAIChatMessage] {
        var result: [SupportSquadAIChatMessage] = []
    ...
    private static func dedupeKey(for message: SupportSquadAIChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [SupportSquadAIChatMessageContent] = [
            SupportSquadAIChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> SupportSquadAIChatAttachmentPayload in
            SupportSquadAIChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> SupportSquadAIChatSessionEntry {
        SupportSquadAIChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: SupportSquadAIChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: SupportSquadAIChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: SupportSquadAIAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = SupportSquadAIChatPendingToolCall(
    ...
    private func addImageAttachment(url: URL?, data: Data, fileName: String, mimeType: String) async {
        ...
        self.attachments.append(
            SupportSquadAIPendingAttachment(
                url: url,
                data: data,
                fileName: fileName,
                mimeType: mimeType,
    ...
    private static func previewImage(data: Data) -> SupportSquadAIPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
