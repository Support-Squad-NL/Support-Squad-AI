...
struct ChatMessageBubble: View {
    let message: SupportSquadAIChatMessage
    let style: SupportSquadAIChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? SupportSquadAIChatTheme.userText : SupportSquadAIChatTheme.assistantText
    ...
    private var inlineAttachments: [SupportSquadAIChatMessageContent] {
    ...
    private var toolCalls: [SupportSquadAIChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [SupportSquadAIChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? SupportSquadAIChatTheme.userBubble
        }
        if self.style == .onboarding {
            return SupportSquadAIChatTheme.onboardingAssistantBubble
        }
        return SupportSquadAIChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return SupportSquadAIChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: SupportSquadAIChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? SupportSquadAIChatTheme.userText : SupportSquadAIChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: SupportSquadAIChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(SupportSquadAIChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(SupportSquadAIChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [SupportSquadAIChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: SupportSquadAIChatTheme.assistantText)
            }
        }
    }
}
