...
@MainActor
struct SupportSquadAIChatComposer: View {
    @Bindable var viewModel: SupportSquadAIChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(SupportSquadAIChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(SupportSquadAIChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \SupportSquadAIPendingAttachment.id)
                { (att: SupportSquadAIPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            SupportSquadAIPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if !self.isComposerCompacted {
                Rectangle()
                    .fill(SupportSquadAIChatTheme.divider)
                    .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(SupportSquadAIChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(SupportSquadAIChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(SupportSquadAIChatTheme.subtleCard)
        .clipShape(Capsule())
    }
