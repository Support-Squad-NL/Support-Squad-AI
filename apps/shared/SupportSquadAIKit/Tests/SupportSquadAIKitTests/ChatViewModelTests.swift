...
import Foundation
import Testing
@testable import SupportSquadAIChatUI
...
private final class TestChatTransport: @unchecked Sendable, SupportSquadAIChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [SupportSquadAIChatHistoryPayload]
    private let sessionsResponses: [SupportSquadAIChatSessionsListResponse]

    private let stream: AsyncStream<SupportSquadAIChatTransportEvent>
    private let continuation: AsyncStream<SupportSquadAIChatTransportEvent>.Continuation

    init(
        historyResponses: [SupportSquadAIChatHistoryPayload],
        sessionsResponses: [SupportSquadAIChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<SupportSquadAIChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> SupportSquadAIChatHistoryPayload {
        ...
        return self.historyResponses.last ?? SupportSquadAIChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [SupportSquadAIChatAttachmentPayload]) async throws -> SupportSquadAIChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return SupportSquadAIChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> SupportSquadAIChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? SupportSquadAIChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: SupportSquadAIChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = SupportSquadAIChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = SupportSquadAIChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { SupportSquadAIChatViewModel(sessionKey: "main", transport: transport) }
        ...
        transport.emit(
            .agent(
                SupportSquadAIAgentEventPayload(
                    runId: sessionId,
                    seq: 1,
                    stream: "assistant",
        ...
        transport.emit(
            .chat(
                SupportSquadAIChatEventPayload(
                    runId: runId,
                    sessionKey: "main",
                    state: "final",
                    message: nil,
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = SupportSquadAIChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = SupportSquadAIChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = SupportSquadAIChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { SupportSquadAIChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
