---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for SupportSquadAI deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, SupportSquadAI cron scheduling for periodic checks, or version status checks on a machine running SupportSquadAI (laptop, workstation, Pi, VPS).
---

# SupportSquadAI Host Hardening

## Overview

Assess and harden the host running SupportSquadAI, then align it to a user-defined risk tolerance without breaking access. Use SupportSquadAI security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim SupportSquadAI changes the host firewall, SSH, or OS updates; it does not.
...
5. SupportSquadAI gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if SupportSquadAI can access sensitive data.
...
### 2) Run SupportSquadAI security audits (read-only)

As part of the default read-only checks, run `supportsquadai security audit --deep`. Only offer alternatives if the user requests them:

1. `supportsquadai security audit` (faster, non-probing)
2. `supportsquadai security audit --json` (structured output)

Offer to apply SupportSquadAI safe defaults (numbered):

1. `supportsquadai security audit --fix`

Be explicit that `--fix` only tightens SupportSquadAI defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check SupportSquadAI version/update status (read-only)

As part of the default read-only checks, run `supportsquadai update status`.
...
- Credential hygiene notes (location of SupportSquadAI creds, prefer disk encryption)
...
- SupportSquadAI security audit (re-run)
...
After SupportSquadAI install or first hardening pass, run at least one baseline audit and version check:

- `supportsquadai security audit`
- `supportsquadai security audit --deep`
- `supportsquadai update status`

Ongoing monitoring is recommended. Use the SupportSquadAI cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `supportsquadai cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `supportsquadai update status`
...
Before creating, `supportsquadai cron list` and match on exact `name`. If found, `supportsquadai cron edit <id> ...`.
If not found, `supportsquadai cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `supportsquadai update status` (preferred for source checkouts and channels)
2. `npm view supportsquadai version` (published npm version)

## SupportSquadAI command accuracy

Use only supported commands and flags:

- `supportsquadai security audit [--deep] [--fix] [--json]`
- `supportsquadai status` / `supportsquadai status --deep`
- `supportsquadai health --json`
- `supportsquadai update status`
- `supportsquadai cron add|list|runs|run`

Do not invent CLI flags or imply SupportSquadAI enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by SupportSquadAI compaction:
