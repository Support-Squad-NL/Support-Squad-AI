...
metadata: { "supportsquadai": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == SupportSquadAI `sessions_spawn`
- **File I/O** == SupportSquadAI `read`/`write`
- **Remote fetch** == SupportSquadAI `web_fetch` (or `exec` with curl when POST is required)
