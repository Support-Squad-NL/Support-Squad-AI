...
import type { SupportSquadAIPluginApi, SupportSquadAIPluginService } from "supportsquadai/plugin-sdk";
...
function normalizeDenyList(cfg: SupportSquadAIPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: SupportSquadAIPluginApi["config"]): string[] {
...
): SupportSquadAIPluginApi["config"] {
  return {
    ...cfg,
    gateway: {
      ...cfg.gateway,
...
export default function register(api: SupportSquadAIPluginApi) {
  ...
  const timerService: SupportSquadAIPluginService = {
