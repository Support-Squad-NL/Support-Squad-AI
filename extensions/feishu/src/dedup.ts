...
import { createDedupeCache, createPersistentDedupe } from "supportsquadai/plugin-sdk";
...
function resolveStateDirFromEnv(env: NodeJS.ProcessEnv = process.env): string {
  const stateOverride = env.SUPPORTSQUADAI_STATE_DIR?.trim() || env.CLAWDBOT_STATE_DIR?.trim();
  if (stateOverride) {
    return stateOverride;
  }
  if (env.VITEST || env.NODE_ENV === "test") {
    return path.join(os.tmpdir(), ["supportsquadai-vitest", String(process.pid)].join("-"));
  }
  return path.join(os.homedir(), ".supportsquadai");
}
