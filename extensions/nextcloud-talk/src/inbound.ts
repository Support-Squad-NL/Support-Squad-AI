...
export async function handleNextcloudTalkInbound(params: {
  ...
  const defaultGroupPolicy = resolveDefaultGroupPolicy(config as SupportSquadAIConfig);
  ...
  const allowTextCommands = core.channel.commands.shouldHandleTextCommands({
    cfg: config as SupportSquadAIConfig,
    surface: CHANNEL_ID,
  });
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as SupportSquadAIConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as SupportSquadAIConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as SupportSquadAIConfig);
