...
import type { SupportSquadAIPluginApi, SupportSquadAIPluginToolContext } from "../../../src/plugins/types.js";
import {
...
function fakeApi(overrides: Partial<SupportSquadAIPluginApi> = {}): SupportSquadAIPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<SupportSquadAIPluginToolContext> = {}): SupportSquadAIPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-lobster-plugin-"));
  });
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: SupportSquadAIPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
