...
SupportSquadAI extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
supportsquadai channels login --channel zalouser
...
```bash
supportsquadai message send --channel zalouser --target <threadId> --message "Hello from SupportSquadAI!"
```
...
```bash
supportsquadai channels login --channel zalouser              # Login via QR
supportsquadai channels login --channel zalouser --account work
supportsquadai channels status --probe
supportsquadai channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
supportsquadai directory self --channel zalouser
supportsquadai directory peers list --channel zalouser --query "name"
supportsquadai directory groups list --channel zalouser --query "work"
supportsquadai directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
supportsquadai message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
supportsquadai message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `supportsquadai channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
supportsquadai channels login --channel zalouser --account work
supportsquadai message send --channel zalouser --account work --target <id> --message "Hello"
