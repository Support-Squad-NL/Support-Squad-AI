...
import type { ReplyPayload, SupportSquadAIConfig } from "supportsquadai/plugin-sdk";
import { createReplyPrefixOptions } from "supportsquadai/plugin-sdk";
...
export type TwitchMonitorOptions = {
  ...
  accountId: string;
  config: unknown; // SupportSquadAIConfig
...
async function processTwitchMessage(params: {
  ...
  const cfg = config as SupportSquadAIConfig;
