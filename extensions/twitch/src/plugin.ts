/**
 * Twitch channel plugin for SupportSquadAI.
 *
...
import { buildChannelConfigSchema } from "supportsquadai/plugin-sdk";
 ...
 * for SupportSquadAI. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: SupportSquadAIConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: SupportSquadAIConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: SupportSquadAIConfig): boolean => {
