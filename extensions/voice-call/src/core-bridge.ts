...
function resolveSupportSquadAIRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.SUPPORTSQUADAI_ROOT?.trim();
  if (override) {
  ...
  for (const start of candidates) {
    for (const name of ["supportsquadai"]) {
  ...
  throw new Error("Unable to resolve core root. Set SUPPORTSQUADAI_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveSupportSquadAIRoot(), "dist", "extensionAPI.js");
