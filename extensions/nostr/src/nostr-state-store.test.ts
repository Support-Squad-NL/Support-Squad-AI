...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.SUPPORTSQUADAI_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-nostr-"));
  process.env.SUPPORTSQUADAI_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const stateEnv = env ?? process.env;
        const override = stateEnv.SUPPORTSQUADAI_STATE_DIR?.trim() || stateEnv.CLAWDBOT_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        const resolveHome = homedir ?? os.homedir;
        return path.join(resolveHome(), ".supportsquadai");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.SUPPORTSQUADAI_STATE_DIR = previous;
    }
