import type { RuntimeEnv, WizardPrompter } from "supportsquadai/plugin-sdk";
...
describe("irc onboarding", () => {
  it("configures host and nick via onboarding prompts", async () => {
    ...
    expect(result.cfg.channels?.irc?.nick).toBe("supportsquadai-bot");
    expect(result.cfg.channels?.irc?.tls).toBe(true);
    expect(result.cfg.channels?.irc?.channels).toEqual(["#supportsquadai", "#ops"]);
    expect(result.cfg.channels?.irc?.groupPolicy).toBe("allowlist");
    expect(Object.keys(result.cfg.channels?.irc?.groups ?? {})).toEqual(["#supportsquadai", "#ops"]);
  });
