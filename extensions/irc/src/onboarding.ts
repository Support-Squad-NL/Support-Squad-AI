  ...
  configure: async ({
    ...
    if (useEnv) {
      ...
      const username = String(
        await prompter.text({
          ...
          initialValue: resolved.config.username || nick || "supportsquadai",
          validate: (value) => (String(value ?? "").trim() ? undefined : "Required"),
        }),
      ).trim();
      ...
      const realname = String(
        await prompter.text({
          ...
          initialValue: resolved.config.realname || "SupportSquadAI",
          validate: (value) => (String(value ?? "").trim() ? undefined : "Required"),
        }),
      ).trim();
      ...
      const channelsRaw = await prompter.text({
        ...
        placeholder: "#supportsquadai, #ops",
    ...
    await prompter.note(
      [
        ...
        "Command: supportsquadai channels status --probe",
        `Docs: ${formatDocsLink("/channels/irc", "channels/irc")}`,
      ].join("\n"),
      "IRC next steps",
    );
