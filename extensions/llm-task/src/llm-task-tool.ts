...
import Ajv from "ajv";
// NOTE: This extension is intended to be bundled with SupportSquadAI.
// When running from source (tests/dev), SupportSquadAI internals live under src/.
// When running from a built install, internals live under dist/ (no src/ tree).
// So we resolve internal imports dynamically with src-first, dist-fallback.
import type { SupportSquadAIPluginApi } from "../../../src/plugins/types.js";
...
export function createLlmTaskTool(api: SupportSquadAIPluginApi) {
  return {
    name: "llm-task",
    label: "LLM Task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via supportsquadai.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "supportsquadai-llm-task-"));
