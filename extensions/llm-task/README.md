...
`supportsquadai.invoke --each`) without adding new SupportSquadAI code per workflow.
...
This extension depends on SupportSquadAI internal modules (the embedded agent runner).
It is intended to ship as a **bundled** SupportSquadAI extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.supportsquadai/extensions` as a standalone plugin directory.
