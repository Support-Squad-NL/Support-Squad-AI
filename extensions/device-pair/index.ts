...
import { approveDevicePairing, listDevicePairing } from "supportsquadai/plugin-sdk";
...
function resolveGatewayPort(cfg: SupportSquadAIPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.SUPPORTSQUADAI_GATEWAY_PORT?.trim()) ??
...
function resolveScheme(
  cfg: SupportSquadAIPluginApi["config"],
...
function resolveAuth(cfg: SupportSquadAIPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: SupportSquadAIPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the SupportSquadAI iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the SupportSquadAI iOS app:",
            "",
            "```",
            qrAscii,
            "```",
