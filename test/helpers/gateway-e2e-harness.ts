...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `supportsquadai-e2e-${name}-`));
  const configDir = path.join(homeDir, ".supportsquadai");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          SUPPORTSQUADAI_SKIP_PROVIDERS: "1",
          SUPPORTSQUADAI_SKIP_GMAIL_WATCHER: "1",
          SUPPORTSQUADAI_SKIP_CRON: "1",
          SUPPORTSQUADAI_SKIP_BROWSER_CONTROL_SERVER: "1",
          SUPPORTSQUADAI_SKIP_CANVAS_HOST: "1",
          SUPPORTSQUADAI_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
