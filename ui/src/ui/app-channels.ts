...
export async function handleWhatsAppStart(host: SupportSquadAIApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: SupportSquadAIApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: SupportSquadAIApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: SupportSquadAIApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: SupportSquadAIApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: SupportSquadAIApp): string {
...
function resolveGatewayHttpAuthHeader(host: SupportSquadAIApp): string | null {
...
function buildGatewayHttpHeaders(host: SupportSquadAIApp): Record<string, string> {
  const authorization = resolveGatewayHttpAuthHeader(host);
  return authorization ? { Authorization: authorization } : {};
}

export function handleNostrProfileEdit(
  host: SupportSquadAIApp,
  accountId: string,
...
export function handleNostrProfileCancel(host: SupportSquadAIApp) {
...
export function handleNostrProfileToggleAdvanced(host: SupportSquadAIApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: SupportSquadAIApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: SupportSquadAIApp) {
  const state = host.nostrProfileFormState;
