...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/supportsquadai/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/supportsquadai");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/supportsquadai/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__SUPPORTSQUADAI_CONTROL_UI_BASE_PATH__ = "/supportsquadai";
    const app = mountApp("/supportsquadai/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/supportsquadai");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/supportsquadai/sessions");
  });
  ...
  it("hydrates token from URL params even when settings already set", async () => {
    localStorage.setItem(
      "supportsquadai.control.settings.v1",
