...
IMAGE_NAME="supportsquadai-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="supportsquadai-net-e2e-$$"
GW_NAME="supportsquadai-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "SUPPORTSQUADAI_GATEWAY_TOKEN=$TOKEN" \
	  -e "SUPPORTSQUADAI_SKIP_CHANNELS=1" \
	  -e "SUPPORTSQUADAI_SKIP_GMAIL_WATCHER=1" \
	  -e "SUPPORTSQUADAI_SKIP_CRON=1" \
	  -e "SUPPORTSQUADAI_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
