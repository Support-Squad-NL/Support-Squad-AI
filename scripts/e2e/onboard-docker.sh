...
IMAGE_NAME="supportsquadai-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/supportsquadai-bin:$PATH"
  mkdir -p /tmp/supportsquadai-bin
  cat > /tmp/supportsquadai-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/supportsquadai-bin/trash
  ...
  wait_for_log() {
    ...
    while true; do
	  ...
	  start_gateway() {
	    node "$SUPPORTSQUADAI_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured > /tmp/gateway-e2e.log 2>&1 &
	    GATEWAY_PID="$!"
	  }
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/supportsquadai-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/supportsquadai-onboard-${case_name}.log"
  ...
  run_wizard() {
    ...
    local validate_fn="${4:-}"
	    ...
	    run_wizard_cmd "$case_name" "$home_dir" "node \"$SUPPORTSQUADAI_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
	  }

  make_home() {
    mktemp -d "/tmp/supportsquadai-e2e-$1.XXXXXX"
  }
  ...
  send_skills_flow() {
    ...
    send "" 2.0
	  ...
	  run_case_local_basic() {
	    ...
	    export HOME="$home_dir"
	    mkdir -p "$HOME"
	    node "$SUPPORTSQUADAI_ENTRY" onboard \
	      --non-interactive \
	      --accept-risk \
      --flow quickstart \
      --mode local \
    ...
    workspace_dir="$HOME/.supportsquadai/workspace"
    config_path="$HOME/.supportsquadai/supportsquadai.json"
    sessions_dir="$HOME/.supportsquadai/agents/main/sessions"
...
NODE
  ...
  run_case_remote_non_interactive() {
    ...
    export HOME="$home_dir"
	    ...
	    node "$SUPPORTSQUADAI_ENTRY" onboard --non-interactive --accept-risk \
	      --mode remote \
...
NODE
  ...
  run_case_reset() {
    ...
    # Seed a remote config to exercise reset path.
	    cat > "$HOME/.supportsquadai/supportsquadai.json" <<'"'"'JSON'"'"'
{
...
NODE
  ...
  run_case_channels() {
	    ...
	    run_wizard_cmd channels "$home_dir" "node \"$SUPPORTSQUADAI_ENTRY\" configure --section channels" send_channels_flow
...
JSON
	    ...
	    run_wizard_cmd skills "$home_dir" "node \"$SUPPORTSQUADAI_ENTRY\" configure --section skills" send_skills_flow
