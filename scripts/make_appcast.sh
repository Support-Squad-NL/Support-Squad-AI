...
ZIP=${1:?"Usage: $0 SupportSquadAI-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/supportsquadai/supportsquadai/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^SupportSquadAI-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/supportsquadai/supportsquadai/releases/download/v${VERSION}/"}
