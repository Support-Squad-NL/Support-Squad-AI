 ...
 * `supportsquadai update`. Use it to verify the completion installation flow
...
\`supportsquadai update\`, \`supportsquadai doctor\`, and \`supportsquadai onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# SupportSquadAI Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`supportsquadai completion --install\` later to enable.`));
    return;
  }
