...
BASE_IMAGE="${BASE_IMAGE:-supportsquadai-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-supportsquadai-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=supportsquadai.sandbox=1)
NOTE
