...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/supportsquadai/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "SupportSquadAI Re-Auth" -c "After getting key, run: ssh $SERVER '~/supportsquadai/scripts/mobile-reauth.sh'" --id supportsquadai-auth
        ;;
    *)
