...
INSTALL_URL="${SUPPORTSQUADAI_INSTALL_URL:-https://supportsquadai.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${SUPPORTSQUADAI_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${SUPPORTSQUADAI_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="supportsquadai"
PACKAGE_NAME="${SUPPORTSQUADAI_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (SUPPORTSQUADAI_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${SUPPORTSQUADAI_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${SUPPORTSQUADAI_INSTALL_LATEST_OUT:-}"
fi
if [[ -n "$CMD_PATH" ]]; then
