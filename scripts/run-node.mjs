...
const shouldBuild = (deps) => {
  if (deps.env.SUPPORTSQUADAI_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.SUPPORTSQUADAI_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[supportsquadai] ${message}\n`);
};

const runSupportSquadAI = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["supportsquadai.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
...
export async function runNodeMain(params = {}) {
  ...
  if (!shouldBuild(deps)) {
    return await runSupportSquadAI(deps);
  }
