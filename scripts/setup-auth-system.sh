#!/bin/bash
# Setup SupportSquadAI Auth Management System
...
echo "=== SupportSquadAI Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'supportsquadai-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  SupportSquadAI message: Send warning via SupportSquadAI itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/supportsquadai-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/supportsquadai-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/supportsquadai-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now supportsquadai-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status supportsquadai-auth-monitor.timer"
echo ""
echo "Quick commands:"
