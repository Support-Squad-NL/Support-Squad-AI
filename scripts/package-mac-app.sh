...
# Build and bundle SupportSquadAI into a minimal .app we can open.
# Outputs to dist/SupportSquadAI.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/SupportSquadAI.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="SupportSquadAI"
BUNDLE_ID="${BUNDLE_ID:-ai.supportsquadai.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/supportsquadai/supportsquadai/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/SupportSquadAI/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :SupportSquadAIBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :SupportSquadAIGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/SupportSquadAI"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/SupportSquadAI"
fi
chmod +x "$APP_ROOT/Contents/MacOS/SupportSquadAI"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/SupportSquadAI" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/SupportSquadAI/Resources/SupportSquadAI.icns" "$APP_ROOT/Contents/Resources/SupportSquadAI.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/SupportSquadAI/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying SupportSquadAIKit resources"
SUPPORTSQUADAIKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/SupportSquadAIKit_SupportSquadAIKit.bundle"
if [ -d "$SUPPORTSQUADAIKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/SupportSquadAIKit_SupportSquadAIKit.bundle"
  cp -R "$SUPPORTSQUADAIKIT_BUNDLE" "$APP_ROOT/Contents/Resources/SupportSquadAIKit_SupportSquadAIKit.bundle"
else
  echo "WARN: SupportSquadAIKit resource bundle not found at $SUPPORTSQUADAIKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running SupportSquadAI"
killall -q SupportSquadAI 2>/dev/null || true
