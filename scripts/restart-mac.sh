#!/usr/bin/env bash
# Reset SupportSquadAI like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${SUPPORTSQUADAI_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="SupportSquadAI.app/Contents/MacOS/SupportSquadAI"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/SupportSquadAI"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/SupportSquadAI"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/SupportSquadAI"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.supportsquadai.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/supportsquadai-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${SUPPORTSQUADAI_RESTART_LOG:-/tmp/supportsquadai-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${SUPPORTSQUADAI_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.supportsquadai/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  SUPPORTSQUADAI_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node supportsquadai.mjs daemon install --force --runtime node"
      log "  node supportsquadai.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.supportsquadai/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.supportsquadai.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing SupportSquadAI instances"
kill_all_supportsquadai
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product SupportSquadAI"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/SupportSquadAI.app" ]]; then
    APP_BUNDLE="/Applications/SupportSquadAI.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/SupportSquadAI.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/SupportSquadAI.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/SupportSquadAI.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set SUPPORTSQUADAI_APP_BUNDLE to your installed SupportSquadAI.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node supportsquadai.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node supportsquadai.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".supportsquadai", "supportsquadai.json"), "utf8");
...
if pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1; then
  log "OK: SupportSquadAI is running."
else
