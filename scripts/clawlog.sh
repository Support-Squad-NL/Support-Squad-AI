...
show_usage() {
    cat << EOF
clawlog - SupportSquadAI Logging Utility

USAGE:
    clawlog [OPTIONS]

DESCRIPTION:
    View SupportSquadAI logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    SupportSquadAI logs flow through the macOS unified log (subsystem: ai.supportsquadai).
