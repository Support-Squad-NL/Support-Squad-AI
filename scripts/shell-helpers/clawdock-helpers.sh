#!/usr/bin/env bash
# ClawDock - Docker helpers for SupportSquadAI
# Inspired by Simon Willison's "Running SupportSquadAI in Docker"
# https://til.simonwillison.net/llms/supportsquadai-docker
#
# Installation:
...
# Common paths to check for SupportSquadAI
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/supportsquadai"
  "${HOME}/workspace/supportsquadai"
  "${HOME}/projects/supportsquadai"
  "${HOME}/dev/supportsquadai"
  "${HOME}/code/supportsquadai"
  "${HOME}/src/supportsquadai"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found SupportSquadAI at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/supportsquadai"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ SupportSquadAI not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/supportsquadai/supportsquadai.git ~/supportsquadai"
    echo "  cd ~/supportsquadai && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^SUPPORTSQUADAI_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d supportsquadai-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart supportsquadai-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f supportsquadai-gateway
}
...
clawdock-workspace() {
  cd ~/.supportsquadai/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec supportsquadai-gateway \
    bash -c 'echo "alias supportsquadai=\"./supportsquadai.mjs\"" > /tmp/.bashrc_supportsquadai && bash --rcfile /tmp/.bashrc_supportsquadai'
}
...
clawdock-cli() {
  _clawdock_compose run --rm supportsquadai-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build supportsquadai-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "SUPPORTSQUADAI_GATEWAY_TOKEN=$token" supportsquadai-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" supportsquadai-gateway \
    bash -c './supportsquadai.mjs config set gateway.remote.token "$TOKEN" && ./supportsquadai.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec supportsquadai-gateway \
    bash -c "./supportsquadai.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart supportsquadai-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm supportsquadai-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec supportsquadai-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'supportsquadai config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for SupportSquadAI${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (supportsquadai alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to supportsquadai project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.supportsquadai)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'supportsquadai channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'supportsquadai status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.supportsquadai.ai${_CLR_RESET}"
  echo ""
}
