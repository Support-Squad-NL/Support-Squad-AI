...
SupportSquadAI integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- SupportSquadAI owns lifecycle when `channels.signal.autoStart=true`.
...
- SupportSquadAI spawns `imsg rpc` as a child process (legacy iMessage integration).
