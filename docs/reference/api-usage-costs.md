...
SupportSquadAI features that can generate provider usage or paid API calls.
...
- `supportsquadai status --usage` and `supportsquadai channels list` show provider **usage windows**
...
SupportSquadAI can pick up credentials from:
...
- `supportsquadai status --usage`
- `supportsquadai models status --json`
...
`supportsquadai models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
