...
- Publish as `supportsquadai/plugin-sdk` (or export from core under `supportsquadai/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `SupportSquadAIPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: SupportSquadAIConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: SupportSquadAIConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: SupportSquadAIConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: SupportSquadAIConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: SupportSquadAIConfig): string;
  };
};
```
...
- Introduce `supportsquadai/plugin-sdk`.
- Add `api.runtime` to `SupportSquadAIPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `supportsquadaiRuntime: ">=2026.2.0"`).
