---
summary: "CLI reference for `supportsquadai security` (audit and fix common security footguns)"
read_when:
...
It also warns when existing sandbox browser Docker containers have missing/stale hash labels (for example pre-migration containers missing `supportsquadai.browserConfigEpoch`) and recommends `supportsquadai sandbox recreate --browser --all`.
...
```bash
supportsquadai security audit --json | jq '.summary'
supportsquadai security audit --deep --json | jq '.findings[] | select(.severity=="critical") | .checkId'
```

If `--fix` and `--json` are combined, output includes both fix actions and final report:

```bash
supportsquadai security audit --fix --json | jq '{fix: .fix.ok, summary: .report.summary}'
```
