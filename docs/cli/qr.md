---
summary: "CLI reference for `supportsquadai qr` (generate iOS pairing QR + setup code)"
read_when:
...
```bash
supportsquadai qr
supportsquadai qr --setup-code-only
supportsquadai qr --json
supportsquadai qr --remote
supportsquadai qr --url wss://gateway.example/ws --token '<token>'
```
