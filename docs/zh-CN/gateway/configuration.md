...
read_when:
  - 添加或修改配置字段时
summary: ~/.supportsquadai/supportsquadai.json 的所有配置选项及示例
title: 配置
x-i18n:
...
SupportSquadAI 从 `~/.supportsquadai/supportsquadai.json` 读取可选的 **JSON5** 配置（支持注释和尾逗号）。

如果文件不存在，SupportSquadAI 使用安全的默认值（内置 Pi 智能体 + 按发送者分会话 + 工作区 `~/.supportsquadai/workspace`）。通常只在以下情况需要配置：
...
SupportSquadAI 只接受完全匹配 schema 的配置。
未知键、类型错误或无效值会导致 Gateway 网关 **拒绝启动**以确保安全。

验证失败时：

- Gateway 网关不会启动。
- 只允许诊断命令（例如：`supportsquadai doctor`、`supportsquadai logs`、`supportsquadai health`、`supportsquadai status`、`supportsquadai service`、`supportsquadai help`）。
- 运行 `supportsquadai doctor` 查看具体问题。
- 运行 `supportsquadai doctor --fix`（或 `--yes`）应用迁移/修复。
...
请使用 `config.patch` 或 `supportsquadai config set`。请备份 `~/.supportsquadai/supportsquadai.json`。
...
```bash
supportsquadai gateway call config.get --params '{}' # capture payload.hash
supportsquadai gateway call config.apply --params '{
...
```bash
supportsquadai gateway call config.get --params '{}' # capture payload.hash
supportsquadai gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.supportsquadai/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@supportsquadai", "reisponde"] },
      },
    ],
  },
  channels: {
...
```json5
// ~/.supportsquadai/supportsquadai.json
{
  gateway: { port: 18789 },
...
```json5
// ~/.supportsquadai/agents.json5
{
  defaults: { sandbox: { mode: "all", scope: "session" } },
  list: [{ id: "main", workspace: "~/.supportsquadai/workspace" }],
}
```
...
{ "$include": "/etc/supportsquadai/base.json5" } // 绝对路径
...
```json5
// ~/.supportsquadai/clients/mueller/agents.json5
[
...
```json5
// ~/.supportsquadai/clients/mueller/broadcast.json5
{
...
SupportSquadAI 从父进程（shell、launchd/systemd、CI 等）读取环境变量。

此外，它还会加载：

- 当前工作目录中的 `.env`（如果存在）
- `~/.supportsquadai/.env`（即 `$SUPPORTSQUADAI_STATE_DIR/.env`）作为全局回退 `.env`
...
可选便利功能：如果启用且预期键均未设置，SupportSquadAI 会运行你的登录 shell 并仅导入缺失的预期键（不会覆盖）。
...
- `SUPPORTSQUADAI_LOAD_SHELL_ENV=1`
- `SUPPORTSQUADAI_SHELL_ENV_TIMEOUT_MS=15000`
...
```json5
{
  ...
  gateway: {
    auth: {
      token: "${SUPPORTSQUADAI_GATEWAY_TOKEN}",
    },
  },
}
```
...
SupportSquadAI 在以下位置存储**每个智能体的**认证配置文件（OAuth + API 密钥）：

- `<agentDir>/auth-profiles.json`（默认：`~/.supportsquadai/agents/<agentId>/agent/auth-profiles.json`）

另请参阅：[/concepts/oauth](/concepts/oauth)

旧版 OAuth 导入：

- `~/.supportsquadai/credentials/oauth.json`（或 `$SUPPORTSQUADAI_STATE_DIR/credentials/oauth.json`）
...
- `~/.supportsquadai/agent/*`（由 `supportsquadai doctor` 迁移到 `~/.supportsquadai/agents/<defaultAgentId>/agent/*`）

覆盖：

- OAuth 目录（仅旧版导入）：`SUPPORTSQUADAI_OAUTH_DIR`
- 智能体目录（默认智能体根目录覆盖）：`SUPPORTSQUADAI_AGENT_DIR`（推荐）、`PI_CODING_AGENT_DIR`（旧版）

首次使用时，SupportSquadAI 会将 `oauth.json` 条目导入到 `auth-profiles.json` 中。
...
如果设置了，SupportSquadAI 会推导默认值（仅在你未明确设置时）：
...
- 默认日志文件：`/tmp/supportsquadai/supportsquadai-YYYY-MM-DD.log`
- 如需稳定路径，将 `logging.file` 设为 `/tmp/supportsquadai/supportsquadai.log`。
- 控制台输出可通过以下方式单独调整：
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/supportsquadai/supportsquadai.log",
    consoleLevel: "info",
    consoleStyle: "pretty",
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // 可选覆盖。默认：~/.supportsquadai/credentials/whatsapp/biz
          // authDir: "~/.supportsquadai/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- 旧版单账号 Baileys 认证目录由 `supportsquadai doctor` 迁移到 `whatsapp/default`。
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@supportsquadai", "supportsquadai"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          // 仅这些文本模式会触发响应
          mentionPatterns: ["reisponde", "@supportsquadai"],
        },
      },
    ],
  },
...
- `agents.list[]`：每智能体覆盖。
  ...
  - `name`：智能体的显示名称。
  - `workspace`：默认 `~/.supportsquadai/workspace-<agentId>`（对于 `main`，回退到 `agents.defaults.workspace`）。
  - `agentDir`：默认 `~/.supportsquadai/agents/<agentId>/agent`。
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.supportsquadai/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.supportsquadai/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.supportsquadai/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.supportsquadai/workspace-home" },
      { id: "work", workspace: "~/.supportsquadai/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` 启用 `/config`（读写 `supportsquadai.json`）。
...
SupportSquadAI 仅在存在 `channels.telegram` 配置段时启动 Telegram。机器人 token 从 `channels.telegram.botToken`（或 `channels.telegram.tokenFile`）解析，`TELEGRAM_BOT_TOKEN` 作为默认账号的回退。
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["supportsquadai-dm"], // 可选群组私聊白名单
      },
      guilds: {
        "123456789012345678": {
          // 服务器 id（推荐）或 slug
          slug: "friends-of-supportsquadai",
...
SupportSquadAI 仅在存在 `channels.discord` 配置段时启动 Discord。token 从 `channels.discord.token` 解析，`DISCORD_BOT_TOKEN` 作为默认账号的回退（除非 `channels.discord.enabled` 为 `false`）。在为 cron/CLI 命令指定投递目标时，使用 `user:<id>`（私聊）或 `channel:<id>`（服务器频道）；裸数字 ID 有歧义会被拒绝。
...
SupportSquadAI 在提供商启用且两个 token 都已设置时启动 Slack（通过配置或 `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`）。在为 cron/CLI 命令指定投递目标时使用 `user:<id>`（私聊）或 `channel:<id>`。
...
请先安装：`supportsquadai plugins install @supportsquadai/mattermost`（或从 git checkout 使用 `./extensions/mattermost`）。
...
SupportSquadAI 在账号已配置（机器人 token + 基础 URL）且已启用时启动 Mattermost。token + 基础 URL 从 `channels.mattermost.botToken` + `channels.mattermost.baseUrl` 或默认账号的 `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` 解析（除非 `channels.mattermost.enabled` 为 `false`）。
...
SupportSquadAI 会生成 `imsg rpc`（通过 stdio 的 JSON-RPC）。无需守护进程或端口。
...
在系统提示的 Runtime 行中显示的可选仓库根目录。如果未设置，SupportSquadAI 会从工作区（和当前工作目录）向上查找 `.git` 目录进行检测。路径必须存在才能使用。

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/supportsquadai" } },
}
```
...
当文件超过此限制时，SupportSquadAI 会记录警告并注入带标记的头尾截断内容。
...
设置用户时区用于**系统提示上下文**（不用于消息信封中的时间戳）。如果未设置，SupportSquadAI 在运行时使用主机时区。
...
如果未设置 `messages.responsePrefix`，默认不应用前缀。WhatsApp 自聊天回复是例外：它们在设置时默认为 `[{identity.name}]`，否则为 `[supportsquadai]`，以保持同一手机上的对话可读性。
...
WhatsApp 入站前缀通过 `channels.whatsapp.messagePrefix` 配置（已弃用：`messages.messagePrefix`）。默认保持**不变**：当 `channels.whatsapp.allowFrom` 为空时为 `"[supportsquadai]"`，否则为 `""`（无前缀）。使用 `"[supportsquadai]"` 时，如果路由的智能体设置了 `identity.name`，SupportSquadAI 会改用 `[{identity.name}]`。
...
为出站回复启用文字转语音。开启后，SupportSquadAI 使用 ElevenLabs 或 OpenAI 生成音频并附加到回复中。Telegram 使用 Opus 语音消息；其他渠道发送 MP3 音频。
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.supportsquadai/settings/tts.json",
      elevenlabs: {
...
SupportSquadAI 还内置了一些别名快捷方式。默认值仅在模型已存在于 `agents.defaults.models` 中时才应用：
...
如果省略提供商，SupportSquadAI 目前假定 `anthropic` 作为临时弃用回退。
...
- `tools.web.search.apiKey`（推荐：通过 `supportsquadai configure --section web` 设置，或使用 `BRAVE_API_KEY` 环境变量）
...
- `group:nodes`：`nodes`
- `group:supportsquadai`：所有内置 SupportSquadAI 工具（不包含提供商插件）
...
- 智能体工作区访问：`workspaceAccess: "none"`（默认）
  - `"none"`：在 `~/.supportsquadai/sandboxes` 下使用每范围的沙箱工作区
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.supportsquadai/sandboxes",
        docker: {
          image: "supportsquadai-sandbox:bookworm-slim",
          containerPrefix: "supportsquadai-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "supportsquadai-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "supportsquadai-sandbox-browser:bookworm-slim",
          containerPrefix: "supportsquadai-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
SupportSquadAI 使用 **pi-coding-agent** 模型目录。你可以通过编写
`~/.supportsquadai/agents/<agentId>/agent/models.json` 或在 SupportSquadAI 配置中的 `models.providers` 下定义相同的 schema 来添加自定义提供商（LiteLLM、本地 OpenAI 兼容服务器、Anthropic 代理等）。
按提供商的概述 + 示例：[/concepts/model-providers](/concepts/model-providers)。

当存在 `models.providers` 时，SupportSquadAI 在启动时将 `models.json` 写入/合并到
`~/.supportsquadai/agents/<agentId>/agent/`：
...
OpenCode Zen 是一个具有每模型端点的多模型网关。SupportSquadAI 使用
...
- 快捷方式：`supportsquadai onboard --auth-choice opencode-zen`。
...
快捷方式：`supportsquadai onboard --auth-choice zai-api-key`。
...
- 在环境中设置 `MOONSHOT_API_KEY` 或使用 `supportsquadai onboard --auth-choice moonshot-api-key`。
...
- 在环境中设置 `KIMI_API_KEY` 或使用 `supportsquadai onboard --auth-choice kimi-code-api-key`。
...
- 设置 `SYNTHETIC_API_KEY` 或使用 `supportsquadai onboard --auth-choice synthetic-api-key`。
...
- 设置 `MINIMAX_API_KEY` 环境变量或使用 `supportsquadai onboard --auth-choice minimax-api`。
...
- 如果你希望 `models.json` 存储在其他位置，请使用 `SUPPORTSQUADAI_AGENT_DIR`（或 `PI_CODING_AGENT_DIR`）覆盖智能体配置根目录（默认：`~/.supportsquadai/agents/main/agent`）。
...
```json5
{
  session: {
    ...
    // 默认已按智能体存储在 ~/.supportsquadai/agents/<agentId>/sessions/sessions.json
    // 你可以使用 {agentId} 模板进行覆盖：
    store: "~/.supportsquadai/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`：`dm`、`group` 和 `thread` 的每会话覆盖。
  - 如果你只设置了旧版 `session.idleMinutes` 而没有任何 `reset`/`resetByType`，SupportSquadAI 保持仅空闲模式以向后兼容。
...
控制内置白名单、安装偏好、额外 Skills 文件夹和每 Skills 覆盖。适用于**内置**Skills 和 `~/.supportsquadai/skills`（工作区 Skills 在名称冲突时仍然优先）。
...
控制插件发现、允许/拒绝和每插件配置。插件从 `~/.supportsquadai/extensions`、`<workspace>/.supportsquadai/extensions` 以及任何 `plugins.load.paths` 条目加载。**配置更改需要重启 Gateway 网关。**
...
### `browser`（SupportSquadAI 管理的浏览器）

SupportSquadAI 可以为 SupportSquadAI 启动一个**专用、隔离的** Chrome/Brave/Edge/Chromium 实例并暴露一个小型 local loopback 控制服务。
...
- 配置文件颜色：`#FF4500`（龙虾橙）
- 注意：控制服务器由运行中的 Gateway 网关（SupportSquadAI.app 菜单栏或 `supportsquadai gateway`）启动。
...
```json5
{
  browser: {
    ...
    profiles: {
      supportsquadai: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/supportsquadai" }
...
- 示例：`"/ui"`、`"/supportsquadai"`、`"/apps/supportsquadai"`。
- 默认：根路径（`/`）（不变）。
...
- 当连接来自这些 IP 之一时，SupportSquadAI 使用 `x-forwarded-for`（或 `x-real-ip`）来确定客户端 IP，用于本地配对检查和 HTTP 认证/本地检查。
- 仅列出你完全控制的代理，并确保它们**覆盖**传入的 `x-forwarded-for`。

说明：

- `supportsquadai gateway` 拒绝启动，除非 `gateway.mode` 设为 `local`（或你传递了覆盖标志）。
...
- 优先级：`--port` > `SUPPORTSQUADAI_GATEWAY_PORT` > `gateway.port` > 默认 `18789`。
...
- `gateway.auth.password` 可在此设置，或通过 `SUPPORTSQUADAI_GATEWAY_PASSWORD`（推荐）。
- `gateway.auth.allowTailscale` 允许 Tailscale Serve 身份头部
  （`tailscale-user-login`）在请求通过 local loopback 到达且带有 `x-forwarded-for`、
  `x-forwarded-proto` 和 `x-forwarded-host` 时满足认证。SupportSquadAI 在接受之前
...
- SupportSquadAI.app 监视 `~/.supportsquadai/supportsquadai.json`，当 `gateway.mode` 或 `gateway.remote.url` 变更时实时切换模式。
...
Gateway 网关监视 `~/.supportsquadai/supportsquadai.json`（或 `SUPPORTSQUADAI_CONFIG_PATH`）并自动应用更改。
...
- `~/.supportsquadai/supportsquadai.json`（或 `SUPPORTSQUADAI_CONFIG_PATH`）
...
- `SUPPORTSQUADAI_STATE_DIR`（会话/凭据）
- `agents.defaults.workspace`（记忆）
- `gateway.port`（每实例唯一）

便利标志（CLI）：

- `supportsquadai --dev …` → 使用 `~/.supportsquadai-dev` + 端口从基础 `19001` 偏移
- `supportsquadai --profile <name> …` → 使用 `~/.supportsquadai-<name>`（端口通过配置/环境变量/标志）
...
```bash
SUPPORTSQUADAI_CONFIG_PATH=~/.supportsquadai/a.json \
SUPPORTSQUADAI_STATE_DIR=~/.supportsquadai-a \
supportsquadai gateway --port 19001
```
...
```json5
{
  hooks: {
    ...
    path: "/hooks",
    presets: ["gmail"],
    transformsDir: "~/.supportsquadai/hooks",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
- `x-supportsquadai-token: <token>` **或**
- `?token=<token>`
...
Gmail 辅助配置（由 `supportsquadai webhooks gmail setup` / `run` 使用）：

```json5
{
  hooks: {
    gmail: {
      account: "supportsquadai@gmail.com",
...
- 设置 `SUPPORTSQUADAI_SKIP_GMAIL_WATCHER=1` 禁用自动启动（用于手动运行）。
- 避免在 Gateway 网关旁边单独运行 `gog gmail watch serve`；它会
  因 `listen tcp 127.0.0.1:8788: bind: address already in use` 而失败。

注意：当 `tailscale.mode` 开启时，SupportSquadAI 将 `serve.path` 默认为 `/`，以便
...
默认根目录：`~/.supportsquadai/workspace/canvas`
默认端口：`18793`（选择此端口以避免 SupportSquadAI 浏览器 CDP 端口 `18792`）
...
- 监视目录并通过 `/__supportsquadai__/ws` 的 WebSocket 端点广播重载
- 目录为空时自动创建起始 `index.html`（以便你立即看到内容）
- 同时在 `/__supportsquadai__/a2ui/` 提供 A2UI，并作为 `canvasHostUrl` 通告给节点
...
{
  canvasHost: {
    root: "~/.supportsquadai/workspace/canvas",
    port: 18793,
    liveReload: true,
  },
}
...
- 环境变量：`SUPPORTSQUADAI_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // 省略时使用 ~/.supportsquadai/bridge/tls/bridge-{cert,key}.pem。
      // certPath: "~/.supportsquadai/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.supportsquadai/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
控制 LAN mDNS 发现广播（`_supportsquadai-gw._tcp`）。

- `minimal`（默认）：从 TXT 记录中省略 `cliPath` + `sshPort`
- `full`：在 TXT 记录中包含 `cliPath` + `sshPort`
- `off`：完全禁用 mDNS 广播
- 主机名：默认为 `supportsquadai`（通告 `supportsquadai.local`）。通过 `SUPPORTSQUADAI_MDNS_HOSTNAME` 覆盖。
...
启用后，Gateway 网关在 `~/.supportsquadai/dns/` 下使用配置的发现域（示例：`supportsquadai.internal.`）为 `_supportsquadai-gw._tcp` 写入单播 DNS-SD 区域。
