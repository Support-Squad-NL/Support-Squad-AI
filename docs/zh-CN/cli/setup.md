...
read_when:
  ...
  - 你想设置默认工作区路径
summary: "`supportsquadai setup` 的 CLI 参考（初始化配置 + 工作区）"
title: setup
x-i18n:
...
初始化 `~/.supportsquadai/supportsquadai.json` 和智能体工作区。
...
```bash
supportsquadai setup
supportsquadai setup --workspace ~/.supportsquadai/workspace
```

通过 setup 运行向导：

```bash
supportsquadai setup --wizard
```
