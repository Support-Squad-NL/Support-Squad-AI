...
- `~/Library/Application Support/SupportSquadAI/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `supportsquadai-canvas://<session>/<path>`

示例：

- `supportsquadai-canvas://main/` → `<canvasRoot>/main/index.html`
- `supportsquadai-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `supportsquadai-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
supportsquadai nodes canvas present --node <id>
supportsquadai nodes canvas navigate --node <id> --url "/"
supportsquadai nodes canvas eval --node <id> --js "document.title"
supportsquadai nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__supportsquadai__/a2ui/
```
...
supportsquadai nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
supportsquadai nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "supportsquadai://agent?message=Review%20this%20design";
```
