...
summary: SupportSquadAI macOS 配套应用（菜单栏 + Gateway 网关代理）
title: macOS 应用
x-i18n:
...
# SupportSquadAI macOS 配套应用（菜单栏 + Gateway 网关代理）

macOS 应用是 SupportSquadAI 的**菜单栏配套应用**。它拥有权限，在本地管理/附加到 Gateway 网关（launchd 或手动），并作为节点向智能体暴露 macOS 功能。
...
- 根据请求通过 npm/pnpm 安装全局 CLI（`supportsquadai`）（不建议使用 bun 作为 Gateway 网关运行时）。

## 本地 vs 远程模式

- **本地**（默认）：如果存在运行中的本地 Gateway 网关，应用附加到它；否则通过 `supportsquadai gateway install` 启用 launchd 服务。
...
应用管理一个标记为 `bot.molt.gateway` 的每用户 LaunchAgent（使用 `--profile`/`SUPPORTSQUADAI_PROFILE` 时为 `bot.molt.<profile>`；旧版 `com.supportsquadai.*` 仍会卸载）。
...
如果 LaunchAgent 未安装，从应用中启用它或运行 `supportsquadai gateway install`。
...
```
~/.supportsquadai/exec-approvals.json
```
...
应用为本地操作注册 `supportsquadai://` URL 方案。

### `supportsquadai://agent`

触发 Gateway 网关 `agent` 请求。

```bash
open 'supportsquadai://agent?message=Hello%20from%20deep%20link'
```
...
- `swift run SupportSquadAI`（或 Xcode）
...
```bash
cd apps/macos
swift run supportsquadai-mac connect --json
swift run supportsquadai-mac discover --timeout 3000 --json
```
...
提示：与 `supportsquadai gateway discover --json` 比较，查看 macOS 应用的发现管道（NWBrowser + tailnet DNS-SD 回退）是否与 Node CLI 基于 `dns-sd` 的发现不同。
