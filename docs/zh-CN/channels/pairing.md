...
"配对"是 SupportSquadAI 的显式**所有者批准**步骤。它用于两个地方：
...
存储在 `~/.supportsquadai/credentials/` 下：
...
- 旧版 `node.pair.*` API（CLI：`supportsquadai nodes pending/approve`）是一个单独的 Gateway 网关拥有的配对存储。WS 节点仍然需要设备配对。
