...
summary: "Architecture of SupportSquadAI's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in SupportSquadAI
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

# Pi Integration Architecture

This document describes how SupportSquadAI integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

SupportSquadAI uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, SupportSquadAI directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in SupportSquadAI's local TUI mode)                                             |
...
├── pi-tools.ts                    # createSupportSquadAICodingTools()
...
├── supportsquadai-tools.ts              # SupportSquadAI-specific tools
...
2. **Custom Replacements**: SupportSquadAI replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **SupportSquadAI Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
SupportSquadAI wraps this with `guardSessionManager()` for tool result safety.
...
SupportSquadAI maintains an auth profile store with multiple API keys per provider:
...
SupportSquadAI loads custom pi extensions for specialized behavior:
...
SupportSquadAI also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | SupportSquadAI Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom SupportSquadAI tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.supportsquadai/agents/<agentId>/sessions/` (or `$SUPPORTSQUADAI_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `SUPPORTSQUADAI_LIVE_TEST=1`)
