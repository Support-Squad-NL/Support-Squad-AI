...
Deploy SupportSquadAI on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/supportsquadai/supportsquadai)
...
repository configures everything needed to run SupportSquadAI:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: SUPPORTSQUADAI_STATE_DIR
        value: /data/.supportsquadai
      - key: SUPPORTSQUADAI_WORKSPACE_DIR
        value: /data/workspace
      - key: SUPPORTSQUADAI_GATEWAY_TOKEN
        generateValue: true # auto-generates a secure token
    disk:
      name: supportsquadai-data
      mountPath: /data
      sizeGB: 1
```
...
The web dashboard is available at `https://<your-service>.onrender.com/supportsquadai`.
...
If you use the original SupportSquadAI repository, Render will not auto-deploy your SupportSquadAI. To update it, run a manual Blueprint sync from the dashboard.
...
For SupportSquadAI, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any SupportSquadAI host.
