...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in SupportSquadAI
...
Running SupportSquadAI via Node with `tsx` fails at startup with:

```
[supportsquadai] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `supportsquadai status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch supportsquadai.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node supportsquadai.mjs status` works on Node 25.
