---
summary: "Agent tool surface for SupportSquadAI (browser, canvas, nodes, message, cron) replacing legacy `supportsquadai-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `supportsquadai-*` skills
title: "Tools"
---

# Tools (SupportSquadAI)

SupportSquadAI exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `supportsquadai-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `supportsquadai.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, SupportSquadAI logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:supportsquadai`: all built-in SupportSquadAI tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`supportsquadai node run`).
...
SupportSquadAI tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires a Brave API key (recommended: `supportsquadai configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated SupportSquadAI-managed browser.
...
- Quick smoke: `supportsquadai nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `supportsquadai gateway` restart in-place)
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, SupportSquadAI clamps `tools.sessions.visibility` to `tree`.
