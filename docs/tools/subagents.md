...
- For manual spawns, delivery is resilient:
  - SupportSquadAI tries direct `agent` delivery first with a stable idempotency key.
...
2. SupportSquadAI creates or binds a thread to that session target in the active channel.
