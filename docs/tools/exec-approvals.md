...
- **gateway host** → `supportsquadai` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.supportsquadai/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.supportsquadai/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `supportsquadai approvals allowlist ...`).
- `supportsquadai security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `supportsquadai doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.supportsquadai/exec-approvals.json` directly.

CLI: `supportsquadai approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
