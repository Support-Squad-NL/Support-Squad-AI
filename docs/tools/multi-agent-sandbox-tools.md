...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `supportsquadai sandbox explain`.
...
Legacy `agent.*` configs are migrated by `supportsquadai doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=supportsquadai-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${SUPPORTSQUADAI_STATE_DIR:-$HOME/.supportsquadai}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
