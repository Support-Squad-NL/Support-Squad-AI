...
- `gateway`/`node` approvals are controlled by `~/.supportsquadai/exec-approvals.json`.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  SupportSquadAI prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`supportsquadai security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `supportsquadai doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
