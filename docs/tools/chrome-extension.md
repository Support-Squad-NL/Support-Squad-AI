---
summary: "Chrome extension: let SupportSquadAI drive your existing Chrome tab"
read_when:
...
The SupportSquadAI Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate supportsquadai-managed Chrome profile.
...
SupportSquadAI then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the SupportSquadAI release (npm package) as static files. There is no separate “build” step.

After upgrading SupportSquadAI:

- Re-run `supportsquadai browser extension install` to refresh the installed files under your SupportSquadAI state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

SupportSquadAI ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `SUPPORTSQUADAI_GATEWAY_TOKEN`)
...
- Open the tab you want SupportSquadAI to control.
...
- `ON`: attached; SupportSquadAI can drive that tab.
...
Debugging: `supportsquadai sandbox explain`
...
`supportsquadai browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `supportsquadai browser extension install` first to copy the extension to a stable location under your SupportSquadAI state directory.
...
- **This is not isolated** like the dedicated supportsquadai-managed profile.
