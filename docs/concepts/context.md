...
read_when:
  - You want to understand what “context” means in SupportSquadAI
...
“Context” is **everything SupportSquadAI sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (SupportSquadAI-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How SupportSquadAI builds the system prompt

The system prompt is **SupportSquadAI-owned** and rebuilt each run. It includes:
...
By default, SupportSquadAI injects a fixed set of workspace files (if present):
