...
- `/usage cost` in chats: local cost summary aggregated from SupportSquadAI session logs.
- CLI: `supportsquadai status --usage` prints a full per-provider breakdown.
- CLI: `supportsquadai channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
