...
- Same path: run the app from a fixed location (for SupportSquadAI, `dist/SupportSquadAI.app`).
...
Workaround: move files into the SupportSquadAI workspace (`~/.supportsquadai/workspace`) if you want to avoid per-folder grants.
