...
service via the external `supportsquadai` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`SUPPORTSQUADAI_PROFILE`; legacy `com.supportsquadai.*` is supported).
