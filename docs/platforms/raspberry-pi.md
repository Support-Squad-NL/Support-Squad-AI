---
summary: "SupportSquadAI on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up SupportSquadAI on a Raspberry Pi
  - Running SupportSquadAI on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# SupportSquadAI on Raspberry Pi

## Goal

Run a persistent, always-on SupportSquadAI Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most SupportSquadAI features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled supportsquadai

# Enable if not
sudo systemctl enable supportsquadai

# Start on boot
sudo systemctl start supportsquadai
```
...
```bash
# Check logs
journalctl -u supportsquadai --no-pager -n 100

# Common fix: rebuild
cd ~/supportsquadai  # if using hackable install
npm run build
sudo systemctl restart supportsquadai
```
