...
```bash
supportsquadai tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `supportsquadai tui --deliver`
...
- Check the Gateway logs: `supportsquadai logs --follow`.
- Confirm the agent can run: `supportsquadai status` and `supportsquadai models status`.
...
- No agents in picker: check `supportsquadai agents list` and your routing config.
