...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/supportsquadai`)
...
If the page fails to load, start the Gateway first: `supportsquadai gateway`.
...
you revoke it with `supportsquadai devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.supportsquadai/supportsquadai.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. SupportSquadAI
...
SupportSquadAI **blocks** Control UI connections without device identity.
