...
description: "Complete field-by-field reference for ~/.supportsquadai/supportsquadai.json"
summary: "Complete reference for every SupportSquadAI config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.supportsquadai/supportsquadai.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — SupportSquadAI uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `supportsquadai doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["supportsquadai-dm"] },
      guilds: {
        "123456789012345678": {
...
Mattermost ships as a plugin: `supportsquadai plugins install @supportsquadai/mattermost`.
...
SupportSquadAI spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@supportsquadai"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `supportsquadai.json`).
...
Default: `~/.supportsquadai/workspace`.
...
Optional repository root shown in the system prompt's Runtime line. If unset, SupportSquadAI auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, SupportSquadAI assumes `anthropic` (deprecated).
    ...
    defaults: {
      sandbox: {
        ...
        browser: {
          ...
          network: "supportsquadai-sandbox-browser",
          cdpPort: 9222,
...
- `none`: per-scope sandbox workspace under `~/.supportsquadai/sandboxes`
...
noVNC observer access uses VNC auth by default and SupportSquadAI emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `supportsquadai-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.supportsquadai/workspace",
        agentDir: "~/.supportsquadai/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@supportsquadai"] },
        sandbox: { mode: "off" },
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:supportsquadai`   | All built-in tools (excludes provider plugins)                                           |
...
SupportSquadAI uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.supportsquadai/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `SUPPORTSQUADAI_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `supportsquadai onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `supportsquadai onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `supportsquadai onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `supportsquadai onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `supportsquadai onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `supportsquadai onboard --auth-choice minimax-api`.
...
- Loaded from `~/.supportsquadai/extensions`, `<workspace>/.supportsquadai/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or SUPPORTSQUADAI_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `SUPPORTSQUADAI_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.supportsquadai-dev` + port `19001`), `--profile <name>` (uses `~/.supportsquadai-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.supportsquadai/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-supportsquadai-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `SUPPORTSQUADAI_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or SUPPORTSQUADAI_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__supportsquadai__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__supportsquadai__/a2ui/`
...
- Also serves A2UI at `/__supportsquadai__/a2ui/`.
...
- Hostname defaults to `supportsquadai`. Override with `SUPPORTSQUADAI_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.supportsquadai/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `supportsquadai dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.supportsquadai/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${SUPPORTSQUADAI_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.supportsquadai/credentials/oauth.json`.
...
- Default log file: `/tmp/supportsquadai/supportsquadai-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `supportsquadai doctor --fix` can strip unknown keys).
