...
- During heartbeat runs, SupportSquadAI treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `supportsquadai sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, SupportSquadAI skips the heartbeat run entirely (no model call).
...
```bash
supportsquadai system event --text "Check for urgent follow-ups" --mode now
```
