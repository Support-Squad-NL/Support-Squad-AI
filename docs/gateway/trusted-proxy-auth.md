...
read_when:
  - Running SupportSquadAI behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of SupportSquadAI
...
- You run SupportSquadAI behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. SupportSquadAI checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. SupportSquadAI extracts the user identity from the configured header
...
```yaml
routes:
  - from: https://supportsquadai.example.com
    to: http://supportsquadai-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
supportsquadai.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy supportsquadai:18789 {
...
location / {
    ...
    proxy_pass http://supportsquadai:18789;
...
`supportsquadai security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update SupportSquadAI config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `supportsquadai security audit` and review findings
