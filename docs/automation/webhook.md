...
read_when:
  ...
  - Wiring external systems into SupportSquadAI
title: "Webhooks"
---
...
```json5
{
  hooks: {
    enabled: true,
    token: "${SUPPORTSQUADAI_HOOKS_TOKEN}",
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your SupportSquadAI config directory (typically `~/.supportsquadai/hooks/transforms`).
...
- `supportsquadai webhooks gmail setup` writes `hooks.gmail` config for `supportsquadai webhooks gmail run`.
...
curl -X POST http://127.0.0.1:18789/hooks/agent \
  -H 'x-supportsquadai-token: SECRET' \
  -H 'Content-Type: application/json' \
