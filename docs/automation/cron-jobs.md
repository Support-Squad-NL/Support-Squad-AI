...
- Jobs persist under `~/.supportsquadai/cron/` so restarts don’t lose schedules.
...
supportsquadai cron run <job-id>
supportsquadai cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.supportsquadai/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `supportsquadai cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, SupportSquadAI applies a
...
If `delivery` is omitted for isolated jobs, SupportSquadAI defaults to `announce`.
...
- Job store: `~/.supportsquadai/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.supportsquadai/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
```json5
{
  cron: {
    ...
    store: "~/.supportsquadai/cron/jobs.json",
...
- `SUPPORTSQUADAI_SKIP_CRON=1` (env)
...
supportsquadai cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
supportsquadai cron edit <jobId> --agent ops
supportsquadai cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
supportsquadai cron run <jobId>
supportsquadai cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
supportsquadai cron edit <jobId> \
...
```bash
supportsquadai cron edit <jobId> --exact
```
...
```bash
supportsquadai system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`supportsquadai system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `SUPPORTSQUADAI_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- SupportSquadAI applies exponential retry backoff for recurring jobs after consecutive errors:
