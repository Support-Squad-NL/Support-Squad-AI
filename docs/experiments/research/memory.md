...
read_when:
  - Designing workspace memory (~/.supportsquadai/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep SupportSquadAI integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.supportsquadai/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.supportsquadai/workspace` as canonical human-readable memory.
...
```
~/.supportsquadai/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in SupportSquadAI**, but keep a separable core library.

### Why integrate into SupportSquadAI?

- SupportSquadAI already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `supportsquadai memory recall "…" --k 25 --since 30d`
  - `supportsquadai memory reflect --since 7d`
...
Pragmatic take for `~/.supportsquadai/workspace`:
