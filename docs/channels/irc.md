---
title: IRC
description: Connect SupportSquadAI to IRC channels and direct messages.
summary: "IRC plugin setup, access controls, and troubleshooting"
read_when:
  - You want to connect SupportSquadAI to IRC channels or DMs
  - You are configuring IRC allowlists, group policy, or mention gating
---

Use IRC when you want SupportSquadAI in classic channels (`#room`) and direct messages.
IRC ships as an extension plugin, but it is configured in the main config under `channels.irc`.

## Quick start

1. Enable IRC config in `~/.supportsquadai/supportsquadai.json`.
2. Set at least:
...
```json
{
  "channels": {
    "irc": {
      ...
      "port": 6697,
      "tls": true,
      "nick": "supportsquadai-bot",
      "channels": ["#supportsquadai"]
    }
  }
}
```
...
Even if a channel is allowed (via `groupPolicy` + `groups`) and the sender is allowed, SupportSquadAI defaults to **mention-gating** in group contexts.
