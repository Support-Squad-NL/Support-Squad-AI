...
Tlon is a decentralized messenger built on Urbit. SupportSquadAI connects to your Urbit ship and can
...
```bash
supportsquadai plugins install @supportsquadai/tlon
```

Local checkout (when running from a git repo):

```bash
supportsquadai plugins install ./extensions/tlon
```
...
By default, SupportSquadAI blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `supportsquadai message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, SupportSquadAI replies in-thread.
