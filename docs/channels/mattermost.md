---
summary: "Mattermost bot setup and SupportSquadAI config"
read_when:
...
```bash
supportsquadai plugins install @supportsquadai/mattermost
```

Local checkout (when running from a git repo):

```bash
supportsquadai plugins install ./extensions/mattermost
```
...
4. Configure SupportSquadAI and start the gateway.
...
- Approve via:
  - `supportsquadai pairing list mattermost`
  - `supportsquadai pairing approve mattermost <CODE>`
...
Use these target formats with `supportsquadai message send` or cron/webhooks:
