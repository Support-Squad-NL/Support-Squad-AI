...
```bash
supportsquadai plugins install @supportsquadai/msteams
```

Local checkout (when running from a git repo):

```bash
supportsquadai plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
SupportSquadAI will offer the local install path automatically.
...
3. Configure SupportSquadAI with those credentials.
...
- Talk to SupportSquadAI via Teams DMs, group chats, or channels.
...
- On startup, SupportSquadAI resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.supportsquadai/supportsquadai.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring SupportSquadAI, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `supportsquadai-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `supportsquadai plugins install @supportsquadai/msteams`
   - From a local checkout: `supportsquadai plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "SupportSquadAI in Teams", "full": "SupportSquadAI in Teams" },
...
SupportSquadAI handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, SupportSquadAI only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/SupportSquadAIShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

SupportSquadAI sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `supportsquadai message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.supportsquadai/msteams-polls.json`.
...
```bash
supportsquadai message send --channel msteams \
...
supportsquadai message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
supportsquadai message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
supportsquadai message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
supportsquadai message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
