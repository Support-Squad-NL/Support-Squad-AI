...
read_when:
  - Setting up Twitch chat integration for SupportSquadAI
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. SupportSquadAI connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
supportsquadai plugins install @supportsquadai/twitch
```

Local checkout (when running from a git repo):

```bash
supportsquadai plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `SUPPORTSQUADAI_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "supportsquadai", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use SUPPORTSQUADAI_TWITCH_ACCESS_TOKEN env var)
...
```bash
SUPPORTSQUADAI_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
