...
Matrix is an open, decentralized messaging protocol. SupportSquadAI connects as a Matrix **user**
...
```bash
supportsquadai plugins install @supportsquadai/matrix
```

Local checkout (when running from a git repo):

```bash
supportsquadai plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `supportsquadai plugins install @supportsquadai/matrix`
   - From a local checkout: `supportsquadai plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: SupportSquadAI calls the same
     login endpoint, stores the access token in `~/.supportsquadai/credentials/matrix/credentials.json`,
...
- On first connection, SupportSquadAI requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  SupportSquadAI logs a warning.
...
`~/.supportsquadai/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `supportsquadai pairing list matrix`
  - `supportsquadai pairing approve matrix <CODE>`
...
- On startup, SupportSquadAI resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
