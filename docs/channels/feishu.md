...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects SupportSquadAI to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
supportsquadai plugins install @supportsquadai/feishu
```

Local checkout (when running from a git repo):

```bash
supportsquadai plugins install ./extensions/feishu
```
...
If you just installed SupportSquadAI, run the wizard:
...
2. Configuring app credentials in SupportSquadAI
...
1. You already ran `supportsquadai channels add` for Feishu
2. The gateway is running (`supportsquadai gateway status`)
...
## Step 2: Configure SupportSquadAI
...
Edit `~/.supportsquadai/supportsquadai.json`:
...
```bash
supportsquadai pairing approve feishu <CODE>
```
...
- **Approve pairing**:
  ...
  ```bash
  supportsquadai pairing list feishu
  supportsquadai pairing approve feishu <CODE>
  ```
...
2. Run `supportsquadai logs --follow` and look for `chat_id`
...
2. Run `supportsquadai logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `supportsquadai gateway status`  | Show gateway status           |
| `supportsquadai gateway install` | Install/start gateway service |
| `supportsquadai gateway stop`    | Stop gateway service          |
| `supportsquadai gateway restart` | Restart gateway service       |
| `supportsquadai logs --follow`   | Tail gateway logs             |
...
4. Check logs: `supportsquadai logs --follow`
...
5. Ensure the gateway is running: `supportsquadai gateway status`
6. Check logs: `supportsquadai logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.supportsquadai/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.supportsquadai/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
