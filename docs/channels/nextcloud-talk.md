...
```bash
supportsquadai plugins install @supportsquadai/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
supportsquadai plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "SupportSquadAI" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `supportsquadai pairing list nextcloud-talk`
  - `supportsquadai pairing approve nextcloud-talk <CODE>`
