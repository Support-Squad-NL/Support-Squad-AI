---
summary: "Use Venice AI privacy-focused models in SupportSquadAI"
read_when:
  - You want privacy-focused inference in SupportSquadAI
...
```bash
supportsquadai onboard --auth-choice venice-api-key
```
...
```bash
supportsquadai agent --model venice/llama-3.3-70b --message "Hello, are you working?"
```

## Model Selection

After setup, SupportSquadAI shows all available Venice models. Pick based on your needs:
...
```bash
supportsquadai models set venice/claude-opus-45
supportsquadai models set venice/llama-3.3-70b
```

List all available models:

```bash
supportsquadai models list | grep venice
```

## Configure via `supportsquadai configure`

1. Run `supportsquadai configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
SupportSquadAI automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
supportsquadai agent --model venice/llama-3.3-70b --message "Quick health check"

# Use Claude via Venice (anonymized)
supportsquadai agent --model venice/claude-opus-45 --message "Summarize this task"

# Use uncensored model
supportsquadai agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
supportsquadai agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
supportsquadai agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `supportsquadai models list` to see currently available models. Some models may be temporarily offline.
