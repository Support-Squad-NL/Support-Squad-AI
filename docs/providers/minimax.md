---
summary: "Use MiniMax M2.1 in SupportSquadAI"
read_when:
  - You want MiniMax models in SupportSquadAI
...
```bash
supportsquadai plugins enable minimax-portal-auth  # skip if already loaded.
supportsquadai gateway restart  # restart if gateway is already running
supportsquadai onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/supportsquadai/supportsquadai/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `supportsquadai.json`:
...
- Use `supportsquadai models list` and `supportsquadai models set minimax/MiniMax-M2.1` to switch.
...
- Running `supportsquadai configure` and selecting **MiniMax M2.1**, or
